...
### Windows
Download the [Proxy Manager installer](https://github.com/luminati-io/luminati-proxy/releases/download/v1.670.998/luminati-proxy-manager-v1.670.998-setup.exe)
...
- Install Node.js ([nodejs.org](https://nodejs.org/en/download/))
  Node.js version for the proxy manager should be at least 24.13.0
...
```
If you are trying to install the Proxy Manager from china on Mac/Linux and the default npm registry is unavailable, use the alternative registry:
```sh
sudo npm install -g @luminati-io/luminati-proxy --legacy-peer-deps --registry=https://r.cnpmjs.org/
```
### Upgrade
- Use npm to upgrade
```sh
...
The `--ssl` parameter enables SSL analyzing. HTTPS requests can be made without
enabling SSL analyzing.

#### Production CA certificate

The CA certificates bundled with Proxy Manager are intended for testing
purposes only and must not be used in production.

For production deployments, provide your own CA certificate and matching
private key in the Proxy Manager working directory:

```text
lpm.crt
lpm.key
```
The files must be PEM-encoded and contain a valid matching certificate and
private key. Proxy Manager gives these files priority over the bundled
certificates.

```md
The working directory can be configured using the `--dir` option:

```sh
proxy-manager --dir /path/to/working-directory
proxy-manager --dir /path/to/working-directory
```
```md
Keep `lpm.key` private, restrict its file permissions, and never commit or
distribute it. Restart Proxy Manager after adding or replacing the files.

Clients using SSL analyzing must trust the corresponding `lpm.crt` CA
certificate.
