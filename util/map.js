...
class Notify_map extends Map {
    ...
    #strict;
    constructor(opt={}){
        const {iterable, on_change, on_set, on_delete, on_clear, on_changeset,
            compare_equal, strict} = opt;
        super();
        this.#on_change = typeof on_change == 'function' ? on_change : _.noop;
        this.#on_set = typeof on_set == 'function' ? on_set : _.noop;
        this.#on_delete = typeof on_delete == 'function' ? on_delete : _.noop;
        this.#on_clear = typeof on_clear == 'function' ? on_clear : _.noop;
        this.#on_changeset = typeof on_changeset == 'function' ?
            on_changeset : _.noop;
        this.#compare_equal = typeof compare_equal == 'function' ?
            compare_equal : _.isEqual;
        this.#current_changeset = null;
        this.#changes = [];
        this.#strict = !!strict;
        if (iterable)
        {
    ...
    get current_changeset(){
        return this.#current_changeset || undefined;
    }
    get changes(){
        return this.#changes.slice();
    }
    #emit_change(change){
        if (this.#current_changeset)
            return this.#changes.push(change);
        this.#on_change(change);
    }
    set(key, new_val){
        const had_key = this.has(key);
        const old_val = had_key ? this.get(key) : undefined;
        if (had_key && this.#compare_equal(old_val, new_val))
            return this;
        super.set(key, new_val);
        this.#emit_change({type: 'set', key, new_val, old_val, target: this});
        if (!this.#current_changeset)
            this.#on_set({key, new_val, old_val, target: this});
        return this;
    }
    delete(key){
        if (!this.has(key))
            return false;
        const old_val = this.get(key);
        const res = super.delete(key);
        this.#emit_change({type: 'delete', key, old_val, target: this});
        if (!this.#current_changeset)
            this.#on_delete({key, old_val, target: this});
        return res;
    }
    clear(){
        ...
        if (!old_size)
            return this;
        super.clear();
        this.#emit_change({type: 'clear', old_size, target: this});
        if (!this.#current_changeset)
            this.#on_clear({old_size, target: this});
        return this;
    }
    start_changeset(label){
        if (this.#current_changeset)
        {
            if (this.#strict)
                throw new Error('Nested changeset');
            return this;
        }
        if (this.#strict && (!label || typeof label !== 'string'))
            throw new TypeError('Bad changeset label '+label);
        this.#current_changeset = label || 'changeset';
        this.#changes = [];
        return this;
    }
    end_changeset(){
        if (!this.#current_changeset)
            return this;
        const label = this.#current_changeset;
        this.#current_changeset = null;
        if (!this.#changes.length)
            return this;
        const changes = this.#changes.slice();
        this.#changes = [];
        this.#on_change({type: 'changeset', label, changes, target: this});
        this.#on_changeset({label, changes, target: this});
        return this;
    }
    // nested calls join the active changeset
    with_changeset(label, callback){
        ...
        if (typeof callback !== 'function')
            return this;
        const is_join = !!this.#current_changeset;
        if (!is_join)
            this.start_changeset(label);
        try {
