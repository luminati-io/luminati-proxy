...
const logger = require('./logger.js').child({category: 'Tracer'});
const os = require('os');
...
class Tracer {
    ...
    trace(opt={}){
        ...
        return etask(function*tracer_trace(){
            this.on('uncaught', e=>logger.error(e.message));
            let res, next;
            do {
...
class Puppeteer {
    ...
    set_browser_hooks(browser){
        ...
        browser.on('targetcreated', target=>{
            logger.info('<TargetCreated>', target.url());
        });
    }
    set_page_hooks(page){
        page.on('frameattached', frame=>{
            logger.info('<FrameAttached>', frame.name(), frame.url());
        });
        page.on('framenavigated', frame=>{
            logger.info('<FrameNavigated>', frame.name(), frame.url());
        });
        page.on('response', res=>{
            logger.info('<Response>', res.status(), res.url());
        });
        page.on('load', ()=>{
            logger.info('Load event dispatched');
        });
        page.on('pageerror', err=>{
            logger.info('Page error: ', err);
        });
        page.on('requestfailed', req=>{
            logger.info('Req Failed: ', req.url());
        });
        page.on('requestfinished', req=>{
            logger.info('Req Finished: ', req.url());
        });
        page.on('error', err=>{
            logger.info('[Page Error] ', err.message);
        });
        page.on('request', req=>{
            logger.info('Intercept '+req.resourceType()+' '+req.url());
            req.continue();
        });
    }
    ...
    return etask(function*(){
        ...
        logger.info('Launching browser with the following args: '
            +JSON.stringify(args));
        ...
        const main_task = etask(function*(){
            this.on('uncaught', e=>logger.error('main_task'+e.message));
            try {
                yield page.goto(url,
                ...
                err = e.message;
                logger.warn('loading page in chromium:\n'+err);
                done = true;
            }
        });
        ...
        if (err)
        {
            logger.error('had to close browser because of the error');
            yield browser.close();
            return {err};
        }
        let ss = {};
        ...
        } catch(e){ logger.warn('make_screenshot\n'+e.message); }
        /* jshint -W117 */
        let html;
        try {
        ...
        } catch(e){ logger.warn('evaluate page\n'+e.message); }
        yield browser.close();
