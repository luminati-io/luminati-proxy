...
const fetch = endpoint=>etask(function*(){
    ...
    try {
        ...
        res = yield util.json(endpoint, {headers, timeout: 20*date.ms.SEC});
    } catch(e){
        res = {};
    }
    return res;
...
const get_current_carrier_version = domain=>etask(function*(){
    const res = yield fetch(`http://client.${domain}/api/carriers/version`);
    if (res.statusCode==200)
        return res.body;
    logger.warn('Unable to get current carriers version. Carriers will not be '
    +'displayed');
});

const get_current_carrier_data = domain=>etask(function*(){
    const res = yield fetch(`http://client.${domain}/api/carriers/asns`);
    if (res.statusCode==200)
        return res.body;
    logger.warn('Unable to get current carrier data. Carriers will not be '+
    'displayed');
});

const get_asn_carrier_map = domain=>etask(function*(){
    ...
    const current_carrier_version = yield get_current_carrier_version(domain);
    ...
    if (carrier2asn.version != current_carrier_version)
    {
        ...
        const carrier_data = yield get_current_carrier_data(domain);
        if (!carrier_data)
            return {};
...
E.ensure_data = www_domain=>etask(function*(){
    ...
    const [vipdb_data, asn2carrier] = yield etask.all(
        [locations.all_locations(), get_asn_carrier_map(www_domain)]);
    const csv_data = [];
