...
E.ensure_data = (mgr, clear=false)=>etask(function*ensure_data(){
    ...
    try {
        ...
        util.reset_schedule_retry(this.name);
    } catch(e){
        logger.warn('Unable to fetch updated data: %s', e.message);
        if (!util.schedule_retry(this.name, ()=>E.ensure_data()))
        {
            logger.notice('Can not retry, falling back to default');
