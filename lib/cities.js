...
const ENDPOINT_ATTEMPTS = 5;

let data;
let pending;

const fetch = endpoint=>etask(function*(){
    let res = {}, i = 0;
    const headers = {'user-agent': util.user_agent};
    while (i++ < ENDPOINT_ATTEMPTS && res.statusCode != 200)
        res = yield util.json(endpoint, {headers});
    return res;
});

const get_current_carrier_version = ()=>etask(function*(){
    const res = yield fetch('http://client.luminati.io/api/carriers/version');
    if (res.statusCode==200)
        return res.body;
    logger.notice('Unable to get current carriers version: %s %s %s',
        res.statusCode, res.statusMessage, res.body);
});

const get_current_carrier_data = ()=>etask(function*(){
    const res = yield fetch('http://client.luminati.io/api/carriers/asns');
    if (res.statusCode==200)
        return res.body;
    logger.notice('Unable to get current carrier data: %s %s %s',
        res.statusCode, res.statusMessage, res.body);
});

const get_asn_carrier_map = ()=>etask(function*(){
    const carriers_path = path.resolve(__dirname, '../db/carriers.json');
    let carrier2asn = {};
    try { carrier2asn = JSON.parse(file.read_e(carriers_path)); }
    catch(e){
        if (e.code != 'ENOENT')
            logger.notice('Failed to parse carriers %s', zerr.e2s(e));
    }
    const current_carrier_version = yield get_current_carrier_version();
    if (!current_carrier_version)
        return yield get_asn_carrier_map();
    if (carrier2asn.version != current_carrier_version)
    {
        logger.notice('Fetching carriers data. New version %s available '
        +'(current is %s)', current_carrier_version, carrier2asn.version);
        const carrier_data = yield get_current_carrier_data();
        if (!carrier_data)
            return yield get_asn_carrier_map();
    ...
    for (const carrier in carrier2asn.data)
    {
        for (const asn of carrier2asn.data[carrier])
        {
            ...
            {
                console.log('conflict on asn %s, %s, %s', asn,
...
const prepare_data = (csv_data, vipdb_data, asn2carrier)=>{
    ...
    return {cities, regions, countries, asns: vipdb_data.asns, carriers};
};
...
E.ensure_data = ()=>etask(function*(){
    ...
    const [vipdb_data, asn2carrier] = yield etask.all(
        [locations.all_locations(), get_asn_carrier_map()]);
    const csv_data = [];
