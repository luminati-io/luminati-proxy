...
const get_current_carrier_version = domain=>etask(function*(){
    const res = yield util.fetch(
        `http://client.${domain}/api/carriers/version`);
...
const get_current_carrier_data = domain=>etask(function*(){
    const res = yield util.fetch(`http://client.${domain}/api/carriers/asns`);
    if (res.statusCode==200)
        return res.body;
...
E.ensure_data = www_domain=>etask(function*(){
    ...
    const [vipdb_data, asn2carrier] = yield etask.all([
        locations.all_locations(www_domain),
        get_asn_carrier_map(www_domain),
    ]);
    const csv_data = [];
