...
Sess_mgr.prototype.update_last_ip = function(ip){
    // XXX krzysztof: WIP single session based on ip
    return;
    this.last_ip = ip;
    if (this.opt.preset!='session_long' || !this.sessions ||
        !this.sessions.sessions || !this.sessions.sessions[0])
    {
        return;
    }
    this.sessions.sessions[0].ip = ip;
};
