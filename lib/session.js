...
function*set_keep_alive(_this, session, opt={}){
    try {
        ...
        while (session.keep_alive && session.keep_alive==this)
        {
...
function*keep_alive_handler(_this, session){
    ...
    const info = res.info;
    let err;
    const curr_ips = _this.sessions.sessions.map(s=>s.info && s.info.ip);
    const this_ip = session.info && session.info.ip;
    if (proxy_err || res.err || !info)
        err = 'keep alive failed';
    else if (info.ip!=this_ip && curr_ips.includes(info.ip))
        err = `IP ${info.ip} already in the pool`;
    if (err)
    {
        _this.log.warn('removing session %s: %s', session.session, err);
        _this.remove_session(session);
        if (_this.pool_type==E.pool_types.long_availability ||
            _this.pool_type==E.pool_types['round-robin'])
        {
            _this.pool_fetch({immediate: false});
        }
        return;
    }
    ...
    session.info = info;
    session.last_res = {ts: Date.now(), ip: info.ip, session: session.session};
...
Sess_mgr.prototype.serialize_session = function(session){
    return {
        ...
        ip: session.last_res && session.last_res.ip,
        host: session.host,
    ...
    if (this.opt.pool_size)
    {
        ...
        else
        {
            ...
            if (sessions.length!=this.opt.pool_size)
                this.pool(this.opt.pool_size-(sessions.length||0));
        }
        ...
        if (!opt.init && (is_session_expired=this.is_session_expired(session)))
        {
            if (this.pool_type==E.pool_types['round-robin'] &&
