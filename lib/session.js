...
const send_sessions_ui = function(){
    // XXX krzysztof: switch to event based architecture
    if (!this.serv.mgr || !this.serv.mgr.wss || !this.sessions)
        return;
    this.serv.mgr.wss.broadcast({
        payload: this.sessions.sessions.map(this.serialize_session),
        path: 'sessions.'+this.serv.port,
    }, 'global');
};
...
Sess_mgr.prototype.add_to_pool = function(session={}){
    ...
    this.serv.emit('add_static_ip', {ip, port: this.serv.port});
};

Sess_mgr.prototype.refresh_sessions = function(){
    logger.notice('Refreshing all sessions');
...
Sess_mgr.prototype.establish_session = function(prefix, pool, opt={}){
    ...
    logger.info('new session added %s:%s', host, session_id);
    return session;
};
...
function*set_keep_alive(_this, session, opt={}){
    try {
        ...
        logger.info('session %s: keep alive ended', session.session);
    } catch(e){
        logger.error(zerr.e2s(e));
    }
});
...
function*keep_alive_handler(_this, session){
    ...
    if (_this.is_session_expired(session) || _this.serv.stopped)
        return;
    logger.info('Keep alive %s:%s', session.host, session.session);
    ...
    {
        logger.info('session %s: ip change %s -> %s', session.session,
...
Sess_mgr.prototype.replace_session = function(session, err, opt={}){
    logger.info('removing session %s: %s', session.session, err);
