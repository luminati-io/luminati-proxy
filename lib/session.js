...
Sess_mgr.prototype.establish_session = function(hosts, prefix, pool){
...
Sess_mgr.prototype._request_session = function(ctx){
    ...
    if (this.opt.sticky_ip)
    {
        ...
        {
            session = this.sticky_sessions[ip] = this.establish_session(hosts,
    ...
    {
        this.session = this.establish_session(hosts, this.serv.port);
        this.session.count++;
    }
    return this.session;
};
