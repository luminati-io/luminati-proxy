...
Sess_mgr.prototype.init = function(){
    ...
    // XXX krzysztof: is this needed? hosts should always exist
    if (!this.serv.hosts)
        return;
    if (this.opt.ips && this.opt.static)
    {
        this.debug_log('pool init %s, %s total',
            this.opt.ips.slice(0, 4).join(', '), this.opt.ips.length);
...
Sess_mgr.prototype.establish_session = function(prefix, pool, opt={}){
    ...
    this.debug_log('new session added %s:%s', host, session_id);
    return session;
};
...
function*set_keep_alive(_this, session, opt={}){
    try {
        ...
        _this.debug_log('session %s: keep alive ended', session.session);
    } catch(e){
        logger.error(zerr.e2s(e));
    }
});
...
function*keep_alive_handler(_this, session){
    ...
    if (_this.is_session_expired(session) || _this.serv.stopped)
        return;
    _this.debug_log('Keep alive %s:%s', session.host, session.session);
    ...
    {
        _this.debug_log('session %s: ip change %s -> %s', session.session,
...
Sess_mgr.prototype.replace_session = function(session, err, opt={}){
    this.debug_log('removing session %s: %s', session.session, err);
...
Sess_mgr.prototype.debug_log = function(message, ...args){
    message = `Sess_mgr[${this.opt.port}]: `+message;
    logger.log(this.opt.debug_logs ? 'info' : 'debug', message, ...args);
};
