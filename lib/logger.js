...
const get_datadog_transport = (opt={})=>{
    ...
    if (opt.port)
        host += ':'+opt.port;
    const u = new URL(`https://${host}/api/v2/logs`);
    if (opt.token)
        u.searchParams.set('dd-api-key', opt.token);
    if (opt.source)
        u.searchParams.set('ddsource', opt.source);
    if (opt.tags)
        u.searchParams.set('ddtags', opt.tags);
    u.searchParams.set('environment', 'prod');
    return get_http_transport({url: u.href, use_ssl: true});
};
