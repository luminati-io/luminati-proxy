...
module.exports = class Context {
    ...
    // adapted from svc/lum/agent/helpers.js E.sum_header_bytes
    sum_header_bytes(_url, headers){
        let sum = _url ? _url.length+10 : 10;
        for (const name in headers)
        {
            const h = name.toLowerCase();
            if (lpm_headers.includes(h) || /^x-(hola|lpm|luminati)/.test(h))
                continue;
            const val = headers[name]||'';
            if (Array.isArray(val))
                sum += val.reduce((acc, v)=>acc+name.length+v.length+3, 0);
            else
                sum += name.length+val.length+3;
        }
        return sum+2;
    }
    init_req_stats(){
        if (this.is_ssl)
        {
            ...
            this.sp.spawn(sp);
            this.wait_bw = sp;
            const orig_socket = this.req.socket.ssl._parentWrap;
            const lpmbr = orig_socket._lpm_bytesRead;
            const lpmbw = orig_socket._lpm_bytesWritten;
            this.req.socket.once('end', ()=>{
                const pbr = orig_socket.bytesRead;
                const pbw = orig_socket.bytesWritten;
                this.response.in_bw += pbr - lpmbr;
                this.response.out_bw += pbw - lpmbw;
                sp.return();
            });
            this.req.socket.once('close', ()=>sp.return());
            return;
        }
        if (!this.is_connect)
        {
            this.response.in_bw += this.sum_header_bytes(
                this.req.url, this.req.headers);
        }
        const on_data = chunk=>this.response.in_bw += chunk.length;
        const socket = this.is_connect ? this.req.socket : this.req;
        socket.on('data', on_data);
        socket.once('close', ()=>socket.removeListener('data', on_data));
    }
    init_res_stats(proxy_res, socket){
        // already calculated in init_req_stats
        if (this.is_ssl)
            return;
        if (!this.is_connect)
        {
            this.response.out_bw +=
                this.sum_header_bytes(null, proxy_res.headers);
        }
        const on_data = chunk=>this.response.out_bw += chunk.length;
        socket.on('data', on_data);
        socket.once('close', ()=>socket.removeListener('data', on_data));
    }
    ...
    init_response(){
        ...
        this.response = {
            ...
            body: [],
            in_bw: this.response && this.response.in_bw || 0,
            out_bw: this.response && this.response.out_bw || 0,
        };
