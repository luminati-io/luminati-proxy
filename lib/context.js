...
const req_util = {
    ...
    retried_from_ub: (req, serv)=>req.ctx.opt.unblock && !serv.opt.unblock,
    retried_to_ub: (req, serv)=>!req.ctx.opt.unblock && serv.opt.unblock,
};
...
module.exports = class Context {
    ...
    static init_req_ctx(req, res, serv, opt){
        ...
        {
            req.ctx.port = port;
            if (req_util.retried_from_ub(req, serv))
                delete req.ctx.opt.unblock;
            else if (req_util.retried_to_ub(req, serv))
                req.ctx.opt.unblock = true;
        }
