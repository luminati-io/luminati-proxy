...
const lpm_headers = qw`x-hola-context proxy-authorization x-hola-agent
    x-lpm-session x-hola-timeline-debug x-lpm-reserved`;
const req_util = {
...
module.exports = class Context {
    ...
    request_start(){
        ...
        if (this.retry)
            this.timeline.retry(this.port, this.opt.trigger);
    ...
    remove_executed_rules(){
        this.rules_chain.concat([this.rules]).filter(r=>r)
            .forEach(r=>r.remove_executed(this.req));
    }
