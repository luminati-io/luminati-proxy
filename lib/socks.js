...
const logger = require('./logger.js').child({catgory: 'SOCKS'});
...
Socks.prototype.proxy_https = function(info, accept, deny){
    ...
    dst.on('connect', ()=>{
        ...
        dst.once('data', d=>{
            ...
            {
                logger.error('connection error %s:%s %s %s: %s',
            ...
            if (!socket)
            {
                logger.warn('connection closed by client %O', info);
                dst.end();
                return;
            }
...
Socks.prototype._handle_connection = function(socket, dst){
    ...
    socket.on('error', error=>{
        logger.error('Socks connection error, %O', error);
    });
    dst.on('error', error=>{
        logger.error('Socks forward connection error, %O', error);
    });
};
