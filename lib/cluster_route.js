...
const proc = {
    cluster_workers_n: ()=>{ return os.cpus().length; },
};
...
let worker_init = channel_needed=>etask(function*worker_init(){
    for (let i=1; i<=proc.cluster_workers_n(); i++)
    {
        ...
        zerr('cluster_route: w2w_channel, to:%s, from:%s', i,
            cluster.worker.id);
    ...
    process.on('message', (msg, recv_handle)=>{
        ...
        rx_w2w_channel.on('message', (w2w_msg, w2w_recv_handle)=>{
            console.log('cluster_route: channel message', w2w_msg, w2w_recv_handle.fd);
...
E.send = (msg, send_handle, to_worker_id)=>etask(function*send(){
    console.log('cluster_route: send', msg, send_handle._handle.fd, to_worker_id, to_worker_id==cluster.worker.id);
    if (to_worker_id==cluster.worker.id)
        return void E.emit('message', msg, send_handle, cluster.worker.id);
    let w2w_channel;
    console.log('send, has_channel', !!tx_w2w_channel[to_worker_id]);
    ...
    console.log('send send_handle_src', msg.send_handle_src);
    if (send_handle && send_handle.encrypted)
    {
        send_handle._handle.readStart();
        console.log('send wrap socket');
    ...
    console.log('send w2w channel send', msg, send_handle._handle.fd);
...
let select_worker_by_server_id = (socket, req)=>{
    let server_id = socket.route.opt.server_id||"none";
    return hash.hash_string(server_id)%proc.cluster_workers_n()+1;
};
let check_fallback = to_worker_id=>{
    console.log('check_fallback start', to_worker_id);
    if (!to_worker_id)
        to_worker_id = cluster.worker.id;
    if (to_worker_id==cluster.worker.id)
        to_worker_id = cluster.worker.id;
    console.log('check_falback end', to_worker_id, cluster.worker.id);
    return to_worker_id;
};
...
E.setup = (server, opt)=>{
    ...
    else if (opt.select=='server_id')
        assign(opt, {select: select_worker_by_server_id});
    ...
    E.on('cluster_route:connection', (msg, socket)=>{
        console.log('on connection1', msg, opt.server_id, socket._handle.fd);
        ...
        console.log('on connection2', msg.consumed_data);
...
E.init = opt=>{
    opt = opt||{};
    if (cluster.isMaster)
        return void master_init();
    worker_init(opt.channel_needed);
};
