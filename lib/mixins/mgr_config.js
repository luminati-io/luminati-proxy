...
E.prototype.get_settings = function(){
    return {
        ...
        should_sanitize: this.should_sanitize(),
        ...
        new_proxy_cert: !!this._defaults.new_proxy_cert,
        new_proxy_cert_rollout_ts: lpm_config.new_proxy_cert_rollout_ts,
...
E.prototype.schedule_new_proxy_cert_rollout = function(){
    if (lpm_config.manager_default.new_proxy_cert ||
        this._defaults.new_proxy_cert ||
        this.new_proxy_cert_rollout_scheduled)
    {
        return;
    }
    const rollout_ts = lpm_config.new_proxy_cert_rollout_ts;
    const left = rollout_ts-Date.now();
    if (left>0)
    {
        this.new_proxy_cert_rollout_scheduled = true;

        logger.notice('Scheduled new certificate rollout on %s',
            new Date(rollout_ts).toISOString());

        this.timeouts.set_timeout(()=>{
            this.new_proxy_cert_rollout_scheduled = false;
            this.schedule_new_proxy_cert_rollout();
        }, Math.min(left, 0x7fffffff));
        return;
    }
    const _this = this;
    this.new_proxy_cert_rollout_scheduled = true;
    etask(function*(){
        this.on('uncaught', e=>{
            logger.error('New certificate rollout failed: %s', e.message);
        });
        this.finally(()=>{
            _this.new_proxy_cert_rollout_scheduled = false;
        });
        logger.notice('Enabling new proxy certificate');
        yield _this.update_settings({new_proxy_cert: true});
    });
};
...
etask._fn(function*(_this, settings, opt={}){
    ...
    let reload_proxy_ca = false;
    let schedule_proxy_cert_rollout = false;
    ...
    const get_add_config_change_fn = (new_v, old_v)=>origin ?
        ...
        username, old_v) : Function.prototype;
    ...
    for (const field in settings)
    {
        ...
        case 'bw_th_webhook_url':
            ...
            break;
        case 'new_proxy_cert':
            if (_.isEqual(old_val, val))
                break;
            _this._defaults[field] = !!val;
            reload_proxy_ca = true;
            schedule_proxy_cert_rollout = !val;
            add_config_change('update_new_proxy_cert', 'defaults');
            break;
        }
    }
    ...
    if (reload_proxy_ca)
    {
        ...
        _this.cluster_mgr.broadcast('UPDATE_CA', {
            ca: _this.get_ssl_ca(),
            new_proxy_cert: _this._defaults.new_proxy_cert,
        });
    }
    if (schedule_proxy_cert_rollout)
...
etask._fn(function*mgr_apply_cloud_config(_this, config, opt){
    ...
    const proxy_cert_changed =
        old_defaults.new_proxy_cert != _this._defaults.new_proxy_cert;
    if (proxy_cert_changed)
        yield ssl.load_ca(_this);
