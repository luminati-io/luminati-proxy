...
E.prototype.get_port_ranges = function(server_conf){
    return server_conf?.cloud?.port_ranges?.[this._defaults.customer_id]
        || null;
};
...
E.prototype.complete_proxy_config = function(conf){
    ...
    c.zones = this.zones_mgr.to_zp_map();
    return c;
};
...
function*mgr_create_single_proxy(_this, conf){
    ...
    const proxy = new Proxy_port(conf, _this);
    proxy.on('tls_error', ()=>{
...
E.prototype.update_proxy_fields = function(proxy){
    ...
    proxy.zones = this.zones_mgr.to_zp_map();
...
const get_nearest_port = (proxies, ranges, port)=>{
    const ports = util_lib.get_ports(proxies);
    do
        port++;
    while (!ports.includes(String(port)) && ranges.meet(port));
    return port;
};
    ...
    if (port!==undefined)
    {
        ...
        else
        {
            ...
            const check_range = !old_proxy_port
                || _this.port_ranges.meet(old_proxy_port);
            if (in_use)
            ...
            else if (_this.argv.zagent && check_range)
            {
                const rest_ports_count = get_nearest_port(_this.proxies,
                    _this.port_ranges, +port)-port-1;
                // 1 is master port
                const max_multiply_value = rest_ports_count+1;
                if (!_this.port_ranges.meet(port))
                {
                    info.push({
                        msg: 'Invalid port. It must be a number in range '
                            +_this.port_ranges.orig+' in Cloud Proxy Manager.',
                        field: 'port',
                    });
                }
...
E.prototype.update_proxies = function(){
    this.proxies.forEach(p=>{
        if (this.logged_in)
        {
            ...
            p.zones = this.zones_mgr.to_zp_map();
            if (p.unblock)
                p.ssl = true;
        }
