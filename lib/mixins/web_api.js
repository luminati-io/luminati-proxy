...
const {get_source, get_username, format_json, convert_bytes,
    some_safe_equal, safe_equal} = util_lib;
...
E.prototype.lpm_token_auth_mw = function(req, res, next){
    ...
    if (bypass || !tokens.length || some_safe_equal(tokens, auth_lpm_token))
        return void next();
...
E.prototype.create_api_server = function(app){
    ...
    const tcp_server = net.createServer(socket=>{
        ...
        if (this.lb_ips?.includes(socket.remoteAddress))
            lb_transform_stream = util_lib.Lb_transform.init(socket);
...
E.prototype.global_banip_api = function(req, res){
    ...
    {
        const valid_ips = ips
            .filter(_ip=>_ip && (util_lib.is_ip(_ip) || util_lib.is_eip(_ip)));
        if (!valid_ips.length)
            return res.status(400).send('No IP provided');
        valid_ips.forEach(_ip=>this.banip(_ip, domain, ms, ports));
        return res.status(204).end();
    }
...
function*add_www_whitelist_ip_api(_this, req, res, next){
    ...
    try { ip = util_lib.normalize_whitelist_ip(ip); }
...
E.prototype.cloud_auth_api = function(req, res){
    ...
    if (!lpm_token || !safe_equal(lpm_token, req.body.lpm_token))
...
function*add_wip_api(_this, req, res, next){
    ...
    if (!token_auth || !safe_equal(token_auth, req.headers.authorization))
        return res.status(403).send('Token required');
    let ip;
    if (!(ip = req.body.ip))
        return res.status(400).send('You need to pass an IP to add\n');
    ...
    if (whitelist_ips.includes(ip))
        return res.send('OK');
    const new_ips = whitelist_ips.concat([ip]);
    if (_this._defaults.sync_config)
        yield _this.lpm_f.update_settings({whitelist_ips: new_ips});
    _this.set_whitelist_ips(new_ips);
    _this.add_config_change('add_whitelist_ip', 'defaults', ip,
        get_source(req), get_username(req));
    yield _this.config.save({skip_broadcast: 1});
    res.send('OK');
});
...
function*remove_wip_api(_this, req, res, next){
    ...
    const old_ips = new Set(_this._defaults.whitelist_ips||[]);
    if (!old_ips.has(ip))
        return res.status(404).send('IP is not whitelisted\n');
    old_ips.delete(ip);
    const new_ips = Array.from(old_ips);
