...
E.prototype.send_error = function(method, _url, res, err, err_origin){
    const message = `[${err_origin}] ${err.message}`;
    let status_code = err.code || err.status_code || 502;
    const status_message = err.status_message || 'Proxy Manager - Bad Gateway';
    if (typeof status_code !== 'number')
    {
        let override = typeof err.status_code === 'number'
            ? err.status_code : 502;
        this.logger.error('[send_error] Attempt to write %s status code "%s"'
            +', override to %s', typeof status_code, status_code, override);
        status_code = override;
    }
    this.logger.info('%s %s %s %s - %s', method, _url, status_code,
...
function*send_bypass_req(_this, task, req, res, head){
    ...
    else
    {
        proxy = lutil.native_request({
            uri: ctx.url,
            host: zurl.parse(ctx.url).hostname,
            method: req.method,
            path: ctx.req_url,
...
function*send_proxy_req_ssl(_this, task, req, res, head){
    ...
    try {
        ...
        let conn = _this.get_reused_conn(ctx), reused_conn = !!conn;
        if (conn)
            ctx.timeline.track('connect');
        else
        ...
        if (!conn || !conn.socket)
            return conn;
        const ctx_headers = ctx.format_headers(ctx.headers);
        const proxy_opt = {
        ...
        {
            let conn_socket = conn.socket.socket;
            if (!conn_socket || conn_socket.destroyed)
            {
                const socket_name = ctx.get_socket_name();
                _this.logger.warn('%s flex_tls socket destroyed before use, '
                    +'retrying with new socket: %s',
                    reused_conn ? 'reused' : 'new', socket_name);
                try { conn.socket.destroy(); } catch(e){}
                conn = yield _this.request_new_socket(task, req, res, head);
                if (!conn || !conn.socket)
                    return conn;
                conn_socket = conn.socket.socket;
                if (!conn_socket || conn_socket.destroyed)
                {
                    let sock_err = new Error('flex_tls_use_destroyed_socket: '
                        +`${socket_name}`);
                    _this.log_fn(sock_err, ctx, `flex_tls, conn.socket`);
                    throw sock_err;
                }
            }
            proxy_opt.lpm_socket = conn.socket;
        }
