...
E.prototype.check_user_auth = function({user, password}={}){
    return !!(user && password && this.lpm_users?.some(u=>
        user==u.email && util_lib.safe_equal(password, u.password)));
};

E.prototype.is_basic_user_auth = function({authorization}={}){
    const match = authorization?.match(/^Basic (.*)/);
    if (!match)
        return false;
    const value = Buffer.from(match[1], 'base64').toString();
    const separator_idx = value.indexOf(':');
    return separator_idx >= 0 && this.check_user_auth({
        user: value.slice(0, separator_idx),
        password: value.slice(separator_idx + 1),
    });
};

E.prototype.user_auth = function(req){
    if (req.user_auth!==undefined)
        return req.user_auth;
    if (this.is_basic_user_auth(req.headers))
        return req.user_auth = true;
    if (this.check_user_auth(req.query))
    {
        metrics.glob_inc('user_auth_qs');
        return req.user_auth = true;
    }
    return req.user_auth = false;
};
...
E.prototype.whitelist_auth = function(ip){
    ...
    return empty && !this.argv.zagent || this.www_wl_ips.contains(ip);
};
...
E.prototype.authenticate = function(req, allow_bypass){
    ...
    const is_cloud_auth = lpm_token &&
        util_lib.safe_equal(cookies.lpm_token, lpm_token);
    ...
    const reseller_user_api = ['/bw_limit_stats/', '/refresh_sessions/',
        '/generate_proxies/'];
    if (this.argv.zagent && this.is_reseller() && this.user_auth(req)
    ...
    const is_local_authenticated = this.user_auth(req)
        || !this.argv.local_login
        || passwd && util_lib.safe_equal(cookies['local-login'], passwd);
...
E.prototype.create_web_interface = etask._fn(function*(_this){
    ...
    app.use(mw.security.cookies(_this.argv.session_path));
    app.use(compression());
