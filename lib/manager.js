...
E.prototype.trace = etask._fn(function*mgr_trace(_this, req, res){
    const opt = assign(_.pick(req.body, qw`url zone country`));
    // XXX krzysztof: move building Tracer on lpm start
    const tracer = new Tracer(_this._defaults, _this.wss);
    const result = yield tracer.trace(opt.url, opt.zone, opt.country);
    res.json(result);
});
