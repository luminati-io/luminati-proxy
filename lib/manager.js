...
const {get_plan, get_perm, is_static_proxy, is_mobile,
...
E.prototype.init = function(argv){
    try {
        ...
        this._log.error('init: %s', zerr.e2s(e));
        throw e;
    }
};
...
E.prototype.get_consts_api = function(req, res){
    ...
    const zones = this.zones
        .map(z=>Object.assign({key: z.zone, value: z.zone, plans: []}, z));
    zones.unshift({key: `Default (${proxy.zone.def})`, value: ''});
    proxy.zone.values = zones;
    const notifs = this.lum_conf.lpm_notifs||[];
    const logins = this.lum_conf.logins||[];
...
E.prototype._complete_proxy_config = function(conf){
    const c = Object.assign({current_ip: this.opts.current_ip}, E.default,
        this._defaults, conf, _.omit(this.argv.overlay_opt, 'password'));
    const zone = c.zones && c.zones[c.zone];
    const plan = zone && get_plan(zone.plans);
...
function*mgr_create_proxy(_this, proxy){
    ...
    proxy = Object.assign({}, _.omit(_this._defaults, qw`zones password`),
...
E.prototype.multiply_port = etask._fn(
function*mgr_multiply_port(_this, port){
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    if (zone!==undefined)
    {
        ...
        {
            db_zone = _this.zones.filter(i=>i.zone==zone)[0];
            if (!db_zone)
            ...
            {
                db_zone_plan = get_plan(db_zone.plans);
...
E.prototype.batch_req_api = etask._fn(function*_batch_req(_this, req, res){
    try {
        ...
        let resps_count = 0;
        const e_this = this;
        const DEFAULT_TIMEOUT = 20*date.ms.SEC;
        const timeout = +req.body.timeout||DEFAULT_TIMEOUT;
        var _timout = setTimeout(()=>e_this.continue(), timeout);
        var all_resps = [];
        for (var i=0;i<requests.length;i++)
        {
            const opt = Object.assign({followRedirect: false, timeout},
                requests[i]);
            ...
            request(opt, (err, response, body)=>{
                let resp = {req: requests[index], i: index, body};
                if (err)
                    resp.error = err.message;
                if (response)
                    resp.status_code = response.statusCode;
                all_resps.push(resp);
                if (resps_count==requests.length-1)
                    e_this.continue();
                resps_count++;
            });
        }
...
E.prototype.authenticate = function(req, res, next){
    ...
    const _defaults = Object.assign({}, _.pick(E.default,
...
E.prototype.create_api_interface = function(){
    ...
    app.post('/batch_req/:port', this._api(this.batch_req_api));
    return app;
};
...
const zones_from_conf = config=>{
    ...
    return _.map(config._defaults.zones, (v, k)=>{
        return {zone: k, perm: get_perm(v), plans: v.plans,
...
function*mgr_check_user(_this){
    try {
        const conf = yield _this.get_lum_local_conf(null, null, true);
...
function*mgr_get_lum_local_conf(_this, customer, token, force_login){
    ...
    if (token)
        Object.assign(_this._defaults, {token});
    return _this.lum_conf;
});
...
function*mgr_fix_config(_this, new_conf){
    ...
    if (_this._defaults.zone==new_conf._defaults.zone)
        Object.assign(_this._defaults, new_conf._defaults);
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    this.on('uncaught', e=>_this._log.error('start %s', zerr.e2s(e)));
    try {
        perr.run();
        yield _this.get_ip();
