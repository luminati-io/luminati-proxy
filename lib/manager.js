...
const numeric_fields = qw`port session_init_timeout proxy_count pool_size
    max_requests request_timeout session_ducation throttle keep_alive`;
...
Manager.prototype.stop = etask._fn(function*stop(_this, force, cb){
    const done = ()=>{
        _this.emit('stop');
        if (cb)
            cb();
...
function*status(_this, req, res){
    ...
    else if (yield _this.check_credentials())
    {
        res.json({
            status: 'ok',
    ...
    else if (yield _this.check_firewall())
    {
        res.json({
            status: 'error',
    ...
    else if (yield _this.check_network())
    {
        res.json({
            status: 'error',
            description: 'The firewall is blocking access to the proxy.',
        });
    }
    else
    {
...
Manager.prototype.check_network = etask._fn(
function*check_network(_this){
    try {
...
Manager.prototype.check_firewall = etask._fn(
function*check_firewall(_this, cred){
    try {
        cred = _.assign({}, _this.argv, cred);
        yield _this.json(`https://${cred.proxy}:${cred.proxy_port}/`);
        return true;
    } catch(e){ ef(e); return false; }
});
...
function*creds_set(_this, req, res){
    let cred = req.body;
    if (/^lum-.*/.test(cred.customer))
        _.assign(cred, parse_username(cred.customer));
    let credentials = yield _this.check_credentials(cred);
    if (!credentials)
    {
        res.sendStatus(407);
    ...
    let init = !_this.argv.customer;
    if (cred.timeout)
    {
        cred.request_timeout = cred.timeout;
        delete cred.timeout;
    }
    _.assign(_this.argv, cred);
    _this.save_config();
    if (init)
        yield _this.init_proxies();
    res.json(_this.get_creds());
});
...
function*create_web_interface(_this){
    ...
    app.get(/^\/(settings|proxies|zones|tools)(\/[^\/]+)?$/,
        (req, res, next)=>{
        req.url = '/index.html';
        next('route');
    });
    app.use(express.static(path.join(__dirname, '../bin/pub')));
    app.use('/require.js', (req, res)=>{
        res.sendFile(
            path.join(__dirname, '../node_modules/requirejs/require.js'));
    });
    app.use('/angular.min.js', (req, res)=>{
        res.sendFile(
            path.join(__dirname, '../node_modules/angular/angular.min.js'));
    });
    app.use('/lodash.min.js', (req, res)=>{
        res.sendFile(
            path.join(__dirname, '../node_modules/lodash/lodash.min.js'));
    });
    app.use('/css.min.js', (req, res)=>{
        res.sendFile(
            path.join(__dirname, '../node_modules/require-css/css.min.js'));
    });
    app.use('/bootstrap', express.static(path.join(__dirname,
        '../node_modules/bootstrap/dist')));
    app.use('/bootstrap-datepicker', express.static(path.join(__dirname,
        '../node_modules/bootstrap-datepicker/dist')));
    app.use('/jquery', express.static(path.join(__dirname,
        '../node_modules/jquery/dist')));
    app.use('/angular-chartjs', express.static(path.join(__dirname,
        '../node_modules/angular-chart.js/dist')));
    app.use('/chartjs', express.static(path.join(__dirname,
        '../node_modules/chart.js/dist')));
    app.use('/moment', express.static(path.join(__dirname,
        '../node_modules/moment/min')));
    app.use('/notosans/css', express.static(path.join(__dirname,
        '../node_modules/notosans-fontface/css')));
    app.use('/notosans/fonts', express.static(path.join(__dirname,
    ...
    _this.io.on('connection', socket=>etask(function*(){
        ...
        notify('network', yield _this.check_network());
        notify('firewall', yield _this.check_firewall());
        notify('credentials', yield _this.check_credentials());
