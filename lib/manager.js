    ...
    opt)
{
    ...
    if (proxy.opt.proxy_type=='persist')
    {
        ...
        yield _this.config.save({skip_cloud_update: opt.skip_cloud_update});
    }
});
...
etask._fn(function*mgr_update_settings_api(_this, req, res){
    ...
    if (req.body.sync_config!==undefined)
    {
        delete _this._defaults.ask_sync_config;
        if (req.body.sync_config && !_this._defaults.sync_config)
            skip_cloud_update = 1;
        _this._defaults.sync_config = req.body.sync_config;
...
E.prototype.creds_user_api = etask._fn(function*mgr_creds(_this, req, res){
    ...
    const whitelist_ips = _this._defaults.whitelist_ips || [];
...
E.prototype.add_lpm_wip_api = function(req, res){
    const lpm_token = (this._defaults.lpm_token||'').split('|')[0];
    if (!lpm_token || lpm_token!=req.body.lpm_token)
        return res.status(403).send('Forbidden');
    const new_ips = [...new Set(this._defaults.www_whitelist_ips).add(req.ip)];
    this.set_www_whitelist_ips(new_ips);
    res.send('OK');
};
...
E.prototype.authenticate = function(req, res, next){
    ...
    // and stats
    const bypass = ['/version', '/add_wip', '/add_lpm_wip'].includes(req.url);
...
E.prototype.create_api = function(){
    ...
    app.get('/swagger', (req, res)=>res.json(swagger));
    ...
    app.post('/creds_user', this.creds_user_api.bind(this));
    app.post('/verify_two_step', this.verify_two_token_api.bind(this));
    app.post('/send_two_step_email', this.send_two_step_email_api.bind(this));
    app.get('/gen_token', this.gen_token_api.bind(this));
    app.get('/config', this.config_get_api.bind(this));
    app.post('/config', this.config_set_api.bind(this));
    ...
    app.post('/shutdown', this.shutdown_api.bind(this));
    app.post('/logout', this.logout_api.bind(this));
    app.post('/upgrade', this.upgrade_api.bind(this));
    app.post('/downgrade', this.downgrade_api.bind(this));
    app.post('/restart', this.restart_api.bind(this));
    ...
    app.post('/async_req/:port', this.async_req_api.bind(this));
    ...
    app.post('/add_lpm_wip', this.add_lpm_wip_api.bind(this));
...
E.prototype.create_server = function(app){
    ...
    let tcp_server = net.createServer(socket=>{
        ...
        socket.once('timeout', ()=>util_lib.ensure_socket_close(socket));
...
E.prototype.lpm_f_ws_login = etask._fn(function*(_this, opt, cb){
    ...
    if (!_this.ws_conn || !_this.ws_conn.connected)
        return;
    const auth_res = yield _this.ws_conn.ipc.auth(
...
etask._fn(function*mgr_lpm_f_ws_get_conf(_this, opt){
    ...
    if (!_this.ws_conn || !_this.ws_conn.connected || !_this.lpm_f_authorized)
        return;
    let resp = yield _this.ws_conn.ipc.get_conf({lpm_token});
    if (!opt.retried && resp && resp.err && resp.err=='not_authorized')
    {
...
etask._fn(function*mgr_apply_cloud_config(_this, config, opt){
    if (lpm_config.is_win || !_this.argv.zagent&&!_this._defaults.sync_config)
        return;
    opt=opt||{};
    const has_cust_field = !!_.get(config, '_defaults.customer');
    if (!opt.force && (!config.ts ||
...
E.prototype.init_lpm_f_ws = function(){
    if (!this.argv.cloud || !this.argv.config)
        return;
    ...
    this.ws_conn = new zws.Client(uri_ws, {
        ...
        ipc_client: {
            ...
            get_conf: {type: 'call', timeout: 10*date.ms.SEC},
        },
    }).on('connected', ()=>{
...
function*mgr_first_lpm_action(_this, action, ua, no_retry){
    ...
    try {
        ...
        yield util_lib.perr(`first_${action}`, {
            ...
            lpm_zagent: _this.argv.zagent ? os.hostname() : null,
            browser,
        });
