...
const PROXY_ONLY_BYPASS = qw`googleapis.com cdnjs.com gstatic.com google.co.*
    googleusercontent.com google.* luminati.io luminati-china.io
            ...
            etask(function*history_aggregator(){
                try {
                    ...
                    _this.db.main.stmt.history.run(row, function(err){
                        ...
                        _this.wss.broadcast(JSON.stringify(har_req));
                    });
                } catch(e){
...
E.prototype.enable_ssl = function(req, res){
    this.proxies.forEach(p=>{
        if (p.port!=22225)
            p.ssl = true;
    });
    Object.values(this.proxies_running).forEach(p=>{
        if (p.port!=22225)
            p.config.ssl = true;
    });
    this.save_config();
...
E.prototype.trace = etask._fn(function*mgr_trace(_this, req, res){
    ...
    const tracer = new Tracer(_this._defaults);
...
function*mgr_recent_stats(_this, req, res){
    ...
    res.json(Object.assign({ports}, stats, general[0]));
});
