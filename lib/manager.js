...
function*proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    let session = new_proxy_config.session;
    if (port!==undefined)
    {
    ...
    if (pool_size!==undefined)
    {
        ...
        if ((is_mr||is_sd||is_ka) && !check(pool_size) && !check(sticky_ip))
        {
            info.push({
                msg: 'max_requests, session_duration and keep_alive '
                +'will not take effect without specifying pool_size or '
                +'sticky_ip',
                lvl: 'warn',
            });
        }
    ...
    {
        info.push({
            msg: 'sticky_ip and session will take effect when pool_size is '
            +'specified',
            lvl: 'warn',
        });
    }
