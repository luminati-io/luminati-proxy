...
function*mgr_login_user(_this, google_token, username, password, customer){
    if (!_this.luminati_jar)
        _this.luminati_jar = {jar: _this.get_cookie_jar()};
    const response = yield _this.api_request({
        method: 'POST',
    ...
    _this.lpm_token = response.body;
    let config;
    try {
        config = yield _this.get_lum_local_conf(customer, google_token);
    } catch(e){
        if (!e.status)
            throw e;
