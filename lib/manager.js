...
E.prototype.get_super_proxy_ports = function(server_conf){
    ...
    return Object.entries(proxy_ports).reduce((ports, [port, cust_ids])=>{
...
function*get_collect_data_api(_this, req, res, next){
    ...
    if (Object.keys(errs).length && !return_data)
    {
        const [k, v] = Object.entries(errs)[0];
        return res.status(500).send(`${k}: ${v}`);
    }
    if (!return_data)
        return res.status(200).send('ok');
    return res.json({result: Object.assign({
