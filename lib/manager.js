...
function Manager(args, run_config){
    ...
    this.stats_aggregator = data=>{
        try {
            ...
            // XXX krzysztof: stat is to be removed
...
E.prototype.filtered_get = etask._fn(function*mgr_filtered_get(_this, req){
    ...
    let where = wheres ? 'WHERE ('+wheres+')' : '';
    if (req.query.port_from && req.query.port_to)
    {
        where = (where ? ' AND (' : 'WHERE ')+`port >= ${req.query.port_from}
        AND port <= ${req.query.port_to}`+(where ? ')' : '');
    }
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|login|howto|proxy_tester|logs|logs2|config)|(overview|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
