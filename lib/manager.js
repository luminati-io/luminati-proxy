...
Manager.prototype.stop = etask._fn(function*stop(_this, force, done){
    if (!done)
        done = !module.parent ? ()=>process.exit() : ()=>{};
    let servers = [];
...
function*creds_set(_this, req, res){
    let credentials = yield _this.check_credentials(req.body);
    if (!credentials)
    {
        res.sendStatus(407);
...
function*create_web_interface(_this){
    ...
    server.stop = force=>etask(function*(){
        return yield etask.nfn_apply(server, '.shutdown', []);
    });
