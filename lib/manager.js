...
E.prototype.update_proxy_fields = function(proxy){
    ...
    proxy.password = get_password(proxy, zone_name, this.zones) ||
        this.argv.password || this._defaults.password;
    proxy.gb_cost = get_gb_cost(zone_name, this.zones);
    ...
    conf.static = is_static_proxy(zone_name, this.zones);
    conf.mobile = is_mobile(zone_name, this.zones);
    conf.unblock = is_unblocker(zone_name, this.zones);
    return conf;
};
...
etask._fn(function*mgr_update_settings_api(_this, req, res, next){
    this.on('uncaught', next);
    if (_this.argv.zagent && req.body.request_stats!==undefined)
        return res.status(400).send('Not allowed to set request stats');
...
E.prototype.update_proxies = function(){
    this.proxies.forEach(p=>{
        if (this.logged_in)
        {
            ...
            p.gb_cost = get_gb_cost(zone.zone, this.zones);
            p.mobile = is_mobile(zone.zone, this.zones);
            p.unblock = is_unblocker(zone.zone, this.zones);
        }
