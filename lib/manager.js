...
E.prototype.save_config = function(){
    ...
    let proxies = this.proxies
        ...
        .map(p=>_.omit(p, qw`stats proxy_type zones`))
...
function*get_lum_local_conf(_this, customer, token, force_login){
        ...
        _this.lum_conf._defaults.password != _this._defaults.password))
    {
        ...
        {
            let dropin_proxy;
            if (dropin_proxy = _this.proxies.find(p=>
                p.port==Luminati.dropin.port))
            {
                assign(dropin_proxy, _.pick(_this._defaults,
                    qw`zone password`));
                if (_this.proxies_running[dropin_proxy.port])
                    yield _this.proxy_update(dropin_proxy, dropin_proxy);
            }
        }
    }
    if (token)
