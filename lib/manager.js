...
E.prototype.create_proxy = etask._fn(
    ...
    // XXX viktor: conf is a Object.assign({}, proxy)
    // with some additional fields
    // so proxy is a subset of proxies[x]
...
function*mgr_refresh_ip(_this, ip, port){
    ...
    {
        // XXX viktor: replace with proxy.opt
...
E.prototype.api_request = etask._fn(function*mgr_api_request(_this, opt){
    ...
    {
        const msg = `API call to ${opt.url} FAILED with status `
            +res.statusCode;
        throw new Error(msg);
    }
    return res;
...
E.prototype.init_lpm_f_ws = function(){
    ...
    this.ws_agent_conn = new zws.Client(uri_ws, {
        label: 'lpm_f',
    ...
    if (login_failed)
    {
        ...
        if (username && password)
        {
            yield etask.nfn_apply(request,
                [{url: _this._defaults.api, jar: jar}]);
            ...
            if (response.statusCode!=200)
            {
                ...
                {
                    let api_url = _this._defaults.www_api;
                    return {error: {
