...
function Manager(args, run_config){
    ...
    let conf = this._get_proxy_configs(args);
...
E.prototype._get_proxy_configs = function(args){
    ...
    this.argv_explicit = _.pick(this.argv,
        proxy_params.filter(p=>args.includes(`--${p}`)));
    if (args.includes('-p'))
        this.argv_explicit.port = this.argv.port;
    this.argv_overlay = _.omit(this.argv_explicit, qw`socks port`);
    ...
    if (this.argv_explicit.port||this.argv_explicit.socks||!proxies.length)
        proxies.push(assign({proxy_type: 'config'}, this.argv_explicit));
    ...
    assign(_defaults, _.pick(this.argv_overlay, default_fields));
...
E.prototype._complete_proxy_config = function(conf){
    return assign({}, E.default, this._defaults, conf, this.argv_overlay);
};
