...
E.prototype.lpm_f_ws_login = etask._fn(function*(_this, opt, cb){
    ...
    if (opt.apply_cloud_config)
        _this.apply_cloud_config(auth_res.config);
    if (cb)
        yield cb();
});
...
E.prototype.apply_cloud_config = function(config){
    if (!config.ts || this.config_ts && date(config.ts)<=date(this.config_ts))
        return;
    this.config.save_local_backup();
    this.config.set_string(this.config.prepare_cloud_config(config),
        {skip_cloud_update: 1});
...
E.prototype.init_lpm_f_ws = function(){
    ...
    this.ws_conn = new zws.Client(uri_ws, {
        ...
        if (d.msg!='new_conf')
            return;
        this.apply_cloud_config(d.new_conf);
    }).on('disconnected', ()=>{
