...
const credential_params = qw`customer zone password`;
const proxy_params = _(swagger.definitions.proxy.properties)
    .pickBy(k=>!credential_params.includes(k)).keys().value();
...
function Manager(args, run_config){
    ...
    let {_defaults, proxies} = this._get_proxy_configs(args);
...
E.prototype._config_from_file = function(fname, proxy_type){
    let conf = load_json(fname, false, {});
    let defaults = conf._defaults||{};
    let proxies = [].concat(conf.proxies||conf);
    proxies = proxies.map(c=>assign({proxy_type}, defaults, c));
    return {_defaults: defaults, proxies};
};

E.prototype._get_proxy_configs = function(args){
    let _defaults = assign({}, _.pick(E.default, _.keys(proxy_fields)));
    let proxies = [];
    // this.argv includes default values; we want only args explicitly given
    let cli_argv = _.pick(this.argv,
        proxy_params.filter(p=>args.includes(`--${p}`)));
    if (args.includes('-p'))
        cli_argv.port = this.argv.port;
    let cli_over = _.omit(cli_argv, qw`socks port`);
    if (this.mgr_opts.config)
    {
        let conf = this._config_from_file(this.mgr_opts.config, 'persist');
        assign(_defaults, _.pick(conf._defaults, _.keys(proxy_fields)));
        proxies = proxies.concat(conf.proxies);
    }
    proxies = proxies.concat([].concat.apply([], this.argv._
        .map(fname=>this._config_from_file(fname, 'config').proxies)));
    if (this.mgr_opts.dropin)
        proxies.push(Luminati.dropin);
    if (cli_argv.port || cli_argv.socks || !proxies.length)
        proxies.push(assign({proxy_type: 'persist'}, cli_argv));
    ...
    proxies = proxies.map(c=>assign({}, E.default, c, cli_over));
    assign(_defaults, cli_over);
    proxies.forEach(c=>this._log.debug('Config', c));
    this._log.debug('_defaults', _defaults);
...
E.prototype._api = function(f, roles){
    ...
    return (req, res, next)=>etask(function*_api(){
        ...
        res.json = o=>{
            _this._log.debug('API json resp', req.method, req.originalUrl, o);
            json(o);
        };
...
E.prototype.save_config = function(){
    ...
    let proxies = this.proxies
        ...
        .map(p=>_.omit(p, _.keys(this._defaults)))
...
E.prototype.get_consts = etask._fn(function*get_consts(_this, req, res){
    ...
    let zones = (yield _this.get_zones()).map(zone=>{
        return {
            ...
            value: zone.zone,
            perm: zone.perm,
            plans: zone.plans ? JSON.parse(zone.plans) : [],
        };
    });
...
function*create_proxy(_this, proxy){
    ...
    if (!_this.logged_in && !conf.allow_proxy_auth)
    {
...
function*check_credentials(_this, cred){
    ...
    try {
        cred = _.pick(assign({}, _this._defaults, cred), credential_params);
...
function*creds_user_set(_this, req, res){
    ...
    {
        let plan = (zones[zone].plans||[]).pop();
        let perm = plan&&plan.perm || zones[zone].perm;
        yield _this.sql(query, zone, perm, zones[zone].ips,
...
E.prototype.allocated_ips_get = etask._fn(function*(_this, req, res){
    const r = yield _this.json({
        ...
        headers: {'x-hola-auth': `lum-customer-${_this.argv.customer}`
        +`-zone-${req.query.zone}-key-${req.query.key||_this.argv.password}`},
    });
    res.send(r.body);
});
