...
E.prototype.sql = etask._fn(function*sql(_this, db){
    try {
        ...
        _this._log.debug(`SQL (${db._name}): `, args);
        let res = yield etask.nfn_apply(db, '.all', args);
...
const open_db = path=>etask(function*(){
    let db;
    yield etask.nfn_apply((path, cb)=>db = new sqlite3.Database(path, cb),
        null, [path]);
    db._name = path;
    return db;
});
...
function*prepare_database(_this){
    ...
    let city_sql = path.resolve(__dirname, '../db/city.sql');
    if (_this.argv.www&&_this.mode!='guest')
    {
        _this.db.cities = yield open_db(':memory:');
        yield etask.nfn_apply(_this.db.cities, '.exec',
            [fs.readFileSync(city_sql, 'utf8')]);
    }
    let now = Date.now();
...
E.prototype.get_countries = etask._fn(function*get_countries(_this){
    if (!_this.db.cities)
        return [];
    let rows = yield _this.sql(_this.db.cities,
        'SELECT * FROM countries ORDER BY name');
    return (rows||[]).map(x=>({
        key: x.name,
        value: x.country_id.toLowerCase(),
    }));
});
...
E.prototype.get_consts = etask._fn(function*get_consts(_this, req, res){
    ...
    let countries = yield _this.get_countries();
...
E.prototype.config_check = function(config){
    ...
    if (config._defaults.www)
        check(config._defaults.www, 'UI/API');
    config.proxies.forEach((p, i)=>{
        const id = `Proxy #${i+1}`;
        const opt = assign({}, config._defaults, p);
        check(opt.port, id);
        if (opt.socks)
            check(opt.socks, id+' SOCKS');
        let multiply = p.multiply||1;
        for (let d = 1; d<multiply; ++d)
        {
            check(opt.port+d, id+' Duplicate');
            if (opt.socks)
                check(opt.socks+d, id+' Duplicate SOCKS');
        }
    });
...
E.prototype.get_regions = etask._fn(function*get_regions(_this, req, res){
    let c_id = req.params.country_id;
    if (!c_id || !_this.db.cities)
        return res.json([]);
    let regions = yield _this.sql(_this.db.cities, `SELECT * FROM regions
        WHERE (country_id = ?) ORDER BY region_id ASC`, c_id);
    regions = (regions||[]).map(x=>({
        key: `${x.name} (${x.region_id.toUpperCase()})`,
        value: x.region_id
    }));
...
E.prototype.get_cities = etask._fn(function*get_cities(_this, req, res){
    ...
    if (!cond.$country_id || !_this.db.cities)
        return res.json([]);
    let wheres = ['country_id = $country_id'];
    if (cond.$region_id)
        wheres.push('region_id = $region_id');
    let query = `SELECT * FROM cities WHERE ${wheres.join(' AND ')}
        ORDER BY name ASC`;
    let cities = yield _this.sql(_this.db.cities, query, cond);
    cities = (cities||[]).map(x=>({
        key: x.name,
        value: x.name,
        region: x.region_id,
    }));
    cities.unshift({key: 'Any', value: '*'});
    cities.unshift({key: 'Default ('+(_this.opts.city||'Any')+')', value: ''});
    res.json(cities);
});
