...
function*mgr_retry_failed_first_actions(_this){
    if (!_this.first_actions.pending.length)
        return;
    let data = _this.first_actions.pending.shift();
    const first_action_retried = data.action;
    do {
        yield _this.first_lpm_action(data.action, data.ua, true);
        data = _this.first_actions.pending.shift();
    } while (data && data.action != first_action_retried);
});
