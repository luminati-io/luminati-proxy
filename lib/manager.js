...
function Manager(args, run_config){
    ...
    this.stats_aggregator = data=>{
        if (data.context=='SESSION KEEP ALIVE'||data.context=='STATUS CHECK')
            return;
    ...
    this.handle_usage = etask._fn(function*handle_usage(_this, data){
...
E.prototype.stop_servers = etask._fn(function*stop_servers(_this, force, www){
    ...
    let servers = [];
    const stop_server = server=>servers.push(etask(function*stop_server(){
        try {
...
const open_db = _path=>etask(function*open_db(){
    let db;
...
E.prototype.update_notifs = etask._fn(function*update_notif(_this, req, res){
...
E.prototype.report_bug = etask._fn(function*report_bug(_this, req, res){
...
function*proxy_banip_api(_this, req, res){
    let port = req.params.port;
...
function*proxy_unbanip_api(_this, req, res){
    let port = req.params.port;
    let proxy = _this.proxies_running[port];
    if (!proxy)
        throw `No proxy at port ${port}`;
...
E.prototype.test_ports = etask._fn(function*test_ports(_this, req, res){
    if (!req.query.ports)
    ...
    let _reqs = [];
    let tasks = ports.map(p=>etask(function*test_port(){
        _reqs.push(request({
...
function*get_all_locations(_this, req, res){
    let data = yield cities.all_locations();
    res.json(data);
});

E.prototype.get_regions = etask._fn(function*get_regions(_this, req, res){
...
E.prototype.get_cities = etask._fn(function*get_cities(_this, req, res){
...
E.prototype._last_version = etask._fn(function*_last_version(_this){
...
E.prototype._upgrade = etask._fn(function*_upgrade(_this, cb){
    yield _this.stop_dbs();
...
E.prototype.start_auto_update = function(){
    ...
    setTimeout(cb = etask._fn(function*start_auto_update(_this){
...
E.prototype.start = etask._fn(function*start(_this){
    try {
        ...
        if (_this.argv.www)
        {
            ...
            console.log('===================================================');
            console.log('=                                                 =');
            console.log('=  Local proxy manager client is running...       =');
            console.log('=                                                 =');
            console.log('=             Open admin browser:                 =');
            console.log('=                                                 =');
            console.log('=          ', _this.www_server.url);
            console.log('=                                                 =');
            console.log('=                                                 =');
            console.log('=  Do not close the process while using the       =');
...
class Ulimit_manager {
    constructor(mgr, opt){
        ...
        this.sp = etask('Ulimit_manager', function*(){ yield this.wait(); });
    ...
    check_fd(){
        const _this = this;
        return etask(function*check_fd(){
            try {
