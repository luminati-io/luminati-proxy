...
E.prototype.async_req_api = function(req, res){
    ...
    const make_callback = (url, c_method, body, status_code, metadata,
        is_err)=>
    {
        ...
        request(opt, (err, response, body)=>{
            if (err)
                this._log.error('async_req_api (callback): %s', err.message);
        });
    };
    request(opt, (err, response, body)=>{
        ...
        if (err)
        {
            this._log.error('async_req_api (proxy req): %s', err.message);
...
E.prototype.authenticate = function(req, res, next){
    ...
    const is_whitelisted = whitelist_blocks.some(wb=>{
        try {
...
function*mgr_get_lum_local_conf(_this, customer, token, force_login){
    ...
    const hash = _this.lum_conf && _this.lum_conf.hash;
