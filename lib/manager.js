...
function*mgr_create_single_proxy(_this, conf){
    ...
    proxy.on('add_static_ip', data=>etask(function*(){
        ...
        _this.set_config_changes('add_static_ip',
            {[data.port]: data.ip});
...
E.prototype.set_config_changes = function(key, data){
    const current_data = zutil.get(this.config_changes, key);
    let new_data;
    if (key.startsWith('add_'))
        new_data = [...new Set(current_data||[]).add(data)];
    else
        new_data = Object.assign({}, current_data, data);
    zutil.set(this.config_changes, key, new_data);
};
...
function*mgr_creds(_this, req, res, next){
    ...
    if (cloud_conf)
    {
        ...
        yield _this.lpm_f.update_stats();
    }
    _this.update_lpm_users(yield _this.lpm_users_get());
...
function*mgr_refresh_ip(_this, ip, vip, port){
    ...
    {
        const proxy_conf = _this.proxies.find(p=>p.port==port);
        const ips = proxy_port.opt.ips.map(_ip=>ips_map[_ip]);
        proxy_conf.ips = ips;
        proxy_port.update_config({ips});
        _this.set_config_changes('proxies.'+port, {ips});
        yield _this.config.save();
    }
...
function*add_wip_api(_this, req, res, next){
    ...
    _this.set_config_changes('add_whitelist_ip', ip);
    yield _this.config.save();
    res.send('OK');
});
