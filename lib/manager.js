...
const def_2_desc = def=>_.fromPairs(_.toPairs(def.properties)
    .map(p=>[p[0], p[1].description]));

const proxy_fields = assign({},def_2_desc(swagger.definitions.proxy),
    def_2_desc(swagger.definitions.manager), {
...
function Manager(args, run_config){
    ...
    .alias({'?': 'help', h: 'help', p: 'port', d: 'daemon', v: 'version'})
...
Manager.prototype.proxy_port_check = etask._fn(
function*proxy_port_check(_this, port, old_port){
    console.log('Check port', port, old_port); // XXX lee ===
    if (port == old_port)
        return;
    if (port == _this.argv.www)
        return 'in use by the UI/API';
    if (_this.proxies_running[port])
        return 'in use by another proxy';
    if (_.values(_this.proxies_running).map(p=>p.opt.socks).includes(port))
        return 'in use by another proxy SOCKS port';
    if (_this.argv.socks && _this.argv.socks.map(s=>s.split(':'[0])
        .includes(port)))
    {
        return 'in use by SOCKS interface';
    }
...
function*proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    if (port!==undefined)
    {
        let in_use = yield _this.proxy_port_check(port, old_proxy_port);
        if (in_use)
        {
            info.push({msg: 'port already '+in_use, lvl: 'err',
                field: 'port'});
        }
    }
    let invalid_zone;
    ...
    {
        let in_use = yield _this.proxy_port_check(socks,
            old_proxy && old_proxy.opt.socks);
        if (in_use)
        {
            info.push({msg: 'socks port already '+in_use, lvl: 'err',
                field: 'socks'});
        }
        info.push({
...
Manager.prototype.history_get = etask._fn(
function*history_get(_this, req, res){
...
Manager.prototype.history_har_get = etask._fn(
...
Manager.prototype.history_csv_get = etask._fn(
...
Manager.prototype.filtered_get = etask._fn(function*filtered_get(_this, req){
...
Manager.prototype.init_proxies = etask._fn(function*init_proxies(_this){
    ...
    if (_this.argv.dropin)
    {
        proxies.push(_this.create_proxy({port: default_port, sticky_ip: true,
            allow_proxy_auth: true, socks: false, pool_size: 0}));
    }
