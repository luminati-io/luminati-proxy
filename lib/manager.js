...
E.prototype.add_config_change = function(key, area, payload, source, username){
    this.post_event(key, source, username, assign(payload||{}, {area}));
...
function*pmgr_domain_update_api(_this, req, res, next){
    ...
    _this.add_config_change('pmgr_domain_update_api', 'defaults',
        req.body.domain, get_source(req), get_username(req));
    yield _this.config.save();
    res.send('OK');
});
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        if (!_this.argv.zagent)
            _this.post_event('Start Proxy Manager', null, _this.last_username);
