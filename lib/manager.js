...
E.prototype.complete_proxy_config = function(conf){
    ...
    c.hosts = this.hosts;
    c.keys = keys;
    c.extra_ssl_ips = [...new Set([...c.extra_ssl_ips||[],
...
function*mgr_open_browser_api(_this, req, res, next){
    ...
    try {
        const proxy = _this.proxy_ports[port];
        let [timezone, country] = qw`timezone country`.map(k=>zutil
            .get(proxy||{}, `opt.${k}`));
        if (timezone=='auto')
            timezone = country && zcountry.code2timezone(`${country}`);
        yield puppeteer.open_page(_this._defaults.test_url, port, {timezone});
    } catch(e){
...
E.prototype.lpm_f_ws_login = etask._fn(function*(_this, opt, cb){
    this.on('uncaught', e=>{
        logger.error('lpm_f_login %s', zerr.e2s(e));
        util_lib.perr('unhandled_lpm_f_login', {error: zerr.e2s(e)});
    });
    opt = opt||{};
...
E.prototype.run_cluster = function(){
    ...
    cluster.on('exit', (worker, code, signal)=>{
        if (worker.exitedAfterDisconnect)
            return;
        this.perr('unhandled_worker_die', {error: ''+code+': '+signal});
...
E.prototype.set_logger_level = function(level, from_argv){
    ...
    Object.values(cluster.workers).forEach(w=>{
        try {
            ...
            this.perr('unhandled_ipc_broken', {
                error: zerr.e2s(e),
            });
        }
    });
