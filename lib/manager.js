...
function Manager(argv, run_config){
    ...
    let conf = _get_proxy_configs(this);
...
function _get_proxy_configs(_this){
...
E.prototype.get_consts = etask._fn(function*mgr_get_consts(_this, req, res){
    let proxy = _.mapValues(lpm_config.proxy_fields, desc=>({desc}));
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|login|howto|tracer|whitelist_ips|proxy_tester|logs|dock_logs|config|settings)|(overview|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
