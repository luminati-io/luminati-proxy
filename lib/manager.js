...
E.prototype.create_proxy = etask._fn(function*mgr_create_proxy(_this, proxy){
    if (proxy.conflict)
    {
...
function*mgr_proxy_create_api(_this, req, res){
    ...
    _this.first_lpm_action('proxy_port', req.get('user-agent'));
});
...
function*mgr_first_lpm_action(_this, action, ua){
    const browser = user_agent.guess_browser(ua).browser;
    try {
        yield _this.api_request({
            method: 'POST',
            url: `${_this._defaults.api}/users/first_lpm_${action}`,
            form: {
        ...
        _this.perr(`First LPM action failed: ${action}`, e);
    }
});
