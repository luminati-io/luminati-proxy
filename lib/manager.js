...
const {randomInt: random_int} = require('crypto');
...
const {assign, keys, values} = Object, {TOKEN_AUTH} = consts;
const {safe_regexp, get_lpm_tasks, headers_to_a, count_fd, Port_ranges,
    count_tcp, is_any_ip, RPS, Wl_ips} = util_lib;
...
const mgr_on_server_conf = function(server_conf){
    if (_.isEqual(this.server_conf, server_conf))
        return;
    logger.system('Updated server configuration');
    let zone_auth_wl_diff = [];
    let super_proxy_ports_diff = [];
    let port_ranges_diff = !this.server_conf;
    let new_port_ranges = this.get_port_ranges(server_conf);
    ...
    if (this.server_conf)
    {
        ...
        av_server_url_diff = this.get_av_server_url(this.server_conf)!=
            new_av_server_url;
        port_ranges_diff = this.get_port_ranges(this.server_conf)!=
            new_port_ranges;
    }
    this.server_conf = server_conf;
    let port = this.check_proxy_port(server_conf.cloud);
    if (port_ranges_diff && this.argv.zagent)
        this.update_port_ranges(new_port_ranges);
...
class Manager extends events.EventEmitter {
    constructor(argv, run_config={}){
        ...
        try {
            ...
            this.conn = {};
            this.config_changes = [];
            this.wss = this.get_empty_wss();
            this.is_upgraded = run_config.is_upgraded;
            this.backup_exist = run_config.backup_exist;
            this.conflict_shown = false;
            this.lpm_conn = new Lpm_conn(this);
            this.zagent_server = new Zagent_api(this);
            this.lpm_f = new Lpm_f(this);
            this.rps = new RPS();
            this.server_conf = {};
            let pranges = this.argv.zagent ? consts.CLOUD_PORT_RANGES : [];
            this.port_ranges = new Port_ranges(pranges);
...
E.prototype.make_www_wl_ips = function(){
    this.www_wl_ips = new Wl_ips([
...
E.prototype.get_empty_wss = function(){
    return {
        close: ()=>null,
        broadcast_json: data=>{
            if (!this.argv.www)
...
E.prototype.update_port_ranges = function(port_ranges){
    try {
        this.port_ranges = new Port_ranges(port_ranges);
    } catch(e){
            ...
            return {
                ...
                details: {
                    ...
                    timeline: timeline.map(t=>{
                        const fbc = t.first_byte||t.connect||start;
                        return {
                            // first_byte: may be undefined if no body
                            ...
                            ...t.end&&{receive: t.end-fbc},
                            ...t.redir&&{redir: 1},
                            end: (t.end||t.redir)-start,
                            port: t.port,
                            session: t.session,
                        };
                    }),
...
E.prototype.check_any_whitelisted_ips = function(){
    const get_not_whitelisted_payload = ()=>{
        if ((this._defaults.whitelist_ips||[]).some(is_any_ip) ||
            (this._defaults.www_whitelist_ips||[]).some(is_any_ip))
        {
            return {type: 'defaults'};
        }
        const proxy_port = this.proxies.find(p=>
            (p.whitelist_ips||[]).some(is_any_ip));
...
E.prototype.test_port = etask._fn(function*lum_test(_this, proxy, headers){
    ...
    try {
        ...
        const x_lpm_fake_hdrs = ['x-lpm-fake-status', 'x-lpm-fake-peer-ip',
            'x-lpm-fake-headers', 'x-lpm-fake-data'];
        const x_lpm_fake = headers['x-lpm-fake'];
        const r = yield util_lib.request({
            ...
            headers: {
                ...
                'x-lpm-fake': x_lpm_fake,
                ..._.pick(headers, x_lpm_fake_hdrs),
            },
        }, !x_lpm_fake);
        success = r.statusCode==200;
        error = r.headers['x-brd-error'] || r.headers['x-lpm-error'] ||
...
E.prototype.test_logs_remote = etask._fn(function*(_this, req, res, next){
    ...
    let type = req.body.type, test_res;
    ...
    if (typeof logger.remote_transports[type].test_fn == 'function')
    {
        test_res = yield logger.remote_transports[type].test_fn(opt);
        return void res.send({success: !test_res, error: test_res});
    }
    ...
    transport.on('logged', this.continue_fn());
    ...
    test_res = yield this.wait(10*date.ms.SEC);
...
E.prototype.filtered_get = function(req){
    ...
    const url_re = safe_regexp(search);
    const un_re = search && safe_regexp('session-[^-]*'+search);
    if (url_re || un_re)
    {
        query.$or = [
            ...url_re && [{url: {'$regex': url_re}}] || [],
            ...un_re && [{username: {'$regex': un_re}}] || [],
        ];
    }
    let status_code;
...
E.prototype.get_params = function(){
    ...
    for (let k in this.argv)
    {
        ...
        if (['$0', 'h', 'help', 'version', 'p', '?', 'v', '_', 'native_args',
            ...
            'token_auth', 'password', 'lpm_token', 'customer', 'account_id',
            'customer_id', 'zone'].includes(k))
        {
            continue;
        }
...
E.prototype.gen_token = function(){
    let ret = '';
    for (let i=0, n=TOKEN_AUTH.CHARSET.length; i<TOKEN_AUTH.LEN; i++)
        ret += TOKEN_AUTH.CHARSET.charAt(random_int(n));
    return ret;
};
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        if (_this.argv.zagent)
        {
            ...
            yield _this.lpm_conn.bootstrap();
            yield _this.zagent_server.bootstrap();
        }
        yield _this.resolve_proxies();
        ...
        _this.schedule_new_proxy_cert_rollout();
        _this.update_lpm_users(yield _this.lpm_users_get());
...
E.prototype.run_stats_reporting = etask._fn(function*(_this){
    ...
    logger.notice('Running stats reporting');
    while (1)
    {
        ...
        if (i%5==0)
        {
            let p = _this.get_mgr_proxy_port();
            stats.tcp_established = yield count_tcp('ESTABLISHED', p);
            stats.tcp_time_wait = yield count_tcp('TIME_WAIT', p);
            i = 0;
        }
        ...
        if (usage>=consts.HIGH_CPU_THRESHOLD)
        {
            _this.timeouts.set_timeout(()=>etask(function*(){
                ...
                if (_usage>=consts.HIGH_CPU_THRESHOLD)
                {
                    ...
                    const tasks = yield get_lpm_tasks({all_processes: true,
                        current_pid: true});
...
E.prototype.should_sanitize = function(){
    return this.argv.sanitize_rules || this.argv.zagent;
};

E.prototype.should_sanitize_rules = function(rules=[]){
    return this.should_sanitize() &&
        Array.isArray(rules) && rules.length > 0;
};

E.prototype.sanitize_rules = function(conf={}, opt={}){
    let errors = rules_util.sanitize(conf.rules)||[];
    if (!errors.length)
        return [conf.rules, errors];
    let rules = opt.clone ? structuredClone(conf.rules) : conf.rules;
    errors.forEach(([i])=>{
        const rule = rules[i];
        if (!rule)
            return;
        rule.active = false;
        rule.type = 'before_send';
        rule.trigger_code = rules_util.NOOP_TRIGGER_CODE;
    });
    return [rules, errors];
};
