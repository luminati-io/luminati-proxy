...
function*mgr_update_notif(_this, req, res){
    ...
    const xsrf = (jar.getCookies(lum_url).find(e=>e.key=='XSRF-TOKEN')||{})
    .value;
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    {
        info.push({
            msg: 'Logs without SSL Log will not record '
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|tools|faq|setup_guide|howto|proxy_tester|stats|config|overview)|(zones|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
