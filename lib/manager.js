...
function*status(_this, req, res){
    ...
    if (yield _this.check_firewall({proxy}))
    {
...
Manager.prototype.config_set = etask._fn(
function*config_set(_this, req, res){
    file.write_e(_this.argv.config, req.body.config);
    res.json({result: 'ok'});
...
function*check_firewall(_this, cred){
    _this._log('DEBUG', 'Testing firewall:', cred);
    try {
        cred = _.assign({}, _this.argv, cred);
        let proxy = yield _this.proxy_ip(cred.proxy);
        let res = yield _this.json(`http://${proxy}:${cred.proxy_port}/ping`);
...
function*check_credentials(_this, cred){
    try {
        ...
        let res = yield _this.json({
            url: `https://${proxy}:${cred.proxy_port}/`,
            headers: {
                'host': 'zproxy.luminati.io',
                'x-hola-auth': `lum-customer-${cred.customer}`
