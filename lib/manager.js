...
E.prototype.update_bw_limits = function(_this, new_proxy, old_proxy={}){
    new_proxy.bw_limit = Object.assign({},
        zutil.pick(old_proxy.bw_limit, 'days', 'bytes', 'start'),
        zutil.pick(new_proxy.bw_limit, 'days', 'bytes', 'start', 'expires'));
...
etask._fn(function*mgr_apply_bw_limits(_this, limits){
    ...
    for (let limit of limits)
    {
        ...
        if (!proxy_port || !proxy)
            continue;
        logger.notice('Port %s set bw limit expires to %s', port,
            expires||'null');
        update_conf = true;
        proxy.bw_limit = Object.assign({}, proxy.bw_limit, {expires, ts});
        if (!expires)
            delete proxy.bw_limit.expires;
        proxy_port.update_config(proxy);
    }
    if (update_conf)
