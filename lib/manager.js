...
const PROXY_INTERNAL_BYPASS = ['google.com', 'luminati.io'];
...
function Manager(args, run_config){
    ...
    this.global_handlers.push({
        ...
        handler: err=>{
            ...
            this.stop('Uncaughout exception', true);
        },
    });
...
Manager.prototype.get_consts = function(req, res){
    ...
    countries_list.unshift({key: 'Any', value: '*'});
    countries_list.unshift({key: 'Default ('+(this.opts.country||'Any')+')',
        value: ''});
    _.merge(proxy, {
...
Manager.prototype.get_cities = function(req, res){
    ...
    let _this = this;
    if (!c_id || !r_id || !this.cities_db)
        return res.json([]);
    let query = 'SELECT * FROM cities WHERE (country_id = ? AND region_id = ?)'
        +' ORDER BY name ASC';
...
function*check_user(_this, username, password, customer){
    ...
    let email_check = yield etask.nfn_apply(request, [{
        url: `${url}/users/check_email?email=`+encodeURIComponent(username),
        json: true,
    }]);
    if (email_check.statusCode!=200 || !email_check.body.used)
    {
        return {error: {
    ...
    if (!customer)
    {
        if (c.customers.length>1)
            return {customers: c.customers.sort()};
        customer = c.customers[0];
    }
    let compliance = yield etask.nfn_apply(request, [{
        url: `${url}/users/get_settings?c=${customer}`,
        jar: jar,
        json: true,
    }]);
    if (compliance.statusCode!=200)
        return {error: {message: 'Cannot access compliance info.'}};
    let comp = compliance.body;
    if (!(comp.submit||comp.settings&&(comp.settings.brand_protect||
        comp.settings.ad_verification||comp.settings.self_test||
        comp.settings.candidate_sourcing||
        comp.settings.other&&comp.settings.other_text)))
    {
        return {error: {message: 'You have not signed the compliance yet.'}};
    }
    return {defaults: c._defaults};
});
