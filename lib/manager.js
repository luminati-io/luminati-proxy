...
function*proxy_update_api(_this, req, res){
    ...
    let new_proxy = req.body.proxy;
    if (new_proxy.rules)
    {
        if (!_this._defaults.rules || !_.isEqual(_this.rules, new_proxy.rules))
        {
            _this._defaults.rules = true;
            _this.rules = new_proxy.rules;
            setImmediate(()=>{
                _this.save_config();
                _this.emit('config_changed');
            });
        }
        delete new_proxy.rules;
    }
    try {
