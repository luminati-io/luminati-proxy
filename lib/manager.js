...
E.prototype.check_proxy_port = function(conf={}){
    ...
    let conf_def = lpm_config.manager_default.proxy_port;
    if (!conf)
        return def;
...
function*mgr_update_ips(_this, req, res){
    ...
    yield _this.proxy_update(proxy, {ips, vips},
        {source: get_source(req.headers), username: get_username(req)});
    res.send('ok');
});
...
E.prototype.get_super_proxy_ports = function(server_conf){
    ...
    return Object.entries(proxy_ports).reduce((ports, [port, account_ids])=>{
        if (account_ids.includes(this._defaults.account_id))
            ports.push(port);
        return ports;
    }, []);
};
...
const get_ports = p=>Array.isArray(p) ? p.map(x=>x.port) : Object.keys(p);
...
function*mgr_proxy_create_api(_this, req, res, next){
    ...
    _this.add_config_change('create_proxy_port', port, req.body.proxy,
        get_source(req.headers), get_username(req));
...
function*mgr_proxy_update(_this, old_proxy, new_proxy, opt={}){
    ...
    _this.add_config_change('update_proxy_port', old_proxy.port,
        Object.assign({}, new_proxy), opt.source, opt.username);
...
function*mgr_proxy_update_api(_this, req, res, next){
    ...
    const {proxy_port, proxy_err} = yield _this.proxy_update(old_proxy,
        req.body.proxy, {source: get_source(req.headers),
        username: get_username(req)});
    if (proxy_err)
        return res.status(400).json({errors: [{msg: proxy_err}]});
...
function*mgr_proxies_delete_api(_this, req, res, next){
    ...
    ports.forEach(port=>_this.add_config_change('remove_proxy_port', port,
        undefined, get_source(req.headers), get_username(req)));
...
etask._fn(function*(_this, settings, opt={}){
    ...
    const add_config_change = (...args)=>{
      if (origin)
          _this.add_config_change(...[...args, source, username]);
    };
    ...
    for (const field in settings)
    {
        ...
        case 'zone':
            ...
            add_config_change('update_zone', 'defaults', val);
            emit_zone_updated = 1;
            break;
        case 'har_limit':
            _this._defaults[field] = val;
            _this.update_ports({har_limit: val});
            add_config_change('update_har_limit', 'defaults', val);
            break;
        case 'debug':
            _this._defaults[field] = val;
            _this.opts[field] = val;
            _this.update_ports({debug: val});
            add_config_change('update_debug', 'defaults', val);
            break;
        case 'lpm_auth':
            _this._defaults[field] = val;
            _this.opts[field] = val;
            _this.update_ports({lpm_auth: val});
            add_config_change('update_lpm_auth', 'defaults', val);
            break;
        case 'logs':
            _this._defaults[field] = val;
            _this.loki.request_trunc(val);
            add_config_change('update_logs', 'defaults', val);
            break;
        case 'log':
            _this._defaults[field] = val;
            _this.set_logger_level(val);
            add_config_change('update_log_level', 'defaults', val);
            break;
        case 'request_stats':
        ...
        case 'www_whitelist_ips':
            ...
            break;
        case 'whitelist_ips':
            add_config_change('update_whitelist_ips', 'defaults', val);
        ...
        case 'logs_settings':
            _this._defaults[field] = val;
            add_config_change('update_logs_settings', 'defaults', val);
            _this.update_ports({logs_settings: val});
            break;
        case 'bw_limit_webhook_url':
        case 'bw_th_webhook_url':
            _this._defaults[field] = val;
            add_config_change('update_'+field, 'defaults', val);
            break;
        }
    }
    ...
    if (_this._defaults.sync_config)
    {
        _this.wss.broadcast_json({
            ...
            settings: Object.assign({}, _this.get_settings()),
            defaults: Object.assign({}, _this.opts),
        });
...
function*mgr_creds(_this, req, res, next){
    ...
    const login_result = yield _this.login_user(Object.assign({}, req.body));
...
E.prototype.update_ports = function(opt, proxy_ports){
    ...
    Object.values(proxy_ports || this.proxy_ports).forEach(p=>{
...
E.prototype.err2res = function(err, res){
    ...
    if (err.headers)
        Object.keys(err.headers).forEach(h=>res.set(h, err.headers[h]));
    if (err.msg)
        res.send(err.msg);
};
...
function*pmgr_domain_update_api(_this, req, res, next){
    ...
    _this.add_config_change('pmgr_domain_update_api', 'defaults',
        req.body.domain, get_source(req.headers), get_username(req));
    yield _this.config.save();
    res.send('OK');
});
