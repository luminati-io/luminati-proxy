...
E.prototype.stats_aggregator = etask._fn(
function*mgr_handle_usage(_this, data){
    this.on('uncaught', e=>{
        _this._log.error('handle usage %s', zerr.e2s(e));
    });
...
E.prototype.history_aggregator = etask._fn(function*hist_agg_mgr(_this, data){
    this.on('uncaught', e=>{
        _this._log.error('history aggregator %s', zerr.e2s(e));
    });
    if (!_this._defaults.logs.value)
    {
        const har_req = _this.har([data]).log.entries[0];
        return _this.emit('request_log', har_req);
    }
