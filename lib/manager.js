...
E.prototype.get_fast_api = function(req, res){
    ...
    const r = req.query.r||'**';
    const server = this.proxies_running[port];
    const _sessions = server.session_mgr.get_fast_sessions(r).map(s=>({
        ip: s.last_res.ip,
        host: s.host,
        session: s.session,
        username: s.username,
    }));
    res.json({sessions: _sessions});
};
...
E.prototype.create_api_interface = function(){
    ...
    app.post('/creds_user', this._api(this.creds_user_set));
    ...
    app.post('/react_error', this._api(this.react_error));
    return app;
};
...
function*mgr_run_stats_reporting(_this){
    while (_this.is_running)
    {
        ...
        try {
            ...
            let fd = 0;
            if (_this.ulimit_mgr)
                fd = yield _this.ulimit_mgr.count_fd();
            stats = {
...
E.prototype.react_error = etask._fn(function*mgr_react_error(_this, req, res){
