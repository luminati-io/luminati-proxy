...
function*mgr_create_proxy(_this, proxy){
    ...
    const password = get_password(proxy, proxy.zone, _this.zones) ||
    ...
    // XXX andreish: quick fix to set correct unblock option in conf
    const zones_map = _this.zones.reduce((zs, z)=>{
        zs[z.zone] = z;
        return zs;
    }, {});
    const servers = yield etask.all(proxies.map(c=>{
        return _this.create_single_proxy(Object.assign(c, {zones: zones_map}));
    }));
...
E.prototype.test_port = etask._fn(function*lum_test(_this, proxy){
    ...
    try {
        const r = yield util_lib.json({
            ...
            method: 'GET',
            proxy: `http://127.0.0.1:${proxy.port || proxy.conf.port}`,
