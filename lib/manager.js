...
E.prototype.update_settings_api = function(req, res){
    ...
    if (req.body.log)
    {
        this._defaults.log = req.body.log;
        this.set_logger_level(req.body.log);
    }
    this.config.save();
...
E.prototype.lpm_users_get = etask._fn(function*mgr_lpm_users_get(_this){
    const response = yield _this.api_request({
        url: `${_this._defaults.api}/lpm_users`,
    });
...
E.prototype.lpm_user_add_api = etask._fn(function*mgr_user(_this, req, res){
    const _res = yield _this.api_request({
        url: `${_this._defaults.api}/lpm_users_add`,
        method: 'POST',
...
function*mgr_refresh_ips(_this, req, res){
    ...
    if (Array.isArray(ips))
        ips = ips.map(ip=>zurl.ip2num(ip)).join(' ');
    const r = yield _this.refresh_ips(zone, ips);
    if (r.error)
        return void res.status(r.status).json(r);
    res.json(r);
});
...
E.prototype.api_request = etask._fn(function*mgr_api_request(_this, opt){
    if (!_this.logged_in && !opt.force)
        return logger.notice('Skipping API call before auth: %s', opt.url);
    ...
    const xsrf = jar && (jar.getCookies(_this._defaults.api).find(e=>
        e.key=='XSRF-TOKEN')||{}).value;
    const {customer, google_token} = _this._defaults;
    ...
    {
        let msg = `API call to ${opt.url} FAILED with status `
            +res.statusCode;
        if (res.body && res.statusCode!=404)
            msg += ' '+res.body.slice(0, 40);
        throw new Error(msg);
    }
    return res;
});
...
function*ext_proxy_created(_this, proxy){
    ...
    yield _this.api_request({
        url: `${_this._defaults.api}/ext_proxy_created`,
        method: 'POST',
        form: {proxy},
    });
});
...
E.prototype.add_www_whitelist_ip_api = function(req, res){
    ...
    this.config.save();
    this.wss.broadcast(ip, 'whitelisted');
    res.send('OK');
};
...
E.prototype.logged_update = etask._fn(function*mgr_logged_update(_this, ua){
    ...
    try {
        const conf = yield _this.get_lum_local_conf();
...
function*verify_two_token_api(_this, req, res){
    try {
        const response = yield _this.api_request({
            ...
            form: {token: req.body.token, lpm_token: _this.lpm_token},
            force: true,
            json: false,
        });
...
E.prototype.get_lum_local_conf = etask._fn(function*(_this){
    this.on('uncaught', e=>{
        if (!e.status)
    ...
    if (!google_token && !_cookie && !global.it)
        throw new Error('no google token and cookie');
    const config = yield etask.nfn_apply(request, [{
        qs: {customer, token: google_token},
    ...
    if (google_token)
        _this._defaults.google_token = google_token;
    return _this.lum_conf;
});

E.prototype.login_user = etask._fn(
function*mgr_login_user(_this, username, password){
    const google_token = _this._defaults.google_token;
    const customer = _this._defaults.customer;
    if (!_this.luminati_jar)
        _this.luminati_jar = {jar: _this.get_cookie_jar()};
    const response = yield _this.api_request({
        method: 'POST',
        url: _this._defaults.api+'/lpm/check_credentials',
        form: {
            username,
            password,
            customer,
...
E.prototype.check_conn = etask._fn(function*mgr_check_conn(_this){
    ...
    _this.conn = {domain, current_country: _this.current_country};
});
...
E.prototype.api_resolve_proxies = etask._fn(function*(_this){
    try {
        const response = yield _this.api_request({
            url: `${_this._defaults.api}/resolve_super_proxy`,
            qs: {limit: 20},
            force: true,
        });
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        _this.update_lpm_users(yield _this.lpm_users_get());
        yield cities.ensure_data(_this._defaults.api_domain);
        if (_this.argv.www)
        {
