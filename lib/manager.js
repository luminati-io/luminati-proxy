...
function Manager(args, run_config){
    ...
    this._log.debug('Effective args', this.argv);
    ...
    this.stats_aggregator = data=>{
        let stat = _.pick(data, qw`response_headers request_headers status_code
            status_message response_time url proxy_peer`);
...
function _get_proxy_configs(_this, args){
    ...
    _this._log.debug('argv_explicit', _this.argv_explicit);
...
E.prototype._complete_proxy_config = function(conf){
    ...
    if (c.zones&&c.zones[c.zone])
        c.mobile = !!(get_zone_perm(c.zones[c.zone])||'').includes('mobile');
    else
        c.mobile = false;
...
function*create_proxy(_this, proxy){
    ...
    {
        conf._vips = conf.vips;
        conf.vips = [conf.vips[0]];
    }
...
E.prototype.request_stats_reset = function(req, res){
    if (this.request_stats)
        this.request_stats = {};
    this.req_status_cnt_reset();
    res.end();
};

E.prototype.get_req_status_cnt = function get_req_status_cnt(){
    const proxies = Object.keys(this.proxies_running).map(key=>({key,
        value: this.proxies_running[key].req_status_cnt}));
    return proxies.reduce((acc, proxy)=>{
        acc.total += proxy.value.total;
        acc.success += proxy.value.success;
        acc[proxy.key] = proxy.value;
        return acc;
    }, {total: 0, success: 0});
};

E.prototype.req_status_cnt_reset = function req_status_cnt_reset(){
    const proxies = Object.keys(this.proxies_running)
    .map(key=>this.proxies_running[key]);
    proxies.forEach(p=>p.req_status_cnt_reset());
};
...
function*create_web_interface(_this){
    ...
    app.get(/^\/((|proxies|tools|faq|intro|howto)|(zones|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
...
E.prototype.start = etask._fn(function*start(_this){
    try {
        ...
        if (_this.argv.www)
        {
            ...
            console.log('open admin browser app ('+_this.www_server.url+')');
        }
...
E.prototype.run_stats_reporting = etask._fn(function*run_stats_reporting(_this)
{
...
E.prototype.perr = function(id, info, opt){
    ...
    info.stats = this.get_req_status_cnt();
