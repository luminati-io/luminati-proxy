...
const proxy_fields = {
    port: 'Listening port',
    log: `Log level (${Object.keys(log.log_level).join('|')})`,
    customer: 'Customer',
    password: 'Password',
...
Manager.prototype.history_get = function(req, res){
    let port = req.params.port;
    this.db.stmt.query_history.all([port], (err, rows)=>{
        const methods = _.uniq(_.map(rows, 'method'));
        methods.push('');
        methods.sort();
        const status_codes = _.uniq(_.map(rows, 'status_code'));
        status_codes.push('');
        status_codes.sort();
        if (req.query.url)
            rows = rows.filter(el=>el.url.includes(req.query.url));
        if (req.query.method)
            rows = rows.filter(el=>el.method==req.query.method);
        if (req.query.status_code)
            rows = rows.filter(el=>el.status_code==req.query.status_code);
        if (req.query.timestamp_min)
            rows = rows.filter(el=>el.timestamp>=req.query.timestamp_min);
        if (req.query.timestamp_max)
            rows = rows.filter(el=>el.timestamp<=req.query.timestamp_max);
        if (req.query.elapsed_min)
            rows = rows.filter(el=>el.elapsed>=req.query.elapsed_min);
        if (req.query.elapsed_max)
            rows = rows.filter(el=>el.elapsed<=req.query.elapsed_max);
        if (req.query.proxy)
            rows = rows.filter(el=>el.proxy.includes(req.query.proxy));
        if (req.query.sort)
        {
            let field;
            if (req.query.sort=='proxy')
            {
                rows.forEach(a=>a.proxy_sort = a.proxy.split('.').map(
                        v=>('00'+v).slice(-3)).join('.'));
                field = 'proxy_sort';
            }
            else
            ...
            if (req.query.sort=='proxy')
                rows.forEach(a=>delete a.proxy_sort);
        }
        const page_size = 10;
        ...
        if (req.query.page)
        {
            page = Math.max(1, Math.min(pages, req.query.page|0));
            rows = rows.slice((page-1)*page_size,
