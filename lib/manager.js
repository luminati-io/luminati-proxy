...
const credential_params = ['customer', 'zone', 'password'];
const proxy_params = _(swagger.definitions.proxy.properties)
    .pick(k=>!credential_params.contains(k)).keys().concat(['no-config']);
...
const load_proxy_config = file_config=>{
    file_config = file_config||{};
    return [].concat(file_config.proxies || file_config);
};
...
function Manager(args, run_config){
    ...
    this._file_config = load_json(_yargs.argv.config, true, {});
    this._defaults = this._file_config._defaults||{};
    if (credential_params.some(k=>args.includes(`--${k}`)))
    {
        this._defaults = _.pick(this._defaults,
            (v, k)=>!credential_params.includes(k));
    }
    ...
    if (!proxy_params.some(k=>args.includes(`--${k}`)) && !args.includes('-p'))
    {
        this.config = load_proxy_config(this._file_config)
            .map(conf=>assign(conf, {proxy_type: 'persist'}));
    }
    this.config = this.config.concat([].concat.apply([], this.argv._
        .map(filename=>load_proxy_config(load_json(filename, true, {})))
        .map(conf=>assign(conf, {proxy_type: 'config'}))));
