...
E.prototype._get_proxy_configs = function(){
    ...
    {
        qw`port`.filter(k=>_this.argv.explicit_opt[k]).forEach(k=>{
...
E.prototype._complete_proxy_config = function(conf){
    const c = assign({current_ip: this.opts.current_ip}, E.default,
        this._defaults, conf, _.omit(this.argv.overlay_opt, 'password'));
    const zone = c.zones && c.zones[c.zone];
    const zone_plan = zone && get_plan(zone.plans);
...
E.prototype.link_test_api = etask._fn(function*mgr_link_test(_this, req, res){
    ...
    const opt = assign(_.pick(req.query, qw`url country city state carrier
