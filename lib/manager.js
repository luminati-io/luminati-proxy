...
const PROXY_ONLY_BYPASS = qw`googleapis.com cdnjs.com gstatic.com
    googleusercontent.com google.com luminati.io hwcdn.net`;
...
Manager.prototype.get_consts = function(req, res){
    ...
    get_countries(_this.cities_db, countries=>{
        countries.forEach(c=>c.key=`${c.value}: ${c.key}`);
...
function*proxy_status_get(_this, req, res){
    ...
    const period = 1000;
    let port = req.params.port;
    var proxy = _this.proxies_running[port];
    if (!proxy)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            let zone = proxy.opt.zone||_this.argv.zone;
            if (zones[zone])
            {
                let perms = zones[zone].perm.split(' ');
                for (let p of ['country', 'state', 'city', 'asn', 'ip'])
                {
                    if (proxy.opt[p]&&!perms.includes(p))
                    {
                        proxy.status = `The "${p}" permission is absent.`;
                        break;
                    }
                }
            }
            else
                proxy.status = 'The provided zone name is not valid.';
            if (!proxy.status)
            {
                let creds = {};
                if (proxy.opt.customer)
                    creds.customer = proxy.opt.customer;
                if (proxy.opt.zone)
                    creds.zone = proxy.opt.zone;
                if (proxy.opt.password)
                    creds.password = proxy.opt.password;
                let creds_valid =
                    (yield _this.check_credentials(creds)).result;
                proxy.status = creds_valid ?
...
Manager.prototype.get_regions = function(req, res){
    ...
    this.cities_db.all(query, [c_id], (err, rows)=>{
        let d = (rows||[]).map(x=>{
            return {key: x.region_id+': '+x.name, value: x.region_id}; });
...
function*check_credentials(_this, cred){
    ...
    try {
        ...
        if (res.statusCode==401)
            return {result: false, error: res.body.error};
...
Manager.prototype.creds_set = function(req, res){
    let zone_autoupdate = req.query.autoupdate && req.body.zone &&
        req.body.zone!=this.argv.zone ?
        {zone: req.body.zone, prev: this.argv.zone} : false;
    assign(this._defaults, req.body);
    this.save_config();
    res.json({result: 'ok'});
    this.emit('config_changed', zone_autoupdate);
};

Manager.prototype.creds_user_set = etask._fn(
function*creds_user_set(_this, req, res){
    let config = yield _this.check_user(
    ...
    _this.sql('DELETE FROM zone');
    let zones = config.defaults.zones;
    let query = 'INSERT INTO zone(zone, perm) VALUES (?, ?)';
    for (let zone in zones)
        yield _this.sql(query, zone, zones[zone].perm);
    let zone_autoupdate =
...
Manager.prototype.upgrade = etask._fn(function*(_this, req, res){
    ...
    try {
        ...
        if (is_win)
        {
            setTimeout(function(){
                res.json({result: 'ok'});
                console.log('Force stop after 5sec');
                process.exit();
            }, 5000); // XXX lee - ask danylo why this delay
        }
        yield task;
        child_process.fork(process.argv[1], process.argv.slice(2),
            {detached: true});
        res.json({result: 'ok'});
        yield _this.stop('upgrade', true);
...
function*check_user(_this, username, password, customer){
    const url = 'https://luminati.io';
    if (!_this.luminati_jar || _this.luminati_jar.password!=password ||
...
Manager.prototype.start = etask._fn(function*start(_this){
    try {
        ...
        _this.db.stmt.history = _this.db.db.prepare(`INSERT INTO request (port,
            ...
            $proxy_peer, $username, $content_size, $download_size,
            $upload_size)`);
        yield _this.logged_update();
