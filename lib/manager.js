...
function*mgr_creds(_this, req, res, next){
    ...
    _this.save_username(req.body.username);
    res.json({result: 'ok'});
});
...
E.prototype.cloud_auth_api = function(req, res){
    ...
    this.save_username(req.body.username);
    res.send('OK');
};
...
E.prototype.save_username = function(username){
    const filename = this.argv.username;
    if (username && zurl.is_valid_email(username))
    {
        try { fs.writeFileSync(filename, JSON.stringify({username}));
        } catch(e){
            logger.error('Couldn\'t save %s: %s', filename, e.message); }
    }
    else
        this.delete_username();
};

E.prototype.delete_username = function(){
    const filename = this.argv.username;
    try { fs.writeFileSync(filename, JSON.stringify({}));
    } catch(e){
        logger.error('Could not save %s: %s', filename, e.message);
    }
};

E.prototype.get_username = function(){
    const filename = this.argv.username;
    const buffer = fs.readFileSync(filename);
    let raw_username = buffer.toString();
    let user;
    try { user = JSON.parse(raw_username); }
    catch(e){ logger.error('Couldn\'t read %s: %s', filename, e.message); }
    if (zurl.is_valid_email(user && user.username))
