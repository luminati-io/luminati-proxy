...
function*mgr_api_url_update_api(_this, req, res){
    _this._defaults.api_domain = req.body.url.replace(/https?:\/\/(www\.)?/,
        '');
    _this._defaults.api = 'https://'+_this._defaults.api_domain;
...
function*mgr_get_all_locations(_this, req, res){
    ...
    const shared_countries = yield _this.api_request(
        {endpoint: '/users/zone/shared_block_countries'});
...
E.prototype.refresh_ips = etask._fn(function*fresh_ips(_this, zone, ips){
    const response = yield _this.api_request({
        method: 'POST',
        endpoint: '/users/zone/refresh_ips',
...
function*mgr_refresh_vips(_this, req, res){
    ...
    const response = yield _this.api_request({
        method: 'POST',
        endpoint: '/users/zone/refresh_vips',
...
E.prototype.check_domain = etask._fn(function*mgr_check_domain(_this){
    ...
    const _url = `https://${_this._defaults.api_domain}/lpm_config.json`
        +`?ver=${pkg.version}`;
    try {
        const res = yield etask.nfn_apply(request, [{
            url: _url,
