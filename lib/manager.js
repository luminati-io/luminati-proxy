...
function Manager(args, run_config){
    ...
    this.stats_aggregator = data=>{
        try {
            ...
            if (data.response_body.type)
                data.response_body = Buffer.from(data.response_body.data);
            data.response_body = data.response_body.slice(0, 2048);
            let url_parts;
        ...
        try {
            ...
            etask(function*history_aggregator(){
                try {
                    if (!opt.full_logs)
                    {
                        let ports = [..._this.stats_proxies].join(',');
                        let limit = _this._defaults.request_stats_limit;
                        const curr_count = yield _this.sql(`SELECT COUNT(*) AS
                            count FROM request WHERE port IN (${ports});`);
                        if (curr_count[0].count>=limit)
                        {
                            let delete_limit = Math.floor(
                                curr_count[0].count-limit/2);
                            yield _this.sql(`DELETE FROM request
                                WHERE port IN (${ports}) AND uuid IN(
                                SELECT uuid FROM request
                                WHERE port IN (${ports})
                                ORDER BY timestamp
                                ASC LIMIT ${delete_limit});`
                            );
                        }
                    }
                    let row = {};
...
function _get_proxy_configs(_this, args){
    ...
    _this.argv_overlay = _.omit(_this.argv_explicit, qw`port`);
    ...
    if (qw`port`.some(k=>_this.argv_explicit[k]))
    {
        qw`port`.filter(k=>_this.argv_explicit[k])
        .forEach(k=>{
...
function*mgr_prepare_database(_this){
    ...
    if (cluster_mode.is_worker(true))
        return;
    let now = Date.now();
...
function*mgr_create_single_proxy(_this, conf){
    ...
    if (!server.opt.history)
        _this.stats_proxies.add(server.opt.listen_port);
    if (conf._update)
    {
        setImmediate(()=>{
...
function*mgr_create_proxy(_this, proxy){
    ...
    server.stop = ()=>etask(function*mgr_server_stop(){
        ...
        return yield etask.all(servers.map(s=>{
            ...
            _this.stats_proxies.delete(port);
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port){
    ...
    if ((null_response||bypass_proxy||direct_include) &&
        _ssl!==undefined && !_ssl)
    {
        info.push({
...
E.prototype.trace = etask._fn(function*mgr_trace(_this, req, res){
    const opt = assign(_.pick(req.body, qw`url port`));
    // XXX krzysztof: move building Tracer on lpm start
    const tracer = new Tracer(_this.wss, _this.proxies_running);
    const result = yield tracer.trace(opt.url, opt.port);
    res.json(result);
});
...
E.prototype.logs_remove = etask._fn(function*mgr_logs_remove(_this, req, res){
    const arr = (req.body.uuids||[]).join(', ');
    yield _this.sql(`DELETE FROM request where uuid in (${arr})`);
    res.send('ok');
});
...
E.prototype.update_settings = function(req, res){
...
E.prototype.create_api_interface = function(){
    ...
    app.post('/logs_remove', this._api(this.logs_remove));
    ...
    app.get('/settings', this._api((req, res)=>res.json(this.get_settings()),
        ['root']));
    app.put('/settings', this._api(this.update_settings, ['root']));
    ...
    app.post('/trace', this._api(this.trace));
...
function*mgr_create_web_interface(_this){
    ...
    app.use((req, res, next)=>{
        const is_whitelisted = (_this.www_whitelist_blocks||[]).some(wb=>{
            try { return wb.contains(req.ip); } catch(e){ return false; }
        });
        if (is_whitelisted)
            return void next();
        res.status(403);
        return res.send(`Connection from your IP is forbidden. If you want to`
            +` access this site ask the administrator to add ${req.ip} to the`
            +` whitelist`);
    });
    ...
    app.get(/^\/((|login|howto|tracer|proxy_tester|logs|dock_logs|config|settings)|(overview|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
...
class Ulimit_manager {
    ...
    handle_overflow(){
        ...
        {
            this.log.crit('Too many opened files, please raise limits');
