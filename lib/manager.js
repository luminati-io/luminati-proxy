...
function*mgr_stop_servers(_this){
    ...
    const stop_server = server=>servers.push(etask(function*mgr_stop_server(){
        try {
            ...
            _this._log.error('Failed to stop server: %s', e.message);
        }
    }));
    if (_this.www_server)
...
function*mgr_create_single_proxy(_this, conf){
    ...
    if (_this.argv.cluster)
    {
        Object.values(cluster.workers).forEach(w=>{
            w.send({code: 'CREATE', conf});
            _this.servers[conf.port] = conf;
        });
        return {};
    }
    const server = new Luminati(conf, _this);
    server.on('error', err=>{
        // XXX krzysztof: move this logic to error handler
        if (err.code=='EMFILE')
        {
            if (_this.emfile_err)
                return;
            _this.emfile_err = true;
            return _this._log.error('EMFILE: out of file descriptors');
        }
...
const zones_from_conf = config=>{
    ...
    return _.map(config._defaults.zones, (v, k)=>{
        return {zone: k, perm: get_perm(v), plan: v.plan,
...
E.prototype.run_cluster = function(){
    ...
    for (let i=0; i<cores; i++)
    {
        ...
        worker.on('message', message=>{
            console.log('message from worker: %s', message);
        });
    }
