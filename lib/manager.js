...
E.prototype.get_zones = etask._fn(function*get_zones(_this){
    ...
    return _.map(config.defaults.zones, (v, k)=>{
...
function*get_lum_local_conf(_this, customer, token){
    if (!_this.luminati_jar)
        _this.luminati_jar = {jar: get_cookie_jar()};
...
function*check_user(_this, token, username, password, customer){
    ...
    let config = yield _this.get_lum_local_conf(customer, token);
    if (!config.error && !config.customers)
