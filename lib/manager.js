...
function Manager(argv, run_config){
    ...
    this.handle_usage = data=>{
        try {
            ...
            setImmediate(()=>this.stats_aggregator(data));
            setImmediate(()=>this.history_aggregator(data));
        } catch(e){
            this._log.error('stats aggregator %s', zerr.e2s(e));
        }
    };
    this.stats_aggregator = etask._fn(function*mgr_handle_usage(_this, data){
        if (data.context=='STATUS CHECK'||!_this._defaults.request_stats||
    ...
    this.history_aggregator = data=>{
        if (!this._defaults.logs.value)
        {
            ...
            return void this.emit('request_log', har_req);
        }
        const _this = this;
        etask(function*history_aggregator(){
            ...
            yield _this.adjust_logs_size({delete_more: true});
            const row = {};
            for (let f in data)
                row['$'+f] = data[f];
...
E.prototype.clear_stats_sql = etask._fn(function*clear_stats_sql_mgr(_this){
...
E.prototype.save_config = function(){
    ...
    const proxies = this.proxies
        ...
        .map(p=>_.omit(p, qw`stats proxy_type zones _update
            www_whitelist_ips request_stats logs`))
...
E.prototype.prepare_db_stmt = function(){
    this.db.main.stmt = {
        ...
        history: this.db.main.prepare(`INSERT INTO request (port, url,
            ...
            $elapsed, $response_time, $country, $timeline, $super_proxy,
            $proxy_peer, $username, $content_size, $context, $success, $out_bw,
            $in_bw, $response_body, $protocol, $hostname, $content_type,
            $remote_address, $password)`),
    };
};
...
E.prototype.timeout_test_api = etask._fn(
function*timeoout_test_api_mgr(_this, req, res){
    yield etask.sleep(+req.query.sleep);
    res.send('ok');
});
...
function*mgr_get_consts(_this, req, res){
    ...
    const pool_types = Object.keys(Luminati.pool_types).map(pool_type=>{
...
E.prototype.adjust_stats = etask._fn(function*adjust_stats_mgr(_this){
    if (!_this._defaults.request_stats)
        return yield _this.clear_stats_sql();
});
...
function*mgr_basic_stat(_this, ports_cond='IS NOT NULL'){
    ...
    const queries = keys.map(k=>
        ...
        AND port_id ${ports_cond} GROUP BY ${k} ORDER BY reqs DESC LIMIT 40`)
    );
...
E.prototype.create_api_interface = function(){
    ...
    app.use((req, res, next)=>{
        ...
        // XXX krzysztof: make a mechanism for bypassing part of the endpoints
    ...
    app.get('/timeout_test', this._api(this.timeout_test_api));
    return app;
};
