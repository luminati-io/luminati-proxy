...
function Manager(argv, run_config={}){
    ...
    try {
        ...
        logger.set_level(this.argv.log);
        ...
        this.on('error', (e, fatal)=>{
            ...
            {
                this.perr('crash', {}, {error: zerr.e2s(e)});
                handle_fatal();
            }
        });
    } catch(e){
...
E.should_resolve_proxy = conf=>{
    ...
    if (conf.unblock || conf.preset=='rotating' && max_requests<=1)
        return false;
    return true;
};
...
E.prototype.create_proxy = etask._fn(
    ...
    // XXX krzysztof: check if this is needed
...
E.prototype.config_check_api = function(req, res){
    ...
    try { errors = this.config.check(JSON.parse(req.body.config)); }
        ...
        logger.warn('Config parsing error %s', e.message);
...
function*mgr_get_all_carriers(_this, req, res){
    const c_res = yield etask.nfn_apply(request, [{
        url: `http://client.${_this._defaults.www_domain}/api/carriers`,
...
E.prototype._last_version = etask._fn(function*mgr__last_version(_this){
    return yield util_lib.get_last_version(_this._defaults.api);
});
...
E.prototype.api_request = etask._fn(function*mgr_api_request(_this, opt){
    ...
    {
        logger.warn('API call to %s FAILED with status %s %s', opt.url,
...
E.prototype.sync_config_file = etask._fn(function*mgr_sync_config(_this){
    const config = _this.config.from_file();
    yield _this.api_request({
        url: `${_this._defaults.api}/update_lpm_config`,
        method: 'POST',
        form: {config: {proxies: config.proxies.slice(0, 20),
...
E.prototype.init_lpm_conn_ws = function(){
    if (!this.argv.cloud)
        return;
    this.ws_agent_conn = new zws.Client('ws://zagent75.luminati.io:3361', {
        label: 'lpm_conn',
    ...
    const _this = this;
    this.ws_agent_conn.on('connected', ()=>etask(function*(){
        const res = yield _this.ws_agent_conn.ipc.auth({
            cname: _this._defaults.customer,
            passwd: 'pass',
        });
        if (!res)
            logger.warn('Auth failed with wss lpm agent');
        else
            logger.notice('Auth OK with wss lpm agent');
    }));
};
            ...
            if (response.body=='unauthorized')
            {
                return {error: {
                    message: `The email address or password is incorrect. `
                        ...
                        +` <a href="${_this._defaults.www_api}/?need_signup=1"`
                        +` target=_blank>`
        ...
        try {
            ...
            if (!config && e.status!=200)
            {
                ...
                if (message=='Your account is not active')
                {
                    ...
                    +`<a href='${_this._defaults.www_api}/cp/billing'>`
...
E.prototype.get_ip = etask._fn(function*mgr_set_location(_this){
    ...
    if (!_this.current_country || _this.current_country=='cn')
        _this._defaults.www_domain = pkg.api_domain;
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        _this.update_lpm_users(yield _this.lpm_users_get());
        yield cities.ensure_data(_this._defaults.www_domain);
        if (_this.argv.www)
        {
...
function*mgr_error(_this, req, res){
    ...
    yield _this.perr(type, {message, context}, _.pickBy({backtrace: stack}));
    res.send('OK');
});
