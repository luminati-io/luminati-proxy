...
function*mgr_create_proxy(_this, proxy){
    ...
    server.stop = ()=>etask(function*mgr_server_stop(){
        ...
        _this.log.notice('Stopping proxies %s', servers.map(s=>s.port ||
...
E.prototype.update_settings_api = function(req, res){
    ...
    if ((ips=req.body.whitelist_ips)!==undefined)
    {
        if (ips==='')
            ...
            this._defaults.whitelist_ips = ips.split(',');
    }
    this.config.save();
...
E.prototype.add_wip_api = function(req, res){
    ...
    this._defaults.whitelist_ips = [...new Set(
        this._defaults.whitelist_ips).add(ip)];
    this.config.save();
    res.send('OK');
};
...
function*mgr_fix_config(_this, new_conf){
    ...
    yield etask.all(_this.proxies.map(p=>etask(function*mgr_fix_proxy_config(){
        ...
        {
            // XXX krzysztof: cp.password is an Array
            // XXX krzysztof: bug: _this.zones is an empty Array
            if (p.zone)
