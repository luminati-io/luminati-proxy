...
function Manager(args){
    ...
    .boolean(qw`history sticky_ip no_dropin ssl`)
    ...
    this.opts = _.pick(this.argv, qw`zone country state city asn max_requests
        ...
        proxy_switch sticky_ip proxy_port`);
...
Manager.prototype.save_config = function(filename){
    ...
    let _defaults = _.pick(this.argv, qw`customer password max_requests
        no_dropin port proxy proxy_count proxy_switch proxy_port
        session_timeout request_timeout ssl sticky_ip www zone`);
...
Manager.prototype.check_creds = etask._fn(function*check_creds(_this){
