...
E.prototype.set_whitelist_ips = function(ips){
    ...
    this.update_ports({whitelist_ips: this._defaults.whitelist_ips});
};
...
function*mgr_create_single_proxy(_this, conf){
    ...
    proxy.on('retry', opt=>{
        const serv = _this.get_server(opt.port);
        if (!serv)
            return _this.log.warn('retry failed: no server %s', opt.port);
        serv.lpm_request(opt.req, opt.res, opt.head);
        serv.once('response', opt.post);
    });
    ...
    proxy.on('add_static_ip', data=>{
        ...
        const serv = _this.proxies_running[data.port];
...
function*mgr_proxy_dup_api(_this, req, res){
    ...
    const ports = Object.assign({}, _this.proxies_running, _this.proxy_ports);
    proxy.port = get_free_port(ports);
...
E.prototype.banip = function(ip, domain, ms){
    const ports = [].concat(Object.values(this.proxies_running),
        Object.values(this.proxy_ports));
    ports.forEach(p=>{
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port){
    const ports = Object.assign({}, _this.proxies_running, _this.proxy_ports);
    const old_proxy = old_proxy_port && ports[old_proxy_port]
        && ports[old_proxy_port].opt || {};
    const info = [];
...
E.prototype.link_test_api = etask._fn(function*mgr_link_test(_this, req, res){
    ...
    const tracer = new Tracer(_this, _this.wss, ports, _this._defaults.zones,
        _this.opts.log);
...
E.prototype.logs_resend_api = function(req, res){
    ...
    for (let i in ids)
    {
        ...
        let proxy;
        const ports = Object.assign({}, this.proxies_running,
            this.proxy_ports);
        if (!(proxy = ports[r.port]))
            continue;
        const opt = {
...
E.prototype.update_ports = function(opt){
    const ports = [].concat(Object.values(this.proxies_running),
        Object.values(this.proxy_ports));
    ports.forEach(p=>p.update_config(opt));
};
...
E.prototype.has_created_proxy_port = function(){
    ...
    return Object.values(ports).some(p=>p.opt.proxy_type=='persist');
};
