...
const UOS = util_lib.format_platform(os.platform());
...
const get_source = ({headers})=>headers.referer ? consts.SOURCES.UI
    : consts.SOURCES.API;

const get_username = req=>cookie.parse(req.headers.cookie||'').username;
...
E.prototype.post_event = function(name, source, username, payload){
    this.lpm_f.event(assign(payload||{}, {name, source, username}));
};
...
const headers_to_a = h=>entries(h).map(p=>({name: p[0], value: p[1]}));
...
E.prototype.complete_proxy_config = function(conf){
    ...
    const perms = !!perm && perm.split(' ');
    c.ssl_perm = !!(plan && plan.ssl);
    c.state_perm = !!perms && perms.includes('state');
    c.zip_perm = !!perms && perms.includes('zip');
    ...
    old_val)
{
    ...
    let ev_new = payload && change_omit_fields.some(f=>payload[f]) ?
    ...
    this.post_event(key, source, username, assign(ev_payload||{}, {area}));
};
...
const get_ports = p=>Array.isArray(p) ? p.map(x=>x.port) : keys(p);

const get_nearest_port = (proxies, port)=>{
    const ports = get_ports(proxies);
    do
        port++;
...
const get_free_port = (proxies, zagent)=>{
    const proxy_ports = get_ports(proxies);
...
function*mgr_proxy_update(_this, old_proxy, new_proxy, opt={}){
    ...
    if (rules_changed)
    {
        _this.post_event('Rules Change', opt.source, opt.username,
...
function*mgr_proxy_check(_this, new_proxy_config, old_proxy_port, opt={}){
...
E.prototype.proxy_tester_api = function(req, res){
    ...
    const opt = assign(zutil.pick(req.body, ...qw`url headers body`),
...
E.prototype.logs_reset_api = function(req, res){
    ...
    this.post_event('Clear HAR', get_source(req), get_username(req));
    res.send('ok');
};
...
E.prototype.test_logs_remote = etask._fn(function*(_this, req, res, next){
    this.on('uncaught', e=>{
        logger.warn('Custom logs test fail: ' + e.message);
        res.send(e.message);
        next();
    });
    if (!_this.argv.zagent)
   ...
   if (!type || !logger.remote_transports[type])
    ...
    transport.on('logged', p=>this.continue());
    ...
    res.send(test_res || 'ok');
});

E.prototype.logs_har_get_api = function(req, res){
    this.post_event('Download HAR', get_source(req), get_username(req));
...
etask._fn(function*(_this, settings, opt={}){
    ...
    const get_add_config_change_fn = (new_v, old_v)=>origin ? (key, area)=>
        _this.add_config_change(key, area, new_v, source, username, old_v)
        : _.noop;
    ...
    for (const field in settings)
    {
        ...
        case 'zone':
            ...
            ports_with_def_zone = current_config.proxies.filter(p=>
                !p.hasOwnProperty('zone')).map(p=>p.port);
...
E.prototype.proxies_running_get_api = function(req, res){
    ...
    for (const p of values(this.proxy_ports))
    {
        ...
        const p_opt_fields = qw`proxy_type port ip vip user zone tls_lib
            route_err proxy_connection_type dns`;
...
E.prototype.cloud_auth_api = function(req, res){
    ...
    {
        const new_ips = [...new Set(this._defaults.whitelist_ips)
            .add(req.remote_ip)];
...
E.prototype.authenticate_ws = function({req}, cb){
    req.remote_ip = this.get_req_remote_ip(req);
    req.query = {};
    let err = this.authenticate(req);
    if (!err)
        return void cb(true);
...
E.prototype.lpm_token_auth_mw = function(req, res, next){
    ...
    if (!this.logged_in || !tokens.length || tokens.includes(auth_lpm_token))
        return void next();
    logger.warn('[Token Auth] access denied for %s %s',
        this.get_req_remote_ip(req), req.url);
    this.err2res({
        status: 403,
...
E.prototype.authenticate = function(req){
    const err = {};
    const bypass = !util_lib.is_ws_upgrade_req(req) && ['/version',
        '/add_wip', '/cloud_auth', '/lpm_stats', '/i18n'].includes(req.url);
...
E.prototype.init_common_api = function(app){
    ...
    app.use(middleware.logger.api(this.argv.www));
...
E.prototype.create_api = function(){
    const app = express();
    app.use(middleware.deprecation.default());
    app.use(middleware.deprecation.sunset('11-01-2023'));
...
E.prototype.create_web_interface = etask._fn(function*(_this){
    ...
    const main_page = (req, res, next)=>{
        ...
        if (_this.whitelist_auth(_this.get_req_remote_ip(req)))
            _this.set_lpm_token_cookie(req, res);
        res.sendFile(path.join(__dirname+'/../bin/pub/index.html'));
    };
    app.use(middleware.security.cookies(_this._defaults.lpm_token,
...
E.prototype.create_api_server = function(app){
    ...
    const tcp_server = net.createServer(socket=>{
        ...
        if (this.lb_ips && this.lb_ips.includes(socket.remoteAddress))
        {
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        if (!_this.argv.zagent)
            _this.post_event('Start Proxy Manager', UOS, _this.last_username);
...
E.prototype.start_web_socket = function(server){
    ...
    class Frontend_ws {
        ...
        ui_event(msg){
            ...
            this.mgr.post_event(msg.name, 'UI', username,
