...
E.prototype.creds_user_api = etask._fn(function*mgr_creds(_this, req, res){
    ...
    _this.lpm_users = yield _this.lpm_users_get();
...
E.prototype.start_auto_update = function(){
    ...
    util_lib.set_timeout(cb = etask._fn(function*mgr_start_auto_update(_this){
        const v = yield _this._last_version();
        if (v.newer && v.auto_update)
            _this._upgrade();
        else
           util_lib.set_timeout(cb, tm);
    }).bind(this), tm);
};
...
function*mgr_run_cpu_usage_monitoring(_this){
    ...
    while (1)
    {
        ...
        if (level)
        {
            let msg = `High CPU usage: ${usage}%. Contact support to `
