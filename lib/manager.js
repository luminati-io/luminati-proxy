...
function*create_proxy(_this, proxy, iface){
    ...
    server.on('response', res=>{
        ...
        if (_this.argv.history)
        {
            ...
            // XXX lee - get that info from the timeline-debug, and only if it does not exists - use the unblocker-debug
            let unblocker_info = (headers['x-hola-unblocker-debug']||'')
                .match(/(\d+\.\d+\.\d+\.\d+) ([^ ]+)/)||['', '', ''];
            let data = {
                port: server.port,
                url: req.url,
                ...
                proxy_peer: unblocker_info[1],
                country: unblocker_info[2],
                timeline: stringify(res.timeline),
                content_size: res.body_size,
            };
            if (res.proxy)
            {
                data.proxy = res.proxy.host;
...
Manager.prototype.archive_timestamps = function(req, res){
    this.db.db.all('SELECT name FROM sqlite_master WHERE type="table"', {},
        (err, rows)=>{
        let timestamps = [];
        for (let r of rows)
        {
...
Manager.prototype.filtered_get = function(req, res, db, callback){
    ...
    const table = (req.query.archive ? `Archive_${req.query.archive}_` : '')
    +'request';
    const wheres = [];
    const vars = {};
...
Manager.prototype.create_api_interface = function(){
    ...
    app.get('/version', this._api((req, res)=>res.json({version: version})));
    ...
    app.get('/archive_timestamps', this._api(this.archive_timestamps));
