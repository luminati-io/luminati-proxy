    ...
    this.history_aggregator = (data, opt={})=>{
        try {
            ...
            if (this.db.history)
            {
                ...
                .catch(err=>this._log.error('sql query error %s',
                    zerr.e2s(err)));
            }
            else
            {
                ...
                etask(function*(){
                    const curr_count = yield _this.sql(`SELECT COUNT(*) AS
                        count FROM request WHERE port = ${data.port}`);
                    if (!opt.full_logs &&
                        curr_count[0].count>=E.default.max_logs)
                    {
                        return;
                    }
                    let row = {};
...
function*mgr_create_socks_servers(_this){
    ...
    function*mgr_create_socks_server(){
        ...
        _this._log.debug(`SOCKS5 is available at port ${port}`);
    })));
});
