...
E.prototype.get_settings = function(){
    return {
        customer: this._defaults.account_id||this._defaults.customer,
...
E.prototype.update_proxies = function(){
    this.proxies.forEach(p=>{
        ...
        {
            p.customer = p.customer || this._defaults.account_id ||
...
etask._fn(function*mgr_apply_cloud_config(_this, config, opt){
    ...
    _this.config.save({skip_cloud_update: 1});
    const has_auth_field = !!zutil.get(config, '_defaults.customer')
...
E.prototype.logged_update = etask._fn(function*mgr_logged_update(_this){
    ...
    try {
        ...
        if (e.message=='Luminati unavailable')
            return;
        logger.notice('LPM is not logged in: %s', e.message);
...
E.prototype.run_cluster = function(){
    ...
    const level = this.get_logger_level(this._defaults.log, true);
    this.setup_workers(level);
...
E.prototype.recreate_worker = function(){
    ...
    this.set_logger_level(this._defaults.log, true);
...
E.prototype.api_resolve_proxies = etask._fn(function*(_this, opt={}){
    try {
        ...
        logger.debug('Resolved %s proxies from API: %s', proxies.length,
            proxies.join(', '));
        return proxies;
    } catch(e){
...
E.prototype.setup_workers = function(level){
    Object.values(cluster.workers).forEach(w=>{
        try {
            w.send({
                code: 'SETUP',
                level,
                customer: this._defaults.account_id,
            });
        } catch(e){
