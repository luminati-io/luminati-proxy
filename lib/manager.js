...
function*mgr_get_all_locations(_this, req, res){
    ...
    const data2 = yield locations.all_locations();
    ...
    res.json(Object.assign(data, {data2,
        shared_countries: shared_countries.body}));
});
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|login|howto|proxy_tester|logs|dock_logs|config|settings)|(overview|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
