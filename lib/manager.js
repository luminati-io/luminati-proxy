...
function*mgr_create_single_proxy(_this, conf){
    ...
    proxy.on('refresh_ip', data=>{
        _this.refresh_ip(data.ctx, data.ip, data.port);
    });
    if (_this.argv.cluster)
    {
...
E.prototype.create_proxy = etask._fn(
    ...
    if (proxy.proxy_type=='persist')
        _this.first_lpm_action('proxy_port', ua);
...
function*mgr_refresh_ip(_this, ctx, ip, port){
    ...
    if ((server.opt.ips||[]).includes(ip))
    {
        ...
        server.config.ips = server.opt.ips;
