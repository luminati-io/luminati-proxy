...
E.prototype.get_zones = etask._fn(function*get_zones(_this){
    ...
    return _.map(config._defaults.zones, (v, k)=>{
        return {zone: k, perm: v.perm, plans: v.plan, password: v.password[0]};
    });
});
...
function*get_lum_local_conf(_this, customer, token){
    ...
    if (!_this.lum_conf)
        _this.lum_conf = {ts: 0};
    let now = Date.now();
    if (now-_this.lum_conf.ts < 60*1000)
    ...
    if (_this.lum_conf._defaults.customer!=_this._defaults.customer ||
