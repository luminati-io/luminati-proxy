...
E.prototype.lpm_users_get = etask._fn(function*mgr_lpm_users_get(_this){
    const response = yield _this.api_request({
        url: `${_this._defaults.api}/users/lpm_users`,
        qs: {customer_name: _this._defaults.customer},
    });
...
function*mgr_lpm_users(_this, req, res){
...
E.prototype.start_auto_update = function(){
    util_lib.set_interval(etask._fn(function*mgr_start_auto_update(_this){
        if (_this.upgrading)
            return;
        const v = yield _this._last_version();
        if (v.newer && v.auto_update && v.ver!=_this.upgraded_v)
        {
            _this.upgrading = true;
            _this._upgrade(e=>{
                _this.upgrading = false;
                if (!e)
                    _this.upgraded_v = v.ver;
            });
            return;
        }
        const upgrade_idle_since = date.add(date(),
            {min: -consts.UPGRADE_IDLE_PERIOD});
        if (_this.upgraded_v && Object.values(_this.proxy_ports)
    ...
    }).bind(this), consts.UPGRADE_CHECK_INTERVAL);
};
...
E.prototype.create_api_interface = function(){
    ...
    app.post('/error', this._api(this.error_api));
...
E.prototype.error_api = etask._fn(
function*mgr_error(_this, req, res){
    const {type, backtrace, message, stack} = req.body;
    const info = Object.assign({message}, _this.get_info());
    yield zerr.perr(type, info, {backtrace: stack+
        (backtrace ? '\n\n'+backtrace : '')});
    res.send('OK');
});
