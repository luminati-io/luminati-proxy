...
function Manager(args, run_config){
    ...
    this.handle_usage = etask._fn(function*mgr_handle_usage(_this, data){
        ...
        if (!_this.lum_conf.lpm_onboarding ||
            !_this.lum_conf.lpm_onboarding.first_request && real_bw)
        {
            yield _this.update_onboarding_req({first_request: 1});
        }
    });
...
function*mgr_get_onboarding(_this, req, res){
    const lum_conf = yield _this.get_lum_local_conf();
    res.json(lum_conf.lpm_onboarding||{});
});

E.prototype.update_onboarding_req = etask._fn(
function*mgr_update_onboarding_req(_this, steps){
    const jar = _this.luminati_jar.jar;
    yield etask.nfn_apply(request, [{url: lum_url, jar}]);
    const xsrf = jar.getCookies(lum_url).find(e=>e.key=='XSRF-TOKEN').value;
    const response = yield etask.nfn_apply(request, [{
    ...
    Object.keys(steps).forEach(s=>{ _this.lum_conf.lpm_onboarding[s] = 1; });
    return response;
});

E.prototype.update_onboarding = etask._fn(
function*mgr_update_onboarding(_this, req, res){
    _this.lum_conf.lpm_onboarding = _this.lum_conf.lpm_onboarding||{};
    ...
    const response = yield _this.update_onboarding_req(steps);
    res.json(response.body);
});
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|proxies|tools|faq|setup_guide|howto|proxy_tester|stats|config|overview)|(zones|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
