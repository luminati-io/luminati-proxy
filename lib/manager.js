...
E.prototype.load_json = function(filename, def){
    let s;
    try {
    ...
    try { return JSON.parse(s); }
        ...
        console.warn(msg);
        const question = `Do you want to reset the config file and continue?`;
        const close = prompt(`${question} [y/N]`);
        if (close=='y')
            return def;
        throw msg;
    }
...
E.prototype._get_proxy_configs = function(){
    ...
    proxies = proxies.concat([].concat.apply([], _this.argv._
        .map(fname=>_this._config_from_file(fname, 'config').proxies)));
