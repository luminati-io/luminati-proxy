...
function*proxy_check(_this, new_proxy_config, old_proxy_port){
    let info = [];
    let {port, debug, zone, password, history, ssl, socks, direct,
        bypass_proxy, max_requests, session_duration, keep_alive,
        pool_size} = new_proxy_config;
    if (port!==undefined)
    {
        let server = _this.proxies_running[port];
    ...
    else if (zone!==undefined && password!==undefined && !invalid_zone)
    {
    ...
    if (direct!==undefined&&ssl!==undefined)
    {
        if ((direct&&(direct.include||direct.exclude)||bypass_proxy)&&!ssl)
        {
            info.push({
