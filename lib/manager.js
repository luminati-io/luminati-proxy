...
const credential_fields = qw`customer zone password`;
...
E.prototype._get_proxy_configs = function(args){
    ...
    if (cli_argv.port || cli_argv.socks || !proxies.length)
        proxies.push(assign({proxy_type: 'config'}, cli_argv));
    ...
    assign(_defaults, _.pick(cli_over, default_fields));
    proxies = proxies.map(c=>assign({}, c, cli_over));
...
E.prototype._api = function(f, roles){
    ...
    return (req, res, next)=>etask(function*_api(){
        ...
        res.json = o=>{
            _this._log.debug('API json resp', req.method, req.originalUrl,
                stringify(o));
            json(o);
        };
...
E.prototype._complete_proxy_config = function(conf){
    return assign({}, E.default, this._defaults, conf);
};
...
E.prototype.get_params = function(){
    ...
    if (!this.argv.config)
    {
        ...
        args.push('--customer', this._defaults.customer,
