...
const uniq_user_id = (customer, run_id)=>{
    return crypto.createHash('md5').update(`${customer}${run_id}`)
        .digest('hex');
};
...
function Manager(args, run_config){
    ...
    this.regenerate_stats = stat=>{
        ...
        top[code] = (top[code]||0)+1;
        top[protocol] = top[protocol]||{count: 0, bw: 0};
        top[protocol].count += 1;
        top[protocol].bw += stat.bw;
        let domain = stat.hostname;
        top[domain] = (top[domain]||0)+1;
        let all = this.request_stats.all = this.request_stats.all||{stats: []};
        if (all.length>1000)
        {
            let rm = all.shift();
            code = rm.status_code;
            ...
            top[code] = _.max(0, top[code]-1);
            top[protocol].count = _.max(0, top[protocol].count-1);
            top[protocol].bw = _.max(0, top[protocol].bw-rm.bw);
            domain = rm.domain;
            top[domain] -= -1;
            if (top[domain]<=0)
                delete top[domain];
    ...
    this.stats_aggregator = data=>{
        ...
        else
        {
            ...
            stat.protocol = stat.protocol.slice(0, -1);
        }
        stat.bw = data.content_size+data.request_body.length;
        ...
        if (this.argv.history||this.proxies.find(_.matches({port: data.port}))
            .history)
        {
    ...
    {
        this.uniq_uid = uniq_user_id(this._defaults.customer,
            this.run_config.id);
    }
    if (this.run_config.ua && this.uniq_uid)
        this.run_config.ua.set('userId', this.uniq_uid);
...
function _get_proxy_configs(_this, args){
    ...
    _this._log.silly('argv_explicit', _this.argv_explicit);
...
function*proxy_create_api(_this, req, res){
    ...
    {
        let ports;
        port = +_.max((ports = Object.keys(_this.proxies_running)).length &&
            ports||[23999])+1;
    }
    else
...
E.prototype.sessions_get = etask._fn(function*sessions_get(_this, req, res){
    ...
    if (server.opt.pool_size)
        sess = server.sessions&&server.sessions.sessions||[];
...
function*check_proxy_creds(_this, cred){
    ...
    try {
        ...
        {
            return cc_cache[cc_key] = {result: false,
                error: res.body.text||res.body.error};
        }
        return cc_cache[cc_key] = {result: false, error: 'invalid_creds'};
...
E.prototype.get_request_stats = function(req, res){
    ...
    res.json({[type]: results.stats && results.stats.map(
...
E.prototype.create_api_interface = function(){
    ...
    app.get('/request_stats/:type?', this._api(this.get_request_stats));
    return app;
};
...
function*create_web_interface(_this){
    ...
    app.get(/^\/((|proxies|tools|faq)|zones(\/[^\/]+)?|status_codes)$/,
        _this._api(
        (req, res, next)=>{
...
function*get_lum_local_conf(_this, customer, token, force_login){
    ...
    _this._log.silly('cookie', cookie);
    ...
    let config = yield etask.nfn_apply(request, [{
        qs: assign({customer, token}, _this.uniq_uid ? {uid: _this.uniq_uid} :
            {}),
    ...
    if (!_this.uniq_uid)
    {
        _this.uniq_uid = uniq_user_id(_this._defaults.customer,
            _this.run_config.id);
    }
    if (_this.uniq_uid&&_this.run_config.ua)
        _this.run_config.ua.set('userId', _this.uniq_uid);
    if (token)
...
function*login_user(_this, token, username, password, customer){
    ...
    try { config = yield _this.get_lum_local_conf(customer, token, true); }
        ...
        {
            return {error: {
                message: e.message||'Something went wrong. Please contact '
                    +'support.',
            }};
        }
    }
