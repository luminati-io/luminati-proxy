...
E.prototype.link_test_api = etask._fn(function*mgr_link_test(_this, req, res){
    const opt = assign(_.pick(req.query, qw`url country city region
        headers`));
    opt.port = req.params.port;
    const result = yield _this.tracer.trace(opt);
...
E.prototype.trace_api = etask._fn(function*mgr_trace(_this, req, res){
    const opt = assign(_.pick(req.body, qw`url port`), {screenshot: true});
    const result = yield _this.tracer.trace(opt);
    res.json(result);
});
...
E.prototype.start = etask._fn(function*mgr_start(_this){
    ...
    try {
        ...
        if (_this.argv.www&&cluster_mode.is_master())
        {
            ...
            _this.tracer = new Tracer(_this.wss, _this.proxies_running,
                _this.opts.log);
