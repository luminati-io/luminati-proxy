...
function*prepare_database(_this){
    ...
    _this.db.stmt.history = _this.db.db.prepare(`INSERT INTO request (port,
        ...
        status_code, timestamp, elapsed, response_time, node_latency, country,
        timeline, super_proxy, proxy_peer, username, content_size) VALUES
        ($port, $url, $method, $request_headers, $request_body,
        $response_headers, $status_code, $timestamp, $elapsed, $response_time,
...
function*create_proxy(_this, proxy, iface){
    ...
    let conf = assign({proxy_cache: _this.proxy_cache},
    ...
    if (ip)
    {
        return res.json({
            ...
            description: `Your proxy is not responding.
                <br>Please go to the
                <a href=https://luminati.io/cp/zones/${_this.argv.zone}>zone
                page</a>, and verify the following:`,
            description_list: [
                'Your credentials (customer name and password) are up to date',
                `Your IP address (${ip}) is in the whitelist`,
            ],
        });
    }
    res.json({
...
function*proxies_cache_get(_this, proxies){
    ...
    return yield _this.sql(`SELECT host FROM proxy_hosts WHERE ${where}`);
});
...
function*proxies_cache_remove(_this, $host){
    return yield _this.db.stmt.run({$host});
});
