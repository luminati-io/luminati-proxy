...
E.prototype.init = function(argv){
    try {
        ...
        this.config.save();
        this.first_actions = this.get_first_actions();
...
E.prototype.get_first_actions = function(){
...
E.prototype.save_first_actions = function(){
    fs.writeFileSync(lpm_config.first_actions, stringify(this.first_actions));
};
...
function*mgr_create_single_proxy(_this, conf){
    ...
    if (_this.argv.cluster)
        proxy = new Proxy_port(conf);
    else
        proxy = new Luminati(conf, _this);
    proxy.on('error', err=>{
    ...
    yield proxy.listen();
    _this.proxies_running[proxy.opt.listen_port] = proxy;
    return proxy;
});
...
E.prototype.proxy_delete = etask._fn(function*mgr_proxy_delete(_this, port){
    ...
    const proxy = _this.proxies_running[port] || _this.proxy_ports[port];
    if (!proxy)
        return;
    proxy.stop();
...
E.prototype.proxy_banips_api = function(req, res){
    ...
    const proxy = this.proxies_running[port] || this.proxy_ports[port];
    if (!proxy)
...
E.prototype.get_banlist_api = function(req, res){
    ...
    {
        const server = this.proxies_running[port] || this.proxy_ports[port];
        if (!server)
...
E.prototype.refresh_sessions_api = function(req, res){
    const port = req.params.port;
    if (!this.proxies_running[port] && !this.proxy_ports[port])
...
E.prototype.refresh_server_sessions = function(port){
    ...
    this.proxies_running[port].session_mgr.refresh_sessions();
};
...
function*refresh_zones(_this, req, res){
    ...
    const logged = yield _this.logged_update();
    if (logged)
...
E.prototype.creds_user_api = etask._fn(function*mgr_creds(_this, req, res){
    ...
    yield _this.first_lpm_action('login', req.get('user-agent'));
...
E.prototype.proxies_running_get_api = function(req, res){
    ...
    if (this.argv.cluster)
    {
        const proxies = Object.values(this.proxy_ports).map(p=>{
            return Object.assign({}, p.opt, {config: p.config});
    ...
    for (let p in this.proxies_running)
    {
        ...
        // XXX krzysztof: config is for UI, it has only chosen options
...
function*mgr_refresh_ip(_this, ip, port){
    const server = _this.proxies_running[port] || _this.proxy_ports[port];
...
E.prototype.has_created_proxy_port = function(){
    const ports = this.argv.cluster ? this.proxy_ports : this.proxies_running;
    return Object.keys(ports).some(p=>ports[p].opt.proxy_type=='persist');
};

E.prototype.first_lpm_action = function(action, ua){
    if (this.first_actions[action] || !this._defaults.customer)
        return;
    if (action=='send_request' && !this.first_actions.create_proxy_port)
    {
        if (this.has_created_proxy_port())
            this.first_lpm_action('create_proxy_port', ua);
        else
        {
            this.first_lpm_action('create_proxy_port_def', ua);
            this.first_actions.create_proxy_port = 1;
        }
    }
    if (action=='send_request_successful')
        this.first_lpm_action('login', ua);
    if (action=='login' && !this.first_actions.create_proxy_port &&
        this.has_created_proxy_port())
    {
        this.first_lpm_action('create_proxy_port', ua);
    }
    const browser = user_agent.guess_browser(ua).browser||null;
    this.perr(`first_${action}`, {
        customer: this._defaults.customer,
        country: this.current_country,
        docker: +!!process.env.DOCKER,
        browser,
    });
    this.first_actions[action] = 1;
    this.save_first_actions();
};
