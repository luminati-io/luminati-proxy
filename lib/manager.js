...
function*mgr_get_lum_local_conf(_this, customer, token){
    ...
    if (hash!=_this.lum_conf.hash)
        _this.update_passwords(_this.lum_conf);
    if (token)
        _this._defaults.token = token;
    return _this.lum_conf;
});

E.prototype.update_passwords = function(new_conf){
    if (!new_conf._defaults)
        return;
    this.proxies.forEach(p=>{
        let cp;
        if (cp = new_conf._defaults.zones[p.zone || this._defaults.zone])
        {
            p.password = p.password || cp.password[0];
