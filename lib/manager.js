...
tested_proxy seen_examples first_request`;
...
E.prototype.proxy_stats_get = function(req, res){
    let port = req.params.port;
    const proxy_stats = (p, idx)=>{
        let empty = {reqs: 0, in_bw: 0, out_bw: 0, port: idx};
        if (!p.stats||!p.stats.by_hsts)
            return empty;
        p = p.stats.by_hsts;
        p = Object.keys(p).reduce((acc, curr)=>{
            curr = p[curr];
            acc.reqs += curr.total_requests;
            acc.in_bw += curr.total_inbound;
            acc.out_bw += curr.total_outbound;
            return acc;
        }, empty);
        return p;
    };
    if (port && !this.proxies_running.hasOwnProperty(port))
        return res.status(404);
    if (port)
        return res.json(proxy_stats(this.proxies_running[port], port));
    return res.json(_.map(this.proxies_running, proxy_stats));
};

E.prototype.proxy_stats_reset = function(req, res){
    let port = req.params.port;
    if (port && !this.proxies_running.hasOwnProperty(port))
        return res.status(404);
    if (port)
    {
        this.proxies_running[port].stats.reset_all();
        return res.end();
    }
    _.forEach(this.proxies_running, p=>p.stats.reset_all());
    return res.end();
};
...
function*create_web_interface(_this){
    ...
    app.get(/^\/((|proxies|tools|faq|setup_guide|howto|proxy_tester)|(zones|status_codes|domains|protocols|proxy)(\/[^\/]+)?)$/,
        _this._api(
        (req, res, next)=>{
    ...
    try {
        ...
        _this.db.main.stmt = {
            ...
            history: _this.db.main.prepare(`INSERT INTO request (port, url,
                ...
                username, content_size, context, success) VALUES ($port, $url,
                $method, $request_headers, $request_body, $response_headers,
