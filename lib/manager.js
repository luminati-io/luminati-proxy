...
E.prototype.logs_reset = function(req, res){
...
E.prototype._upgrade = function(cb){
    this.emit('upgrade', cb);
};
...
function*mgr_recaptcha(_this, req, res){
    const {pageurl, googlekey, u, p, ip} = req.query;
    const key = _this._defaults['2captcha'];
    const auth = {u, p, ip};
    const token = yield get_token(pageurl, key, googlekey, _this._log, auth);
    res.send('\n'+token+'\n');
});
...
E.prototype.emit_ws_api = function(req, res){
    this.wss.broadcast({payload: req.body.payload, path: req.body.payload},
        'global');
    res.send('ok');
};
