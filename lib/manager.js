...
E.prototype.sql = etask._fn(function*sql(_this){
    ...
    args.shift();
    _this._log.debug('SQL: '+args[0], args.slice(1));
    let res = yield etask.nfn_apply(_this.db.db, '.all', args);
...
function*prepare_database(_this){
    ...
    let city_hash = yield _this.file_hash(city_path);
    let tmp_city_path = `${os.tmpdir()}/luminati_city_${city_hash}.db`;
    if (!file.exists(tmp_city_path))
        file.copy(city_path, tmp_city_path);
    yield etask.nfn_apply((fn, cb)=>_this.cities_db =
        new sqlite.Database(fn, cb), null, [tmp_city_path]);
    let now = Date.now();
