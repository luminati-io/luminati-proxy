...
E.prototype.count_logs_sql = etask._fn(function*count_logs(_this){
    this.on('uncaught', e=>_this._log.error('count_logs %s', zerr.e2s(e)));
...
E.prototype.get_size_sql = etask._fn(function*get_size_sql_mgr(_this){
    this.on('uncaught', e=>_this._log.error('get_size %s', zerr.e2s(e)));
...
E.prototype._config_from_file = function(fname, proxy_type, additional){
    const conf = this.load_json(fname, {}, additional);
    const _defaults = conf._defaults||{};
    let proxies = [];
    if (conf.proxies||conf.port)
        proxies = proxies.concat(conf.proxies||conf);
    proxies = proxies.map(c=>assign({proxy_type}, c));
    const _conf = migrate({_defaults, proxies});
    return _conf;
};
...
function*adjust_logs_mgr(_this, opt={}){
    this.on('uncaught', e=>_this._log.error('adjust_logs %s', zerr.e2s(e)));
...
E.prototype.create_api_interface = function(){
    ...
    // XXX krzysztof: decide if needed and get rid of it
...
function*mgr_create_web_interface(_this){
    ...
    app.get(/^\/((|login|(howto)(\/[^\/]+)*|tracer|(api_app.*)|proxy_tester|logs|dock_logs|config|settings)|(overview|proxy\/\d+)(\/[^\/]+)?)$/,
...
function*mgr_proxies_cache_get(_this, proxies){
    ...
    const hosts = yield _this.sql(
        `SELECT host FROM proxy_hosts WHERE ${where}`);
        ...
        if (cp = new_conf._defaults.zones[p.zone||_this._defaults.zone])
        {
            if (p.zone)
            {
                p.password = p.password||cp.password&&
                    encodeURIComponent(cp.password);
            }
            p._update = true;
...
// XXX krzysztof: what is it? remove if useless
class Rmt_lpm_cnf {
    constructor(mgr, opt = {}){
        this.opt = opt;
        this.mgr = mgr;
    ...
    fetch(){
        ...
        return etask(function*rmt_lpm_cfg_fetch(){
            ...
            try {
                ...
                _this.log.warn('fetch error %s', zerr.e2s(e));
            }
        });
    }
    get(){
