...
const proxy_fields = {
    port: 'Listening port',
    log: `Log level (${Object.keys(BaseServer.log_level).join('|')})`,
    customer: 'Customer',
    password: 'Password',
...
function Manager(args){
    ...
    this.id = this.argv.www||'MNGR';
    this.log = this.argv.log;
...
util.inherits(Manager, BaseServer);
...
function*create_socks_servers(_this){
    return yield etask.all([].concat(_this.argv.socks||[]).map(ports=>etask(
    function*(){
        ports = ports.split(':');
        const server = new Socks({local: +ports[0], remote: +ports[1],
            log: _this.argv.log});
        yield server.start();
