...
E.prototype.prepare_database = etask._fn(function*mgr_prepare_database(_this){
    ...
    try {
        const tables = yield _this.sql(`SELECT name FROM sqlite_master WHERE
            type='table'`);
        if (tables.find(t=>t.name=='schema_info'))
        {
...
function*mgr_open_browser_api(_this, req, res){
    ...
    const browser = yield puppeteer.launch({headless: false,
        ignoreHTTPSErrors: true, args: [`--proxy-server=127.0.0.1:${port}`]});
