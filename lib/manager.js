...
function*mgr_create_single_proxy(_this, conf){
    ...
    proxy.on('refresh_sessions', data=>{
        _this.refresh_server_sessions(data.port, data.worker_id);
    });
...
E.prototype.refresh_server_sessions = function(port, worker_id){
    const proxy_port = this.proxy_ports[port];
    return proxy_port.refresh_sessions(worker_id);
};
