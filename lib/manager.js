...
E.prototype.aync_req_api = etask._fn(function*async_req_api(_this, req, res){
    const port = req.params.port;
    const proxy = _this.proxies_running[port];
    if (!proxy)
    ...
    request(opt, (err, response, body)=>{
        // XXX krzysztof: to be implemented
    });
    yield this.wait();
});
...
E.prototype.create_api_interface = function(){
    ...
    app.post('/async_req/:port', this._api(this.aync_req_api));
    return app;
};
...
function*mgr_get_lum_local_conf(_this, customer, token, force_login){
    ...
    if (!_this.lum_conf)
        _this.lum_conf = {ts: 0};
    if (_this.use_local_lum_conf && !force_login)
