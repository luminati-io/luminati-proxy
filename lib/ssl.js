...
function ssl(rsa_keys, ips){
    keys = rsa_keys||keys;
    let ifaces = [].concat(...Object.values(os.networkInterfaces()));
    ips = ips||ifaces.filter(i=>i.family=='IPv4').map(i=>i.address);
    if (!keys)
    {
    ...
    const hosts = {};
    let ip_cert = gen_cert('localhost', ips.map(ip=>({type: 7, ip})));
    return {
