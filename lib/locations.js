...
const get_current_vipdb_data = mgr=>etask(function*(){
    let res = [];
    if (!mgr.server_conf || !mgr.server_conf.client.ccgi_ws)
    {
    ...
    {
        const vipdb_main = yield mgr.lpm_f.get_vipdb_main();
        res = res.concat((vipdb_main||'').split('\n'));
    }
    if (!mgr.server_conf || !mgr.server_conf.client.ccgi_ws)
    {
        const api_resp = yield mgr.api_request({
    ...
    {
        const vipdb_small = yield mgr.lpm_f.get_vipdb_small_geo();
        res = res.concat((vipdb_small||'').split('\n'));
    }
...
E.ensure_data = mgr=>etask(function*(){
    ...
    if (true || !data || data.version!=current_vipdb_version)
    {
