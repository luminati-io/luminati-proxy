...
class Proxy_port extends EventEmitter {
    constructor(opt, mgr){
        ...
        if (mgr.should_sanitize_rules(opt.rules))
        {
            let [rules, errors] = this.mgr.sanitize_rules(opt, {clone: true});
            this.opt.rules = rules;
            this.log_sanitize_errors(errors);
        }
    ...
    log_sanitize_errors(errors=[]){
        errors.forEach(([i, e])=>{
            logger.notice('Port %s rule %s deactivated before init: %s',
                this.opt.port, i+1, e.message||e);
        });
    }
    handle_message(msg){
    ...
    update_config(opt){
        ...
        let sanitized_rules, sanitize_errors;
        if (this.mgr.should_sanitize_rules(opt.rules))
        {
            [sanitized_rules, sanitize_errors] = this.mgr
                .sanitize_rules(this.opt, {clone: true});
            this.opt.rules = sanitized_rules;
            this.log_sanitize_errors(sanitize_errors);
        }
        Object.values(cluster.workers).forEach(w=>{
            this.send(w, {
                code: 'UPDATE_CONFIG',
                ...
                opt: Object.assign({}, opt, {
                    banlist: this.banlist.serialize(),
                    ...sanitized_rules && {rules: sanitized_rules},
                }),
            });
        });
        this.emit('updated');
