...
const {write_http_reply, find_iface, ensure_socket_close, get_host_port,
    req_util, res_util, Timeouts, ensure_socket_emit_close} = lutil;
...
E.prototype.is_whitelisted = function(req){
    ...
    if (auth_header)
    {
        ...
        {
            const ignore = !this.opt.zagent && this.opt.port==E.dropin.port;
...
E.prototype.store_request = function(req){
    ...
    if (req_list.has(req) || req.destroy || socket.destroyed)
        return;
...
function*send_proxy_req_ssl(_this, task, req, res, head){
    ...
    try {
        ...
        _this.logger.error('send_proxy_req_ssl error: %s', zerr.e2s(e));
        return e;
    }
});
...
function*send_proxy_req_http(_this, task, req, res, head){
    ...
    try {
        ...
        _this.logger.error('send_proxy_req_http error: %s', zerr.e2s(e));
        return e;
    }
});
