    ...
    try {
        ...
        res.once('error', e=>{
            if (e.code=='ECONNRESET')
                ...
                _this.logger.debug('Client: %s', e.message);
            return this.return();
        });
    ...
    _headers)
{
    ...
    return proxy_res=>{
        ...
        try {
            ...
            const host = req.ctx.session && req.ctx.session.host || 'no host';
            const auth = req.ctx.session && req.ctx.session.username ||
                'no username';
            this.logger.info('%s %s %s %s %s %s', req.method, ctx.url,
                proxy_res.statusCode, message, host, auth);
...
E.prototype.handle_proxy_connect = function(req, res, proxy, task, head){
    return (proxy_res, proxy_socket, proxy_head)=>{
        ...
        const auth = req.ctx.session && req.ctx.session.username ||
            'no username';
        this.logger.info('%s %s %s %s %s %s', req.method, ctx.url,
            proxy_res.statusCode, proxy_res.statusMessage, host, auth);
