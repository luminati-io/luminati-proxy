...
const {write_http_reply, url2domain, find_iface, ensure_socket_close,
    is_ws_upgrade_req, get_host_port} = lutil;
...
E.prototype.update_config = function(opt){
    ...
    this.cn_hosts = this.opt.cn_hosts||[];
...
E.prototype.init_http_server = function(opt){
    ...
    this.http_server.on('connect', (req, socket, head)=>{
        if (!opt.ssl || this.bypass_intercepting(req.url))
...
E.prototype.is_whitelisted = function(req){
    ...
    if (auth_header)
    {
        ...
        if (user=='token' && this.opt.token_auth && pass==this.opt.token_auth)
            return true;
        if (user=='lpm' && this.opt.lpm_token && pass==this.opt.lpm_token)
            return true;
        ...
        if (user.startsWith('lum-'))
        {
            ...
            // comparing only cname and zone here and letting check the
            // password on super proxy
            if (this.opt.customer==parsed_auth.customer &&
                this.opt.zone==parsed_auth.zone && parsed_auth.password)
            {
                return true;
            }
        }
...
E.prototype.handler = etask._fn(function*handler(_this, req, res, head){
    ...
    try {
        ...
        _this.logger.warn('handler: %s %s %s', req.method, req.url, e.message);
        throw e;
    }
});
    ...
    _headers)
{
    ...
    return proxy_res=>{
        ...
        ctx.responded = true;
        const count$ = E.create_count_stream(ctx.response, this.opt.har_limit);
        try {
...
E.prototype.handle_proxy_connect = function(req, res, proxy, task, head){
    return (proxy_res, proxy_socket, proxy_head)=>{
        ...
        const resp_counter = E.create_count_stream(ctx.response,
            this.opt.har_limit);
