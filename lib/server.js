...
E.prototype.log_req = function(response, _url, hostname, in_bw, out_bw, sp,
    user, password, auth_type, status_code, status_message)
{
    ...
    this.logger.info('%s %s %s %s %s %s %s %s', response.request.method,
        _url, status_code, status_message, sp||'no_proxy', bw, auth,
        auth_type, debug_info);
    let message = {
        ts: date(),
...
E.prototype.usage = function(response){
    ...
    const auth_type = response.lpm_auth_type;
