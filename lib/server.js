...
const Http2_handler = require('./http2_handler.js');
...
E.prototype.init_http_server = function(opt){
    ...
    this.http_server.on('connect', (req, socket, head)=>{
        ...
        if (this.opt.http2)
            this.http2_handler.emit('connection', socket);
        else
...
E.prototype.init_http2_handler = function(){
    this.authorization = {};
    this.req_remote_ip = {};
    this.http2_handler = new Http2_handler(this);
};
...
E.prototype.init_tcp_server = function(){
    this.tcp_server = new net.createServer(socket=>{
        ...
        socket.once('data', data=>{
            ...
            const https_server = this.opt.http2 ? this.http2_handler :
                this.https_server;
            if (protocol_byte==22 && https_server)
...
E.prototype.handler = etask._fn(function*handler(_this, req, res, head){
    ...
    try {
        ...
        _this.logger.error('unexpected error in handler: %s', zerr.e2s(e));
        throw e;
    }
});
...
E.prototype.send_error = function(req, res, err, err_origin){
    ...
    this.logger.info('%s %s 502 %s', req.method, req.url, message);
    if (res.ended)
        return;
...
E.prototype.stop = etask._fn(function*stop(_this){
    try {
        ...
        if (_this.http2_handler)
            _this.http2_handler.stop();
...
E.prototype.send_proxy_req = function(task, req, res, head, host){
    if (req.ctx.is_http2)
        return this.http2_handler.send_proxy_req(task, req, res, head, host);
    if (req.ctx.is_ssl)
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        const proxy_req = https.request(proxy_opt);
        task.once('cancel', ()=>proxy_req.end());
        proxy_req.host = host;
        ctx.proxies.push(proxy_req);
        if (ctx.response.request.body)
            proxy_req.write(ctx.response.request.body);
        req.pipe(proxy_req);
        const cb = ()=>proxy_req.end();
        ctx.end_listeners.push(cb);
        req.once('end', cb);
        return yield _this.request_handler(req, res, proxy_req, head,
            conn && conn.res && conn.res.headers);
    } catch(e){
        return e;
    }
});
...
E.prototype.handle_proxy_resp = function(req, res, proxy_req, task, head,
    _headers)
{
    return proxy_res=>{
        const ctx = req.ctx;
        if (ctx.is_http2)
            proxy_res = this.http2_handler.to_http1_res(proxy_req, proxy_res);
        ...
        if (ctx.responded)
            return this.abort_proxy_req(req, proxy_req, task);
        ctx.proxies.forEach(p=>p!=proxy_req && this.abort_proxy_req(req, p));
        ctx.responded = true;
        const count$ = create_count_stream(ctx.response);
        try {
            ctx.timeline.track('response');
            this.check_proxy_response(ctx.host, proxy_res);
            const message = ctx.is_http2 ? '(HTTP2)' :
                proxy_res.headers['x-luminati-error'] ||
            ...
            if (!res.resp_written)
            {
                ...
                if (this.rules.post(req, res, head, proxy_res))
                    return this.abort_proxy_req(req, proxy_req);
            ...
            if (ctx.is_http2)
                proxy_req.pipe(count$).pipe(req.stream);
            else
                proxy_res.pipe(count$).pipe(res);
            proxy_res.once('end', end_task)
            .once('error', this.log_throw_fn(task));
        } catch(e){
            task.throw(e);
        }
...
E.prototype.abort_proxy_req = function(req, proxy_req, task){
    req.unpipe(proxy_req);
    if (!req.ctx.is_http2)
        proxy_req.abort();
    proxy_req.destroy();
    if (task)
...
E.prototype.handle_proxy_error = function(req, res, proxy_req, task, head){
    return e=>{
        const ctx = req.ctx;
        if (proxy_req.aborted || ctx.responded || ctx.connected)
            return;
        ...
        {
            this.logger.warn('error proxy response %s, retrying', ctx.host);
            this.rules.retry(req, res, head);
            this.abort_proxy_req(req, proxy_req);
            return;
        }
        this.abort_proxy_req(req, proxy_req);
        this.usage_abort(req);
        task.throw(proxy_err||e);
    };
};
