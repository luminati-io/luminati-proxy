...
E.prototype.update_config = function(opt){
    ...
    if (this.opt.proxy_connection_type == 'socks')
        this.opt.proxy_port = this.opt.socks_proxy_port;
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        if (etask.is_final(task))
            conn.socket.end();
        if (_this.opt.session_termination && conn.res &&
            conn.res.statusCode==502 &&
            conn.res.statusMessage==consts.NO_PEERS_ERROR_SSL)
        {
            return _this.handle_session_termination(req, res);
        }
        if (conn.res && conn.res.statusCode!=200)
        {
        ...
        const tl = (conn.res && conn.res.headers['x-hola-timeline-debug']||'')
            .split(' ');
    ...
    _headers)
{
    return proxy_res=>{
        ...
        ctx.init_res_stats(proxy_res, proxy_res);
        if (ctx.responded)
...
E.prototype.handle_proxy_connect = function(req, res, proxy, task, head){
    return (proxy_res, socket, proxy_head)=>{
        ...
        if (ctx.connected)
            return this.abort_proxy_req(req, proxy);
        ctx.init_res_stats(proxy_res, socket);
        ctx.proxies.forEach(p=>p!=proxy && this.abort_proxy_req(req, p));
        ctx.connected = true;
        logger.info('%s %s %s %s', req.method, ctx.url,
