...
const handle_custom_error = (e, req, res, ctx)=>{
    if (!is_custom_error(e))
        return;
    if (e.message == 'Authentication failed')
    {
        logger.info('%s %s 502 %s', req.method, ctx.url, e.message);
...
E.prototype.init_https_server = function(){
    ...
    this.https_server = https.createServer(
        Object.assign({requestCert: false}, ssl()), (req, res, head)=>{
            ...
            {
                logger.debug('request ip fixed %s %s', remote_ip, req.url);
    ...
    this.http_server.on('connect', (req, socket, head)=>{
        ...
        socket.once('error', e=>{
            ...
            logger.error('https socket: %s', zerr.e2s(e));
        });
        socket.once('timeout', ()=>{
            this.timeout_ids.push(ensure_socket_close(socket));
        });
...
E.prototype.init_tcp_server = function(){
    this.tcp_server = new net.createServer(socket=>{
        ...
        socket.once('data', data=>{
            ...
            // first part is not [SMTP->HTTP] and SMTP server is on
            if (protocol_byte!=91 && this.opt.smtp && this.opt.smtp.length)
...
E.prototype.usage = function(response){
    if (!response)
        return this.perr(new Error('invalid_response'));
    if (!response.timeline)
        response.timeline = new Timeline(this.port);
...
E.prototype.handler = etask._fn(function*handler(_this, req, res, head){
    ...
    res.once('close', ()=>setTimeout(()=>{
        this.return();
    }));
    req.once('close', ()=>setTimeout(()=>{
        this.return();
    }));
    req.once('finish', ()=>setTimeout(()=>{
        this.return();
    }));
    try {
        ...
        res.once('error', e=>{
            ...
            logger.warn('client: %s', e.message);
        });
        ...
        logger.error('unexpected error in handler: %s', zerr.e2s(e));
        throw e;
    }
});
...
E.prototype.send_error = function(req, res, err, err_origin){
    ...
    logger.info('%s %s 502 %s', req.method, req.url, message);
    if (res.ended)
        return;
    ...
    try {
        ...
        logger.error('could not send head: %s\n%s', e.message);
    }
};
...
E.prototype.complete_req = function(err, req, res){
    try {
        ...
        logger.error('unexpected error: %s', zerr.e2s(e));
    }
};
...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
        ...
        Date.now()-timestamp<30*SEC)
    {
        ...
        try {
            ...
            logger.warn('Failed to resolve %s (%s): %s', proxy, domain,
                e.message);
        }
    }
    _this.hosts = _.shuffle(Object.keys(hosts));
    if (!_this.hosts.length)
        logger.error('Failed to resolve any proxies');
...
E.prototype.init_proxy_req = function(req, res){
    ...
    logger.debug('requesting using %s', ctx.cred.username);
    ctx.response.proxy = {
        host: ctx.host,
...
function*lpm_request(_this, req, res, head){
    ...
    try {
        ...
        if (ctx.is_connect && parse_ip_url(ctx.url))
            logger.warn(`HTTPS to IP: %s is sent from super proxy`, ctx.url);
        ...
        _this.emit('response', resp);
        if (handle_custom_error(e, req, res, ctx))
...
E.prototype.route_req = etask._fn(function*route_req(_this, req, res, head){
    try {
        logger.debug('%s:%s - %s %s', req.socket.remoteAddress,
...
function*request_handler(_this, req, res, proxy, head, headers){
    ...
    const ensure_end_task = ()=>_this.timeout_ids.push(setTimeout(()=>{
        if (etask.is_final(this))
            return;
        logger.debug('closing long connection after 15 seconds');
...
E.prototype.get_reused_socket = function(ctx){
    const agent_name = this.https_agent.getName({
        servername: ctx.domain,
        port: 443,
        rejectUnauthorized: !this.opt.insecure,
        lpm_username: ctx.cred.username,
    });
    if (this.https_agent.freeSockets[agent_name])
    {
        logger.debug('reusing socket: %s %s', ctx.domain, ctx.cred.username);
        return this.https_agent.freeSockets[agent_name][0];
    }
};
...
function*_request_new_socket(_this, task, req, res, head, host){
    const ctx = req.ctx;
    task.once('cancel', ()=>this.return());
    const conn = yield _this.requester.request_socket(ctx, host, task, {
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        const socket = _this.get_reused_socket(ctx);
        const conn = socket ? {socket}
            : yield _this.request_new_socket(task, req, res, head, host);
        if (!conn.socket)
            return conn;
...
E.prototype.handle_proxy_timeout = function(req, res, proxy, task){
    return ()=>{
        const ctx = req.ctx;
        this.timeout_ids.push(ensure_socket_close(proxy));
        logger.debug('socket inactivity timeout: %s', ctx.url);
        task.return();
    };
};
    ...
    _headers)
{
    return proxy_res=>{
        ...
        try {
            ...
            logger.info('%s %s %s %s', req.method, ctx.url,
            ...
            {
                logger.debug(`ip ${ip} is banned, retrying w/ new session id`);
    ...
    return (proxy_res, socket, proxy_head)=>{
        ...
        try {
            ...
            socket.once('error', err=>{
                logger.warn('error on socket: %s', err.message);
            }).once('end', ()=>{
...
E.prototype.handle_proxy_error = function(req, res, proxy, task, head){
    return err=>{
        ...
        {
            logger.warn('error proxy response %s, retrying', ctx.host);
...
E.prototype.unbanip = function(ip){
