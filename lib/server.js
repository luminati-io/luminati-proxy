...
const {find_iface, ensure_socket_close, req_util, Timeouts, REDIRECT_PARTS,
    ensure_socket_emit_close, is_redirect_status, req_is_ssl} = lutil;
...
const auth2h = (fields=[], req={}, auth={})=>{
    if (_.isEmpty(auth) || !req.headers)
        return;
    for (let p of fields)
    {
        if (!req.headers['x-lpm-'+p] && auth[p])
            req.headers['x-lpm-'+p] = auth[p];
    }
};
...
E.prototype.perr = function(id, info){
    ...
    this.logger.info('sending perr: %s', JSON.stringify({id, ...info}));
    lutil.perr(id, info);
};
