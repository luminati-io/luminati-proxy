...
E.prototype.is_whitelisted = function(req){
    ...
    if (auth_header)
    {
                ...
                lpm_user_opt.lpm_user==this.opt.user))
        {
            ...
            if (lpm_user_opt)
            {
                for (let param of ['session', 'country', 'state', 'city'])
                {
                    if (!req.headers['x-lpm-'+param] && lpm_user_opt[param])
                        req.headers['x-lpm-'+param] = lpm_user_opt[param];
                }
            }
...
E.prototype.get_next_host = function(){
    if (!this.hosts.length)
        throw new Error('No hosts available');
    if (!this.hosts[hosts_cursor])
        hosts_cursor = 0;
    return this.hosts[hosts_cursor++];
};
