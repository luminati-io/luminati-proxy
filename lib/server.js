...
E.prototype.update_config = function(opt){
    ...
    this.rules = new Rules(this, opt.rules, {
        av_server_url: opt.av_check && opt.av_server_url,
        ssl: opt.ssl,
    });
...
E.prototype.init_tcp_server = function(){
    this.tcp_server = new net.createServer(socket=>{
        ...
        if (this.lb_ips.has(socket.remoteAddress))
        {
            lb_transform_stream = lutil.Lb_transform.init(socket, {
                on_parsed: is_smtp_req && (()=>{
...
const get_hostname = _url=>{
    ...
    if (url_parts = _url.match(/^([^/]+?):(\d+)$/))
        return url_parts[1];
    return zurl.parse(_url).hostname;
};
...
E.prototype.usage = function(response){
    ...
    {
        const {protocol, hostname: hn} = zurl.parse(data.url);
...
E.prototype.log_failed_auth = function(req, type='', meta={}){
    if (!this.opt.zagent)
        return;
    const ip = this.get_req_remote_ip(req);
    const whitelisted = !!ip && this.wl_ips.contains(ip);
    const url = String(req.url||'').replace(/\s+/g, ' ').slice(0, 300);
    this.logger.warn(
        'AUTH_FAILED ip=%s whitelisted=%s type=%s method=%s url=%s meta=%s',
        ip||'-', whitelisted, type, req.method||'-', url,
        stringify(meta)
    );
};
    ...
    if (auth_header)
    {
        ...
        if (consts.USERNAME_PREFS.some(p=>user.startsWith(p+'-')))
        {
            ...
            if (customer && zone && password)
            {
                ...
                const zone_auth_passthru = this.opt.zone_auth_passthru;
                if (!zone_auth_passthru && (!is_cust || !is_zone) || !in_wl)
                {
                    this.log_failed_auth(req, 'zone', {
                        customer,
                        zone,
                        is_cust,
                        is_zone,
            ...
            if (pauth.auth=='token' && password)
            {
                ...
                if (!success)
                {
                    this.log_failed_auth(req, 'lum_token', {
                        password_len: String(password).length,
                        token_auth_len: (this.opt.token_auth||'').length,
                        lpm_token_len: lpm_token.length,
                    });
                    return false;
                }
                req.lpm_auth_type = 'lum_token';
                return true;
            }
        }
    }
...
E.prototype.check_proxy_response = function(res){
    const message = res.headers && res.headers['x-brd-error'];
    if (!message)
        return false;
    const err = new Error();
    err.message = message;
...
E.prototype.update_req_redirect = function(req, location){
    ...
    let new_url = new URL(safe_location, prev_url);
...
const ignore_perr_lookup = zutil.bool_lookup(['conn_unknown',
    ...
    'conn_flex_tls_use_destroyed_socket', 'conn_flex_tls_bad_decrypt',
...
E.prototype.log_fn = function(e, ctx, source){
    ...
    else if (msg.startsWith('flex_tls_use_destroyed_socket'))
        perr_id = 'conn_flex_tls_use_destroyed_socket';
...
const validate_reuse_candidate = (socket, use_flex_tls)=>{
    const underlying = use_flex_tls ? socket && socket.socket : socket;
    return socket && !socket.destroyed && !socket.writableEnded &&
        underlying && !underlying.destroyed &&
        underlying.writable !== false && underlying.readable !== false;
};

E.prototype.get_reused_conn = function(ctx){
    const socket_name = ctx.get_socket_name();
    const pool = this.https_agent.freeSockets[socket_name];
    if (!pool || !pool.length)
        return;
    let socket;
    for (let i = pool.length - 1; i >= 0; i--)
    {
        ...
        if (!validate_reuse_candidate(candidate, this.opt.use_flex_tls))
        {
            try { candidate && candidate.destroy(); } catch(e){}
            continue;
        }
        socket = candidate;
        break;
    ...
    if (!socket)
        return;
    this.logger.debug('reusing socket: %s %s', ctx.domain, ctx.cred.username);
    return {socket, res: {
        headers: Object.assign({}, this.socket2headers.get(socket_name)),
    }};
};
...
function*_request_new_socket(_this, task, req, res, head){
    ...
    const ip = conn.res.headers['x-brd-ip'];
...
E.hola_headers = qw`proxy-connection proxy-authentication x-hola-agent
    x-hola-context x-brd-timeline x-brd-peer-timeline
    x-brd-error x-lpm-error x-lpm-authorization x-lpm-user x-brd-ip`;
