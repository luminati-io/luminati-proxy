...
function logger(id, conf, disable_color){
    try {
        const formatter = options=>options.timestamp()+' '
            +(!disable_color&&options.colorize
        ...
        // XXX ofir/ovidiu: add ga_event
        let error, warn, info, verbose, debug;
