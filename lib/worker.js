...
} catch(e){ logger.info('cluster_cache not available'); }
...
class Worker {
    ...
    restore_lpm_headers(req){
        ...
        if (!lpm_headers)
            return;
        Object.keys(lpm_headers).forEach(h=>req.headers[h] = lpm_headers[h]);
    }
    preserve_lpm_headers(req, socket){
        const port = socket.remotePort;
        Object.entries(req.headers).forEach((h, v)=>{
            if (!h?.startsWith?.('x-lpm-'))
                return;
            this.lpm_headers[port] = this.lpm_headers[port] || {};
            this.lpm_headers[port][h] = v;
        });
    }
