...
const Lpm_f = etask._class(class Lpm_f extends events.EventEmitter {
    ...
    *init(_this){
        _this.uri_ws = `wss://zagent75.${_this.mgr._defaults.api_domain}:443`;
        _this.ws = new zws.Client(_this.uri_ws, {
            ...
            ipc_client: {
                ...
                get_vipdb_main: {type: 'call', timeout: 30*date.ms.SEC},
                get_vipdb_small_geo: {type: 'call', timeout: 30*date.ms.SEC},
    ...
    *get_shared_block_cn(_this){
        if (!_this.connected())
            throw new Error('no_lpm_f_conn');
        const resp = yield _this.ws.ipc.get_shared_block_cn();
        if (resp && resp.err)
            throw new Error(resp.err);
        return resp;
    }
    *handler(_this, method){
        if (!_this.connected())
            throw new Error('no_lpm_f_conn');
        const resp = yield _this.ws.ipc[method]();
        if (resp && resp.err)
            throw new Error(resp.err);
        return resp;
    }
    ...
    *get_vipdb_main(_this){
        this.on('uncaught', e=>{
            logger.error('resolve_proxies %s', zerr.e2s(e));
        });
        if (!_this.connected())
            throw new Error('no_lpm_f_conn');
        const stream = _this.ws.mux.open(0, undefined,
            {use_ack: true});
        yield _this.ws.ipc.get_vipdb_main();
        const data = yield stream2data(stream);
        return data.toString();
    }
    *get_vipdb_small_geo(_this){
        this.on('uncaught', e=>{
            logger.error('resolve_proxies %s', zerr.e2s(e));
        });
        if (!_this.connected())
            throw new Error('no_lpm_f_conn');
        const stream = _this.ws.mux.open(1, undefined,
            {use_ack: true});
        yield _this.ws.ipc.get_vipdb_small_geo();
        const data = yield stream2data(stream);
        return data.toString();
    }
...
const stream2data = (stream, opt)=>etask(function*(){
    ...
    this.alarm(opt.timeout||60*date.ms.SEC,
