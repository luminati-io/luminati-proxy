...
class Rules {
    constructor(luminati, rules){
        ...
        this._post = (this.rules.post||[]).map(p=>{
            ...
            p.need_body = p.body || p.action && p.action.process;
            if (_.get(p, 'action.reserve_session'))
                p.pool_key = 'reserve_session';
            if (_.get(p, 'action.retry_port') && !p.action.executed)
                p.action.executed = [];
            return p;
        });
    ...
    remove_executed(req){
        this._post.forEach(p=>{
            if (!_.get(p, 'action.executed'))
                return;
            _.remove(p.action.executed, i=>i==req.ctx.uuid);
        });
    }
    post_executed(req, post){
        if (!post.action)
            return;
        return (post.action.executed||[]).some(i=>i==req.ctx.uuid);
    }
    post(req, res, head, _res){
        ...
        for (let i=0; i<this._post.length; i++)
        {
            ...
            if (!p.trigger.test(_url, ctx) || this.post_executed(req, p))
                continue;
            if (this.check_req_time_range(req, p)&&_res)
            {
                if (this.action(req, res, head, _res, p.action, p,
                    {type: p.trigger_type, value: p[p.trigger_type]}))
                {
                    return true;
                }
            }
            ...
            if (this.cmp(p.status, _res.statusCode || _res.status_code)
                || this.cmp(p.header, _res.headers))
            {
                if (this.action(req, res, head, _res, p.action, p,
                    {type: 'status code', value: p.status.arg}))
                {
                    return true;
                }
            }
    ...
    post_body(req, res, head, _res, body){
        ...
        {
            let p = this._post[i];
            if (!p.need_body || !p.trigger.test(_url, ctx))
                continue;
            if (this.cmp(p.status, _res.statusCode))
            {
                if (this.action(req, res, head, _res, p.action, p,
            ...
            {
                if (this.action(req, res, head, _res, p.action, p,
                    {type: 'HTML body element', value: p.body.arg}))
                {
                    return true;
                }
            }
    ...
    retry(req, res, head, port, trigger){
        ...
        if (!serv)
            serv = this.luminati;
        req.ctx.rules_chain.push(req.ctx.rules);
    ...
    action(req, res, head, _res, action, rule, trigger){
        ...
        if (action.retry_port)
            action.executed.push(req.ctx.uuid);
        return true;
    }
}
