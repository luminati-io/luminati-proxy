...
const {init_captcha_request, get_token} = require('./recaptcha.js');
...
class Rules {
    ...
    action(req, res, head, rule, opt){
        ...
        if (rule.action.solve_captcha)
            return this.solve_captcha(req, res, head, opt);
    ...
    solve_captcha(req, _res, head, opt){
        if (req.recaptcha_progress)
        {
            req.recaptcha_progress.continue(opt);
            return 'switched';
        }
        const $ = cheerio.load(opt.body, {xml: {normalizeWhitespace: true}});
        const googlekey = $('[data-sitekey]').attr('data-sitekey');
        const ctx = req.ctx;
        if (googlekey && ctx.retry)
            ctx.log.warn('captcha was not solved successfully');
        if (ctx.retry || !googlekey)
            return false;
        ctx.log.warn('captcha detected');
        const key = this.luminati.mgr._defaults['2captcha'];
        const _this = this;
        etask(function*(){
            const token = yield get_token(ctx.url, key, googlekey, ctx.log);
            yield init_captcha_request(_this, token, req, _res, head);
        });
        return 'switched';
    }
}
