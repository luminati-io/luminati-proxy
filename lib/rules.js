...
class Rules {
    ...
    action(req, res, head, rule, opt){
        ...
        if (rule.action.ban_ip_domain && _res)
        {
            const t = rule.action.ban_ip_domain||0;
            const domain = url2domain(opt.url);
            if (tl)
                this.luminati.banlist.add(tl.split(' ')[3], t, domain);
            req.session = this.gen_session();
        }
        ...
        if (rule.action.solve_captcha)
            return this.solve_captcha(req, res, head, opt.body);
    ...
    solve_captcha(req, _res, head, body){
        const $ = cheerio.load(body, {xml: {normalizeWhitespace: true}});
        const googlekey = $('[data-sitekey]').attr('data-sitekey');
        const ctx = req.ctx;
        if (ctx.retry || !googlekey)
            return false;
        ctx.log.notice('captcha detected');
        const api = 'http://2captcha.com/';
        const key = this.luminati.mgr._defaults['2captcha'];
        const method = 'userrecaptcha';
        const pageurl = ctx.url;
        const qs = {method, pageurl, key, googlekey, json: 1};
        const _this = this;
        const get_code = (_qs, retry=0)=>etask(function*(){
            ctx.log.notice('trying to get code, retry: %s', retry);
            ...
            ctx.log.notice('resp is %s', resp.body.request);
            const code = resp.body.request;
            if (code!='CAPCHA_NOT_READY')
                return code;
            yield etask.sleep(5000);
            return yield get_code(_qs, retry+1);
        });
        etask(function*(){
            ...
            ctx.log.notice('catpcha id: %s', id);
            yield etask.sleep(10000);
            const _qs = {key, action: 'get', id, json: 1};
            const code = yield get_code(_qs);
            const post_data = 'g-recaptcha-response='+code;
            ctx.force_body = post_data;
            req.headers['Content-Length'] = Buffer.byteLength(post_data);
            req.headers['Content-Type'] = 'application/x-www-form-urlencoded';
            req.method = 'POST';
            _this.retry(req, _res, head);
        });
        return 'switched';
    }
}
