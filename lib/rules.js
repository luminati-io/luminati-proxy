...
class Rules {
    constructor(luminati, rules){
        ...
        this._post = (this.rules.post||[]).map(p=>{
            p.url_cond = new Url_condition(p.url);
    ...
    return etask(function*rules_pre(){
        ...
        for (let i=0; i<_this._pre.length; i++)
        {
            ...
            else if (!p.url_cond.test(_url, ctx))
                continue;
            if (p.session)
            ...
            for (let j=0; j<p.res.length; j++)
            {
                ...
                if (this.check_req_time_range(req, r)&&_res)
                {
                    if (this.action(req, res, head, _res, pr_action, p,
                        {type: r.trigger_type, value: r[r.trigger_type]}))
                    {
                        return true;
                    }
                }
                ...
                if (this.cmp(r.status, _res.statusCode||_res.status_code)
    ...
    post_body(req, res, head, _res, body){
        ...
        {
            let p = this._post[i];
            if (!p.need_body || !p.url_cond.test(_url, ctx))
                continue;
            for (let j=0; j<p.res.length; j++)
            {
                const r = p.res[j], pr_action = r.action||p.action;
...
class Url_condition {
    constructor(url){
        if (!url || typeof url=='string')
            this.regexp = url2regexp(url);
        else if (url.code)
            this.code = url.code;
        else if (url.regexp)
            this.regexp = url2regexp(url.regexp);
        else
