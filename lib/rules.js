...
const {migrate_trigger, get_timeout_value} = require('../util/rules_util.js');
...
const {coercers, is_number, decode_body, url2domain} = require('./util.js');

class Rules {
    constructor(server, rules=[], opt={
        av_server_url: '',
        ssl: false,
    })
    {
        ...
        this.server = server;
        this.ssl = opt.ssl;
        const av_server_url = opt.av_server_url;
        if (av_server_url)
        {
            this.av_client = new zws.Client(`ws://${av_server_url}`, {
                label: `av_client`,
                ipc_client: {scan: {type: 'call', timeout: 10*date.ms.SEC}},
            });
        }
    ...
    *pre(req, res, head){
        ...
        for (let i=0; i<this.rules.length; i++)
        {
            ...
            rule.url = rule.url||'';
            if (rule.domain && opt.url)
                opt.domain = this.ssl ? zurl.get_host(opt.url) : opt.url;
            if (!rule.trigger.test(opt) && !this.try_from_cache(rule, opt.url))
                continue;
            const {min_req_time, min_conn_time} = rule;
            if (rule.type=='timeout' && is_number(min_req_time))
            {
                const _this = this;
                req.min_req_task = etask(function*_min_req_time(){
                    const passed = req.ctx.timeline.passed('min_req_task', 0);
                    if (!req.ctx.n_redirects || !passed)
                        req.ctx.timeline.track('min_req_task');
                    const timeout = passed ? Math.max(0, min_req_time-passed)
                        : min_req_time;
            ...
            else if (rule.type=='timeout' && is_number(min_conn_time))
            {
                const _this = this, ctx = req.ctx;
                req.min_conn_task = etask(function*_min_conn_time(){
                    const passed = ctx.timeline.passed('min_conn_task', 0);
                    if (ctx.n_redirects && !passed)
                        ctx.timeline.track('min_conn_task');
                    const timeout = passed ? Math.max(0, min_conn_time-passed)
                        : min_conn_time;
                    yield etask.sleep(timeout);
                    yield _this.action(req, res, head, rule, opt);
                });
    ...
    *post(req, res, head, _res){
        ...
        for (let i=0; i<this.rules.length; i++)
        {
            ...
            const time_passed = Date.now()-req.ctx?.timeline?.req?.create||0;
            if (rule.type!='after_hdr')
                continue;
            const opt = {url: req.ctx.url, status, time_passed, post: 1,
    ...
    *post_body(req, res, head, _res, body){
        ...
        if (this.av_client && this.av_client.connected)
        {
            try {
                let scan_result = yield this.av_client.ipc.scan({file: _body});
                if (['infected', 'suspicious']
                    .includes(scan_result.result||''))
                {
                    return yield this.action(req, res, head,
                        {action: {malware: 1}}, {_res, malware: scan_result});
                }
            } catch(e){ logger.error('scan_err: %s', e.message); }
        }
        for (let i=0; i<this.rules.length; i++)
        {
            const rule = this.rules[i];
            const should_skip = req.ctx.skip_rule(rule)
                || rule.type!='after_body' && !rule.action.cache;
            if (rule.active===false || should_skip)
                continue;
            const status = _res.statusCode||_res.status_code;
            const time_passed = Date.now()-req.ctx.timeline.req.create;
            const opt = {url: req.ctx.url, status, body: _body, time_passed,
                post_body: 1, res_data: body, sent_reqs: req.ctx.retry+1};
    ...
    post_need_body(req, proxy_res){
        return this.av_client && this.av_client.connected ||
            this.will_cache(req, proxy_res) ||
            this.rules.some(rule=>rule.type=='after_body' &&
                rule.active!==false);
    }
    ...
    can_retry(req, action={}){
        ...
        let retry = Number(coercers.string.boolean(action.retry))||0;
        if (retry>3)
            retry = 3;
        if (action.retry_port)
            retry = 20;
    ...
    *action(req, res, head, rule, opt){
        ...
        if (req.ctx && req.ctx.session)
        {
            ...
            const rh = opt._res && opt._res.headers;
            req.ctx.session.last_res.ip = rh && rh['x-brd-ip'];
        }
        if (req.ctx && req.ctx.rule_executed)
            req.ctx.rule_executed(rule);
        if (rule.action.malware)
        {
            req.ctx.is_malware = true;
            req.ctx.init_stats();
            return this.server.router.send_malware_detected_response(req, res,
                opt.malware);
        }
        if (rule.action.timeout_response)
        {
            req.ctx.is_timeout_response = true;
            req.ctx.init_stats();
            return this.server.router.send_timeout_response(req, res,
                get_timeout_value(rule));
        }
        ...
        const _res = opt._res;
        const hdr = _res && _res.hola_headers || _res && _res.headers;
        const ip = hdr && hdr['x-brd-ip'];
        const vip = hdr && hdr['x-brd-gip'];
        ...
        // leaves can_retry legacy logic untouched
        if (rule.trigger_type==='min_conn_time' && rule.action_type==='retry')
        {
            if ((req.retry||0) >= Math.min(rule.action.retry, 5))
                return false;
        }
        else if (!this.can_retry(req, rule.action))
            return false;
    ...
    dispose(){
        if (this.av_client)
        {
            this.av_client.close();
            delete this.av_client;
        }
    }
}
...
class Trigger {
    ...
    dispose(){
        logger.debug('trigger: disposing');
    }
}
