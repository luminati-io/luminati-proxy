...
class Rules {
    constructor(server, rules=[], av_server_url='', ssl=false){
        this.id = Math.random();
        this.server = server;
        this.ssl = ssl;
        if (av_server_url)
    ...
    *pre(req, res, head){
        ...
        for (let i=0; i<this.rules.length; i++)
        {
            ...
            if (!rule.trigger.test(opt) && !this.try_from_cache(rule, opt.url))
                continue;
            if (rule.type=='timeout' && opt.timeout && rule.min_req_time)
            {
                const _this = this;
                req.min_req_task = etask(function*min_req_time(){
                    ...
                    const timeout = passed ? Math.max(0, opt.timeout-passed)
                        : opt.timeout;
            ...
            else if (rule.type=='timeout' && opt.timeout && rule.min_conn_time)
            {
                const _this = this, ctx = req.ctx;
                req.min_conn_task = etask(function*min_conn_time(){
    ...
    *post_body(req, res, head, _res, body){
    ...
    can_retry(req, action={}){
        ...
        let retry = Number(lutil.coercers.string.boolean(action.retry))||0;
        if (retry>3)
            retry = 3;
        if (action.retry_port)
            retry = 20;
    ...
    *action(req, res, head, rule, opt){
        ...
        if (req.ctx && req.ctx.session)
        {
            ...
            req.ctx.session.last_res.ip = rh &&
                (rh['x-luminati-ip']||rh['x-brd-ip']);
        }
        ...
        const ip = hdr && (hdr['x-luminati-ip']||hdr['x-brd-ip']);
        const vip = hdr && (hdr['x-luminati-gip']||hdr['x-brd-gip']);
