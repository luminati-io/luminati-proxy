...
const all_migrations = {
    ...
    '1.143.166': conf=>{
        ...
        const proxies = conf.proxies.map(p=>{
            p = Object.assign({}, p);
            (p.whitelist_ips||[]).forEach(x=>whitelist_ips.add(x));
            delete p.whitelist_ips;
            return p;
        });
        conf._defaults = conf._defaults||{};
        (conf._defaults.whitelist_ips||[]).forEach(x=>whitelist_ips.add(x));
        conf._defaults.whitelist_ips = [...whitelist_ips];
        if (!conf._defaults.whitelist_ips.length)
            delete conf._defaults.whitelist_ips;
        return Object.assign({}, conf, {proxies});
    },
    '1.148.122': conf=>{
        conf.proxies = conf.proxies||[];
