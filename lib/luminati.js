...
Router.prototype.send_null_response = function(_url, req, res, timeline, response){
    this._log.debug(`${req.req_id}: Returning null response: ${req.method}`
        +`${_url}`);
...
Router.prototype.is_bypass_proxy = function bypass_proxy(_url, _ssl){
...
const lpm_headers = qw`x-hola-context proxy-authorization x-hola-agent
    x-lpm-src-addr x-lpm-session x-hola-timeline-debug`;
...
Request_ctx.prototype.request_start = function(retry){
    this.retry = this.retry+1;
    this.id = this.gen_id(this.id, this.retry);
    if (this.retry)
        this.timeline.retry();
    this.url = this.req.url;
    this.url_full = req_util.full_url(this.req);
    this.process_headers();
    this.src_addr = req.connection.remoteAddress;
    if (this.src_addr==loopback_ip&&this.h_src_addr)
        this.src_addr = this.h_src_addr;
};

Request_ctx.prototype.process_headers = function(){
...
function Session_manager(lum, opt){
    events.EventEmitter.call(this);
    this.opt = opt;
    this._log = lum._log;
    this.lum = lum;
...
util.inherits(Session_manager, events.EventEmitter);

Session_manager.prototype.calculate_username = function(opt){
...
Session_manager.prototype.refresh_sessions = function(){
...
Session_manager.prototype.establish_session = function(prefix, pool){
...
Session_manager.prototype.pool_fetch = function(){
    let pool = this.sessions;
    let session = this.establish_session(`${this.lum.port}_${this.seed}`, pool);
    if (session)
        pool.sessions.push(session);
};

Session_manager.prototype.pool = function(count){
    if (!count)
        return;
...
Session_manager.prototype.update_session = etask._fn(function*(_this, session){
    const res = yield _this.info_request(session,
        (_this.opt.request_timeout||60)*1000, 'SESSION INFO');
    session.info = res.info;
});

Session_manager.prototype.update_all_sessions = etask._fn(function*(_this){
...
Session_manager.prototype._keep_alive_handler = function(session){
...
Session_manager.prototype.stop_keep_alive = function(session){
...
Session_manager.prototype.is_session_expired = function(session, check_only = false){
    if (!session)
        return true;
...
Session_manager.prototype.info_request = etask._fn(
...
Session_manager.prototype.request_session = function(req, auth_header, only,
    src_addr, session_header, rules){
    let session = this._request_session(req, auth_header, only, src_addr,
        session_header);
    if (session && (!session.session || req.retry || session_header))
    {
        ...
        if (session_header && this.session && this.session.session!=
            session_header)
        {
            this.session = null;
        }
        ...
        else if (!this.session && rules && !session_header)
        {
...
Session_manager.prototype._request_session = function(req, auth_header, only,
    src_addr, session_header)
{
    if (only)
        return;
    ...
    if (authorization)
    {
        ...
        this._log.debug(`${req.req_id}: Using request authorization %O`,
            authorization);
        if (session_header)
    ...
    if (this.opt.pool_size)
    {
        ...
        this._log.debug(`${req.req_id}: Selecting pool session %s`,
    ...
    if (this.opt.sticky_ip)
    {
        ...
        if (!this.sticky_sessions[ip])
        {
            this.sticky_sessions[ip] = this.establish_session(
                `${this.lum.port}_${ip}_${this.seed}`, this.sticky_sessions);
        }
        let session = this.sticky_sessions[ip];
        if (this.is_session_expired(session))
            this.sticky_sessions[ip] = null;
        this._log.debug(`${req.req_id}: Selecting sticky session %s`,
            session&&session.session);
        return session;
    }
    if (this.opt.session===true)
    {
        ...
        if (this.is_session_expired(session))
            this.session = null;
        this._log.debug(`${req.req_id}: Selecting seed session %s`,
            session&&session.session);
        return session;
    }
    if (this.opt.session)
    {
        ...
        let session = this.session;
        this.set_keep_alive(session);
        this._log.debug(`${req.req_id}: Selecting explicit session %s`,
            session.session);
        return session;
    }
    this._log.debug(`${req.req_id}: Not using session`);
...
Sess_mgr.prototype.reserve_session = function(sess){
    ...
    sess.reserved = true;
    this.set_keep_alive(sess, this.keep_alive||50*date.ms.SEC);
};

Sess_mgr.prototype.get_reserved_session = function(){
    if (this.reserved_sessions.length>1)
    {
        let s = this.reserved_sessions.shift();
        this.reserved_sessions.push(s);
    }
    return this.reserved_sessions[0]||null;
};
...
function Luminati(opt, mgr){
    ...
    if (opt.ssl)
    {
        ...
        this.http_server.on('connect', (req, res, head)=>{
            ...
            socket.on('error', err=>this._log.error(`${req.req_id}: `
                +`Socket error: %O`, {
    ...
    this.session_mgr = new Session_manager(this, opt);
...
function*_handler(_this, req, res, head){
    ...
    if (wips.length)
    {
        ...
        if (i==wips.length)
        {
            _this._log.debug(`${req.req_id}: Request ip not in whitelist `
                +`${req.url} ${ip}`);
    ...
    {
        _this._log.debug(`${req.req_id}: Placing request on throttle queue`);
...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
    ...
    _this.hosts.forEach(h=>{
        ...
        _this._log.info('adding proxy server %O', {host: h});
    });
...
E.prototype.count_req_status = function count_req_status(success){
    this.req_status_cnt.total++;
    this.req_status_cnt.success += success ? 1 : 0;
};

E.prototype.req_status_cnt_reset = function creq_status_cnt_reset(){
    this.req_status_cnt = {total: 0, success: 0};
};
...
class Rules {
    ...
    pre(req){
        if (!this._pre)
            return;
        req._timeline.track('rules_pre_start');
        ...
        for (let i=0; i<this._pre.length; i++)
        {
            ...
            if (p.reserved_session)
                req.ctx.session = this.get_reserved_session();
            if (p.browser)
            {
                switch (p.browser)
                {
        ...
        req._timeline.track('rules_pre_end');
    }
    ...
    post(req, res, head, _res, hdrs_only){
        if (!this._post)
            return;
        req._timeline.track('rules_post_start');
        let url = req.url_full||req.url;
        for (let i=0; i<this._post.length; i++)
        {
            let p = this._post[i];
        ...
        req._timeline.track('rules_post_end');
    }
    post_body(req, res, head, _res, body){
        if (!this._post)
            return;
        req._timeline.track('rules_body_start');
        ...
        req._timeline.track('rules_body_end');
    }
    post_need_body(req){
        if (!this._post)
            return;
    ...
    retry(req, res, head){
        if (!req.retry)
            req.retry = 0;
        req.retry++;
        this.luminati._log.info(`${req.req_id}: req retry${req.retry}`
            +` ${req.url_full}`);
    ...
    req_status(success){
        this.luminati.count_req_status(success);
    }
    save_session(sess){
...
E.prototype.req_id = req=>{
    let req_id = req.req_id;
    if (!req_id)
        req_id = 'r-0-'+Math.round(Math.random()*10000000);
    if (req.retry)
        req_id=req_id.replace(/-[0-9]*-/, `-${req.retry}`);
    return req_id;

};

E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    let timeline = req._timeline = req._timeline||new Timeline();
    req.req_id = _this.req_id(req);
    if (req.retry)
        timeline.retry();
    if (_this.rules)
    {
    ...
    if (req._queued)
        timeline.track('queued', req._queued);
    ...
    if (req.method=='CONNECT' && ip_re.test(_url))
        _this._log.warn(`${req.req_id}: HTTPS connection to IP: %s`, _url);
    ...
    _this._log.info(`${req.req_id}: ${req.socket.remoteAddress}:`
        +`${req.socket.remotePort} - ${req.method} ${_url}`);

    if (_this.router.is_null_response(_url))
    {
        _this._log.debug(`${req.req_id}: requested url ${_url} matches `
            +`null_response filter `+_this.null_response);
        return _this.router.send_null_response(_url, req, res, timeline, response);
    }
    ...
    let session = _this.session_mgr.request_session(req, auth_header, only,
        src_addr, session_header, _this.rules);
    ...
    if (!stats)
    {
        ...
        _this._log.debug(`${req.req_id}: adding stats for %s`, host);
    }
    ...
    request_stats(req, res, rstats=>{
            ...
            downloaded>=session.bandwidth_max_downloaded)
        {
            ...
            session.bandwidth = Math.round(
                downloaded*1000/(Date.now()-timeline.get('create')));
        }
    });
    const handler = (proxy, _headers)=>etask(function*(){
        ...
        if (req.timeout||_this.timeout)
        {
            proxy.setTimeout(_.min([req.timeout, _this.timeout]), ()=>{
                ...
                if (!_this.rules)
                    return;
                _this._log.debug(`${req.req_id}: `
                    +`${req.method} ${_url} - timeout ${req.timeout}`);
                if (res.resp_written || !_this.rules.can_retry(req));
        ...
        proxy.on('response', _res=>{
            ...
            try {
                ...
                if (session)
                    _this.session_mgr.is_session_expired(session, true);
                write_http_reply(res, _res, _headers);
                ...
                _res.on('end', ()=>{
                    ...
                    _this._log.info(`${req.req_id}: `+timeline.toString());
                    if (_this.opt.log=='debug')
                    {
                        _this._log.debug(util.inspect(response,
                            {depth: null, colors}));
            ...
            try {
                ...
                _this._log.debug(`${req.req_id}:CONNECT - ${_res.statusCode}`);
                if (session)
                    _this.session_mgr.is_session_expired(session, true);
                socket.write(head);
                res.write(_head);
                ...
                socket.on('error', err=>{
                    _this._log.error(`${req.req_id}:Request socket error `
                        +zerr.e2s(err));
                    _this.session_mgr
                        .removeListener('refresh_sessions', end_socket);
                }).on('end', ()=>{
            ...
            {
                _this._log.warn(`${req.req_id}: error proxy response ${host}`);
    ...
    if (_this.router.is_bypass_proxy(_url, _ssl))
    {
        let proxy;
        if (_ssl)
        {
            ...
            proxy.on('connect', ()=>{
                timeline.track('direct_connect');
                stats.active_requests--;
                _this._log.debug(`${req.req_id}: DIRECT CONNECT - ${_url}`);
                write_http_reply(res, {statusCode: 200, statusMessage: 'OK'});
        ...
        else
        {
            ...
            proxy.on('connect', (_res, socket)=>{
                ...
                _this._log.debug(`${req.req_id}: DIRECT REQUEST - ${_url}`);
            });
        ...
        return _this.router.send_null_response(_url, req, res, timeline, response);
    let cred = _this.session_mgr.calculate_username(assign({}, {
        ip: session&&session.ip||_this.opt.ip,
        session: session&&session.session,
        direct: _this.router.is_direct(_url),
        vip: session&&session.vip||_this.opt.vip,
    }, session&&session.authorization||{}));
    res.cred = cred.username;
    _this._log.info(`${req.req_id}: requesting using ${cred.username}`);
    response.proxy = {
        host: host,
        username: cred.username,
    };
    ...
    {
        let _etask = this;
        response.request.url = 'https://'
        +req.url.replace(/^(https?:\/\/[^\/]+)?\//, req.headers.host+'/');
        _this.http.request({
            host: host,
            ...
            if (_res.statusCode!=200)
            {
                if (_this.rules && _this.rules.can_retry(req))
                {
                    _this._log.warn(`${req.req_id}: error proxy response`
                        +` ${host}`);
                    _this.rules.retry(req, res, head);
                    _etask.return();
                    return;
        ...
        if (ctx.banlist)
        {
            ...
            if (ip && ctx.banlist.has(ip))
            {
                ...
                {
                    _this._log.info(`${req.req_id}: ip_banned ${ip}`);
                    req.session = _this.rules.gen_session();
                    _this.rules.retry(req, res, head);
                    _etask.return();
                    return;
                }
