...
function*connect_proxy(_this, session, timeout, context){
    ...
    let err, info;
    try {
        _this.http.request(opt).on('connect', (_res, sock, head)=>{
            timeline.response = timeline.end = Date.now() - timeline.start;
            assign(res, {
            ...
            {
                let ip = (_res.headers['x-hola-timeline-debug']||
                    _res.headers['x-hola-unblocker-debug']||
                    _res.headers['x-hola-ip']||'')
                    .match(/(\d+(?:\.\d+){3})/);
                info = {ip: ip&&ip[1]};
            }
            sock.end();
...
class Rules {
    ...
    retry(req, res, head){
        ...
        this.luminati._request.call(this.luminati, req, res, head);
    }
    can_retry(req, response){
        ...
        if (!ret)
        {
            ua.event('rules', 'max_retry', JSON.stringify({url: req.url_full,
                retry: req.retry, user: response.proxy.username}));
        }
        return ret;
    }
    gen_session(){
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    if (req.socket instanceof tls.TLSSocket)
    {
        ...
        _this.http.request({
            ...
            if (_this.ipcache)
            {
                let tl = (_res.headers['x-hola-timeline-debug']||'').split(' ');
