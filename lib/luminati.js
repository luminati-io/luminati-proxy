...
class Rules {
    ...
    can_retry(req, response, action){
        ...
        if (action)
            ret = !(action.retry==false||action.retry<=retry||!ret);
        return ret;
    }
    gen_session(){
    ...
    action(req, res, head, _res, action){
        ...
        // XXX ofir/maximk: reverted (tests failed), fix and re-commit
        if (action.retry==false) // if (!this.can_retry(req, res, action))
            return false;
