...
Sess_mgr.prototype.refresh_sessions = function(){
    this._log.notice('Refreshing all sessions');
    ...
    check_only = false)
{
    ...
    {
        this._log.info(`session ${session.host}:${session.session} expired,`
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    if (_this.reverse_lookup)
        ctx.set_reverse_lookup_res(yield _this.reverse_lookup_url(ctx.url));
    if (ctx.rules)
    ...
    return yield _this.route_req(req, res, head);
});
...
E.prototype.send_proxy_req_ssl = function(task, req, res, head, host){
    const ctx = req.ctx;
    const _this = this;
