        ...
        this.on('ensure', ()=>{
            ...
            if (--_this.active)
            {
                if (_this.throttle_queue.length)
                    _this.throttle_queue.shift().econtinue();
                return;
            }
...
Luminati.prototype.stop = etask._fn(function*stop(_this){
    yield etask.nfn_apply(_this.http_server, '.close', []);
    if (_this.https_server)
        yield etask.nfn_apply(_this.https_server, '.close', []);
    return _this;
});
        ...
        for (;; tryout++)
        {
            ...
            let username = calculate_username(assign({session: session},
                _.pick(_this.opt, ['customer', 'zone', 'country', 'state',
                    'city', 'asn', 'cid', 'ip', 'raw', 'dns', 'timeout'])));
...
Luminati.prototype._request = etask._fn(function*(_this, req, res, head){
    ...
    {
        _this._log('DEBUG', `Returning null response for ${_url}`);
        write_http_reply(res, {statusCode: 200, statusMessage: 'NULL'});
        res.end();
        return;
    }
    let authorization =
