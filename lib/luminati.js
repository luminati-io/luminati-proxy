...
class Router extends events.EventEmitter {
    ...
    should_init_stats(req){
        return !(this.is_fw_check(req)||req.retry||
            this.is_null_response(req.ctx.url, req));
    }
...
class Context {
    ...
    static init_req_ctx(req, lum, opt={}){
    ...
    init_response(req, res, head){
        res.x_hola_context = this.h_context;
...
class Session {
    ...
    calculate_username(){
        let opt = assign.apply({}, [this.mgr.opt, this.mgr, this.opt]
            .map(o=>_.pick(o||{}, Session.fields)));
        ...
        return {username: username.calc(opt_usr, this.mgr.opt.short_username),
            password: opt.password};
    }
}
Session.fields = qw`customer zone country state city session asn dns
    request_timeout cid ip raw direct debug password mobile vip carrier`;
...
Sess_mgr.prototype.calculate_username = function(opt){
    opt = assign.apply({}, [this.opt, this, opt].map(o=>_.pick(o||{},
        ...
        cid ip raw direct debug password mobile vip carrier`)));
    let opt_usr = _.omit(opt, qw`request_timeout password`);
    if (opt_usr.ip)
        opt_usr = _.omit(opt_usr, qw`session`);
    if (opt.request_timeout)
...
Sess_mgr.prototype.establish_session = function(prefix, pool){
    ...
    this.log.info(`new session added ${host}:${session_id}`);
...
Sess_mgr.prototype.pool = function(count){
    ...
    for (let i=0; i<count; i++)
        this.pool_fetch();
    this.log.debug(`initialized pool - ${this.opt.pool_size}`);
    this.pool_ready = true;
};
...
Sess_mgr.prototype.set_keep_alive = function(session){
    let keep_alive = session.reserved ? this.reserved_keep_alive
        : this.keep_alive;
    if (!keep_alive)
        return;
    ...
    session.keep_alive = this.lum._timeout(this._keep_alive_handler.bind(this),
        keep_alive, session);
    this.log.debug(`Schedule keep alive ${session.host}:${session.session}`);
};

Sess_mgr.prototype._keep_alive_handler = function(session){
    if (!session.pool || session.pool.canceled || this.lum.stopped)
        return;
    const _this = this;
    this.set_keep_alive(session);
    this.log.info(`Keep alive ${session.host}:${session.session}`);
    ...
    check_only = false)
{
    ...
    {
        this.log.info(`session ${session.host}:${session.session} expired,`
            +` ip ${last_res.ip} banned`);
    ...
    {
        this.stop_keep_alive(session);
        this.log.debug(`session ${session.host}:${session.session} expired`);
    }
    return expired;
};
...
Stats.prototype.init_host = function(host){
    ...
    return this.by_hsts[host] = {total_requests: 0, total_inbound: 0,
        ...
        status_code: {}, success: 0, fail: 0, real_inbound: 0,
        real_outbound: 0, real_reqs: 0};
};
...
Stats.prototype.track_bw = function(req, downloaded, uploaded){
    ...
    req.ctx.response.in_bw = downloaded;
    req.ctx.response.out_bw = uploaded;
    stats.total_inbound += downloaded;
    stats.total_outbound += uploaded;
    stats.total_requests++;
    if (req.ctx.h_context!='STATUS CHECK'&&
        req.ctx.h_context!='SESSION KEEP ALIVE')
    {
        stats.real_inbound += downloaded;
        stats.real_outbound += uploaded;
        stats.real_reqs++;
    }
};
    ...
    if (opt.ssl)
    {
        ...
        this.https_server = https.createServer(
            ...
            (req, res, head)=>{
                ...
                {
                    this.log.info(`Request ip fixed`
                        +` ${remote_ip} ${req.url}`);
        ...
        this.http_server.on('connect', (req, res, head)=>{
            ...
            socket.on('error', err=>this.log.error(
                `Socket error: %O`, {
...
function*_handler(_this, req, res, head){
    if (_this.is_overload())
        return _this.send_overload(res);
    ...
    try {
        ...
        if (wips.length)
        {
            ...
            {
                _this.log.debug(
                    `Request ip not in whitelist ${req.url} ${ip}`);
...
function*_check_proxy_response(_this, proxy, res, context){
    ...
    _this.log.warn('invalid proxy response %O',
    ...
    _this.log.warn('removing failed proxy server %O', {host: proxy});
    if (_this.hosts)
    {
...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
        ...
        Date.now()-timestamp<30000)
    {
        ...
        {
            _this.log.debug(`using super proxy ${proxy}`);
            hosts[proxy] = false;
            continue;
        }
        proxies.push(proxy);
        ...
        try {
            ...
            _this.log.debug(`resolved ${proxy} (${domain}) %O`, ips);
            ips.forEach(ip=>hosts[ip] = proxy);
        } catch(e){
            _this.log.debug(`Failed to resolve ${proxy} (${domain}): ${e}`
                +zerr.e2s(e));
        }
    }
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    const ctx = Context.init_req_ctx(req, _this, _this.opt);
    try {
        ...
        if (_this.router.should_init_stats(req))
...
function*send_bypass_req(_this, task, req, res, head){
    ...
    if (ctx.is_connect)
    {
        ...
        proxy.on('connect', ()=>{
            ...
            ctx.log.debug(`DIRECT CONNECT - ${ctx.url}`);
            write_http_reply(res, {statusCode: 200, statusMessage: 'OK'});
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        {
            if (_this.is_overload({code: conn.res.statusMessage},
                conn.res.stausCode))
            {
                throw new Error('overload');
            }
            if (ctx.rules && ctx.rules.can_retry(req, ctx.response, {
                retry: ctx.proxy_retry}))
            {
                ctx.log.warn(`error proxy ${host} response`,
                    `status: ${conn.res.statusCode}, retrying`);
                ctx.rules.retry(req, res, head);
                return yield this.wait();
            }
            yield _this._check_proxy_response(host,
                {statusCode: conn.res.statusCode}, {from:
                'error', error: conn.res.statusMessage});
...
E.prototype.is_overload = function(err, status){
    if (this.overload)
        return true;
    if (err&&(err.code == 'EMFILE'||(err.message||'').includes('EMFILE')))
        return true;
    if (status == 503)
        return true;
    return false;
...
E.prototype.send_overload = function(res){
    this.log.warn('overloaded, rejecting');
    write_http_reply(res, {statusCode: 503, statusMessage: 'Overloaded',
        headers: {Connection: 'close'}});
    res.end();
    if (res.destroy)
        res.destroy();
...
E.prototype._handle_proxy_error= function(req, res, proxy, task, head){
    return err=>{
        ...
        if (ctx.rules && ctx.rules.can_retry(req, ctx.response, {
            retry: ctx.proxy_retry}))
        {
            ctx.log.warn(`error proxy response ${ctx.host}, retrying`);
            ctx.rules.retry(req, res, head);
            this._abort_proxy_req(req, proxy);
            return;
        }
...
class Rules {
    ...
    retry(req, res, head, port){
        if (!req.retry)
            req.retry = 0;
        req.retry++;
        req.ctx.log.info(`req retry ${req.retry}: ${req.ctx.url}`);
        let serv;
        if (port)
        {
    ...
    req_status(req, success){
        if (req.ctx.url.match(/https?:\/\/lumtest\.com/))
            return;
