...
function Luminati(opt){
    ...
    {
        _this.authorization = {};
        _this.https = https.createServer(_this.opt.ssl, (req, res, head)=>{
...
Luminati.prototype._request = etask._fn(function*(_this, req, res, head){
    ...
    if (0)
    {
        const proxy = http.request({protocol: 'http:', host: req.headers.host, port: 80,
