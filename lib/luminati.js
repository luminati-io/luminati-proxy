...
E.prototype.refresh_ip = etask._fn(function*lum_refresh_ip(_this, ip){
    // XXX krzysztof: WIP
    _this.log.info('Refreshing IP %s', ip);
    if (!_this.opt.ips.includes(ip))
        return;
    const r = yield _this.mgr.refresh_ips(_this.opt.zone, zurl.ip2num(ip));
    console.log(r);
});
...
class Rules {
    ...
    action(req, res, head, _res, action){
        ...
        else if (action.refresh_ip && _res)
        {
            if (tl)
            {
                this.luminati.refresh_task = this.luminati.refresh_ip(
                    tl.split(' ')[3]);
            }
        }
