...
E.prototype._send_rule_mail = function(to, action, _url){
    return this.mgr.send_rule_mail(this.port, to, action, _url);
};
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        if (ctx.banlist||_this.mgr.banlist)
        {
            ...
            if (ip && (ctx.banlist && ctx.banlist.has(ip) ||
                _this.mgr.banlist && _this.mgr.banlist.has(ip)))
            {
    ...
    return _res=>{
        ...
        try {
            ...
            if (ctx.rules)
            {
                ...
                else if (ctx.rules.post(req, res, head, _res))
