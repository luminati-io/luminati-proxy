...
class Router extends events.EventEmitter {
    constructor(opt){
        ...
        if (opt.direct_include)
        {
            this.direct = {include: opt.direct_include &&
                new RegExp(opt.direct_include, 'i')};
        }
    ...
    is_fw_check(req){ return !!req.ctx.h_firewall_check; }
    is_proxy_req(req){
...
class Context {
    ...
    request_start(){
        ...
        this.sys_timeout = this.opt.timeout*SEC || 0;
...
function*_keep_alive_handler(_this, session){
    ...
    let res = yield _this.info_request(session, _this.opt.request_timeout*SEC,
        'SESSION KEEP ALIVE');
    let info = res.info;
...
Sess_mgr.prototype._request_session = function(req){
    ...
    if (authorization)
    {
        if (ua&&ua.event)
        {
            ua.event('luminati', 'authorization passed', JSON.stringify(
                authorization));
            ua.send();
        }
...
E.prototype.stop = etask._fn(function*stop(_this, force){
    ...
    tasks.tcp = etask.nfn_apply(_this.tcp_server, stop_method, []);
...
function*init_proxy_req(_this, req, res){
    ...
    if (!ctx.ext_proxy)
        ctx.connect_headers['x-hola-agent'] = ctx.h_agent||E.hola_agent;
});
...
class Rules {
    ...
    pre(req){
        ...
        if (!this._pre)
            return;
        let url = req.ctx.url||req.url_full||req.url;
    ...
    post_body(req, res, head, _res, body){
        ...
        for (let i=0; i<this._post.length; i++)
        {
            ...
            for (let j=0; j<p.res.length; j++)
            {
                ...
                {
                    if (this.action(req, res, head, _res, pr_action, p, _body))
                        return true;
                }
    ...
    can_retry(req, response, action){
        ...
        let ret = (!action||!action.refresh_ip)&&retry<20||retry==0;
        let port;
        if (action)
        {
    ...
    process_response(req, _res, process_rules, body){
    ...
    action(req, res, head, _res, action, rule, body){
        const ctx = req.ctx;
