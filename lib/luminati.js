...
const Luminati = module.exports = function(opt){
    ...
    let handler = (req, res, head)=>etask(function*(){
        try {
            if (opt.pool_size)
            {
                if (!_this.sessions)
...
Luminati.prototype.listen = etask._fn(function*(_this, port){
...
Luminati.prototype._pool = etask._fn(function*(_this, count, retries){
    let fetch = tryout=>etask(function*(){
        for(;;tryout++)
        {
...
Luminati.prototype._request = etask._fn(function*(_this, req, res, head){
    ...
    let username = calculate_username(Object.assign({}, _this.opt, {
