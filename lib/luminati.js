...
E.prototype.init_https_server = function(){
    ...
    this.http_server.on('connect', (req, res, head)=>{
        ...
        socket.on('error', err=>{
            this.log.error(`Socket error: %s`, zerr.e2s(err));
        });
        socket.on('timeout', ()=>{
...
E.prototype.listen = etask._fn(function*listen(_this, listen_port, hostname){
    try {
        ...
        yield this.wait();
        _this.http_server.on('error', e=>{
            this.throw(e);
        });
        yield etask.nfn_apply(_this.http_server, '.listen', [0, '127.0.0.1']);
        if (_this.https_server)
        {
            _this.https_server.on('error', e=>{
                this.throw(e);
            });
            yield etask.nfn_apply(_this.https_server, '.listen',
                [0, '127.0.0.1']);
        }
        _this.emit('ready');
        return _this;
...
E.prototype.stop = etask._fn(function*stop(_this){
    try {
        ...
        yield etask.nfn_apply(_this.http_server, '.forceShutdown', []);
        if (_this.https_server)
            yield etask.nfn_apply(_this.https_server, '.forceShutdown', []);
        _this.emit('stopped');
        return _this;
    } catch(e){
        _this.emit('error', e);
...
E.prototype.check_proxy_response = function(proxy, res, context){
    const err = new Error();
...
function*request_handler(_this, req, res, proxy, head, headers){
    ...
    .on('timeout', _this.handle_proxy_timeout(req, res, proxy, this))
...
E.prototype.is_ip_banned = function(ip, domain){
    ...
    return this.banlist.has(ip, domain) ||
        this.mgr && this.mgr.banlist && this.mgr.banlist.has(ip, domain);
};
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        {
            let proxy_err = _this.check_proxy_response(host, conn.res,
...
function*send_proxy_req_http(_this, task, req, res, head, host){
    ...
    try {
        ...
        const proxy = _this.http.request({
            ...
            method: req.method,
            path: ctx.url,
            // XXX krzysztof: use agent again when super proxy is fixed to
            // reuse sockets
            // agent: ctx.agent,
...
E.prototype.handle_proxy_timeout = function(req, res, proxy, task){
    return ()=>{
        ...
        const err = new Error(`${req.method} ${ctx.url} - socket inactivity`
    ...
    return proxy_res=>{
        ...
        try {
            ...
            if (ctx.rules && !res.resp_written)
            {
                ...
                if (ctx.rules.post(req, res, head, proxy_res))
                ...
                else if (ctx.rules.post_need_body(req))
                {
                    ...
                    proxy_res.on('end', ()=>{
                        ...
                        assign(ctx.response, {
                            ...
                            headers: assign({}, proxy_res.headers,
                            _headers||{}),
                        });
            ...
            proxy_res.on('end', ()=>{
                ...
                assign(ctx.response, {
                    status_code: proxy_res.statusCode,
                    status_message: proxy_res.statusMessage,
                    headers: assign({}, proxy_res.headers, _headers||{}),
                });
                task.return(ctx.response);
            }).on('error', this.log_throw_fn(task));
        } catch(e){
...
E.prototype.log_error = function(err){
    if (err.custom)
        this.log.error(err.message);
    else if (!err.code)
        this.log.error(err.stack.split('\n').slice(0, 2).join('\n'));
    else if (error_messages[err.code])
        this.log.error(error_messages[err.code]);
    else
        this.log.error(err.stack.split('\n').slice(0, 2).join('\n'));
};
