...
function Luminati(opt, mgr){
    ...
    this.http_server = http.createServer((req, res, head)=>{
        ...
        if (this.is_overload())
            return this.send_overload(socket);
        socket.setNoDelay();
    });
...
function*_handler(_this, req, res, head){
    ...
    try {
        ...
        _this.log.debug('_hanler error %s', zerr.e2s(e));
        throw e;
    }
});
...
E.prototype.complete_req = function(err, req, res){
    if (err)
    {
        if (!(err.message||'').includes('ECONNRESET'))
            this.log.error(`%s %s %s`, req.method, req.url, zerr.e2s(err));
        if (!res.ended)
        {
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    if (_this.is_overload())
        ...
        _this.log.debug('_request err %s', zerr.e2s(e));
        throw e;
    }
});
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        if (conn.res.statusCode!=200)
        {
            ...
            {
                ctx.log.warn(`error proxy response ${host}, retrying`);
        ...
        _this.log.debug('send proxy ssl error %s', zerr.e2s(e));
        return e;
    }
});
...
E.prototype.is_overload = function(err){
