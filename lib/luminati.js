...
const get_error_advice = e=>{
    if (e.message.includes('unable to verify the first certificate'))
    {
        return `ADVISE: You can try to enable Insecure option to fix it `
        +`(This option is on the General tab)\n\r`;
    }
    if (e.message.includes('Auth Failed'))
        return `ADVICE: Check password/zone name credentials\n\r`;
    return `Error happened. Look at header "x-lpm-error"`;
};
...
function Luminati(opt, mgr){
    ...
    this.requests_queue = [];
    this.throttle_queue = [];
    if (this.mgr.argv && !this.mgr.argv.high_perf)
    {
        this.req_process = new ReqProcess({
...
E.prototype.init_tcp_server = function(){
    this.tcp_server = new net.createServer(socket=>{
        ...
        socket.on('timeout', ()=>{ ensure_socket_close(socket); });
...
E.prototype.is_whitelisted = function(req){
    ...
    if (auth_header)
    {
        ...
        if (user=='token' && this.mgr._defaults.token_auth &&
            pass==this.mgr._defaults.token_auth)
        {
            return true;
        }
    }
...
E.prototype.handler = etask._fn(
function*handler(_this, req, res, head){
    ...
    try {
        ...
        res.on('error', e=>_this.log.error(zerr.e2s(e)));
...
E.prototype.listen = etask._fn(function*listen(_this, listen_port, hostname){
    ...
    _this.tcp_server.on('error', e=>{
        // XXX krzysztof: try to use error handler
        this.throw(new Error(e));
    });
    _this.tcp_server.listen(listen_port, hostname, this.continue_fn());
    yield this.wait();
    _this.http_server.on('error', err=>_this.error_handler('HTTP', err));
    yield etask.nfn_apply(_this.http_server, '.listen', [0, '127.0.0.1']);
    if (_this.https_server)
    {
        _this.https_server.on('error', err=>_this.error_handler('HTTPS', err));
        yield etask.nfn_apply(_this.https_server, '.listen', [0, '127.0.0.1']);
    }
    _this.emit('ready');
    return _this;
});
...
E.prototype.stop = etask._fn(function*stop(_this){
    ...
    const stop_method = '.forceShutdown';
...
E.prototype.resolve_super_proxy = etask._fn(function*super_proxy(_this){
    if (!_this.super_proxy_ips)
    {
        _this.super_proxy_ips = yield etask.nfn_apply(dns, '.resolve',
            [lpm_config.default_superproxy_domain]);
    }
    return _this.super_proxy_ips;
});
...
function*lpm_request(_this, req, res, head){
    ...
    try {
        ...
        const get_process = _this.mgr.argv && !_this.mgr.argv.high_perf &&
            !lpm_config.is_win && req.socket.remoteAddress=='127.0.0.1';
        if (get_process)
        ...
        resp.statusCode = 502;
        if (ctx.rules && ctx.rules.post(req, res, head, resp))
            return yield ctx.req_sp.wait();
        prepare_resp(req, resp);
        resp.headers = {Connection: 'close', 'x-lpm-error': e.message};
        _this.emit('response', resp);
        _this.send_advice_req_error(res, e);
        ensure_socket_close(req.socket);
...
E.prototype.route_req = etask._fn(function*route_req(_this, req, res, head){
    try {
        ...
        if (_this.router.is_fake_request(req))
...
E.prototype.is_ip_banned = function(ip, domain){
    ...
    return this.banlist.has(ip, domain) ||
        this.mgr.banlist && this.mgr.banlist.has(ip, domain);
};
...
function*send_proxy_req_ssl(_this, task, req, res, head, host){
    ...
    try {
        ...
        const conn_req = _this.http.request({
            ...
            agent: ctx.agent,
            rejectUnauthorized: !_this.opt.insecure && !_this.opt.unblock,
        })
        ...
        // XXX jesse remove duplication of this block
        if (ctx.force_body)
            ctx.response.request.body = ctx.force_body;
        if (ctx.response.request.body)
            proxy.write(ctx.response.request.body);
        req.setMaxListeners(30);
        req.pipe(proxy);
...
function*send_fake_request(_this, task, req, res){
    try {
        ...
        return yield _this._request_handler(req, res, fake_proxy, undefined,
            _res.headers);
    } catch(e){
        ...
        try {
            ...
            if (ctx.rules && !res.resp_written)
            {
                ...
                else if (ctx.rules.post_need_body(req))
                {
                    ...
                    _res.on('end', ()=>{
                        const rule_res = ctx.rules.post_body(req, res, head,
                            _res, temp_data);
                        if (rule_res)
                            return this._abort_proxy_req(req, proxy);
                        for (let i=0; i<temp_data.length; i++)
                        {
                            if (ctx.response.body_size>=1024)
                                break;
...
E.prototype.handle_proxy_connect = function(req, res, proxy, task, head){
    return (proxy_res, socket, proxy_head)=>{
        ...
        try {
            ctx.timeline.track('connect');
            const proxy_err = this._check_proxy_response(ctx.host, proxy_res,
                'connect');
            if (proxy_err)
            {
                req.ctx.log.warn('proxy api err %s: %s', proxy_err.code,
            ...
            socket.on('error', err=>{
                ...
                if (this.is_overload({code: proxy_res.statusMessage},
                    proxy_res.statusCode))
                {
                ...
                {
                    const applied = ctx.rules.post(req, res, head, proxy_res);
                    if (!applied && ctx.rules.post_need_body(req))
                    {
                        ...
                        proxy_res.on('end', ()=>{
                            if (ctx.rules.post_body(req, res, head, proxy_res,
                                ctx.response.body))
                            {
                                return this._abort_proxy_req(req, proxy);
                            }
...
E.prototype.send_proxy_error = function(req, res, err){
    ...
    write_http_reply(res, {statusCode: 502, headers,
        statusMessage: 'LPM - Proxy Error'}, undefined, {end: true});
};

E.prototype.send_advice_req_error = function(res, err){
    if (res.ended)
        return;
    const advice = get_error_advice(err);
    const headers = {'x-lpm-error': err.message,
        'Content-Length': advice.length};
    write_http_reply(res, {statusCode: +err.code||502, headers,
        statusMessage: 'LPM - Proxy Error'}, undefined, {end: false});
    res.write(advice);
};

E.prototype.send_lpm_error = function(req, res, err){
    this.log.error(zerr.e2s(err));
    if (res.ended)
        return;
    const headers = {Connection: 'close', 'x-lpm-error': err.message};
    write_http_reply(res, {statusCode: 502, headers,
        statusMessage: 'LPM - Bad Gateway'}, undefined, {end: true});
};
