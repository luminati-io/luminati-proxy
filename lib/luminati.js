...
function*set_keep_alive(_this, session){
    ...
    {
        _this.log.debug(
            `Schedule keep alive ${session.host}:${session.session}`);
...
function*_keep_alive_handler(_this, session){
    if (_this.is_session_expired(session) || _this.lum.stopped)
        return false;
    _this.log.info(`Keep alive ${session.host}:${session.session}`);
...
E.prototype._handle_proxy_error= function(req, res, proxy, task, head){
    return err=>{
        ...
        // XXX maximk: check if X-Luminati-Error is caught here
