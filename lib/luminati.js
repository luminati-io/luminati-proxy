...
function Luminati(opt){
    ...
    {
        this.session = {
            session: opt.session===true ? this.seed : opt.session,
            count: 0,
            stats: true,
        };
    }
...
E.prototype._keep_alive = function(session){
    if (session.pool && session.pool.canceled || this.stopped)
        return;
...
function*request_session(_this, req, auth_header, only){
    ...
    if (_this.session)
    {
        _this._log.debug('Selecting opt session', _this.session.session);
        _this.set_keep_alive(_this.session);
        return _this.session;
    }
    _this._log.debug('No using session');
