...
Session.fields = qw`customer zone country state city session asn dns
    request_timeout cid ip raw direct debug password mobile vip`;
...
E.prototype.get_req_host = function(req){
    ...
    return ctx.session&&ctx.session.host
        || (ctx.only_bypass ? loopback_ip : this.hosts[0]);
};
