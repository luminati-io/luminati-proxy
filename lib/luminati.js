...
function Luminati(opt){
    ...
    _this.opt = opt;
    opt.port = opt.port||24000;
    opt.proxy_port = opt.proxy_port||22225;
    _this._log = log(opt.port, opt.log);
    _this.stats = {};
    _this.active = 0;
    _this.failure = {};
    _this.requests_queue = [];
    _this.throttle_queue = [];
    _this.sticky_sessions = {};
    _this.next_connection_id = 0;
    _this.active_connections = {};
    if (opt.socks)
    {
        _this.socks_server = new Socks({local: opt.socks, remote: opt.port,
            log: opt.log});
    }
    let handler = (req, res, head)=>etask(function*request(){ // TODO lee move to a member function
        _this.active++;
        ...
        if (opt.throttle && _this.active>=opt.throttle)
        {
    ...
    const on_connection = socket=>{
        socket.setNoDelay();
        let id = _this.next_connection_id++;
        socket.on('close', ()=>delete _this.active_connections[id]);
        _this.active_connections[id] = socket;
    };
    ...
    {
        _this.authorization = {};
        _this.https_server = https.createServer(_this.opt.ssl,
            (req, res, head)=>{
                let authorization = _this.authorization[req.socket.remotePort];
                if (authorization)
...
Luminati.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
    ...
    while (Object.keys(hosts).length<(_this.opt.proxy_count||1) &&
...
Luminati.prototype.request = function(){
    ...
    args[0].proxyHeaderExclusiveList = [].concat(
        args[0].proxyHeaderExclusiveList||[], qw`proxy-connection
        proxy-authentication x-hola-agent x-hola-debug x-hola-tunnel-key
        x-hola-tunnel-ip x-hola-tunnel-session x-hola-auth
        x-hola-unblocker-debug x-hola-session x-hola-cid x-hola-country
        x-hola-forbid-peer x-hola-dst-ips x-hola-ip x-hola-immediate
        x-hola-dns-only x-hola-response x-hola-direct-first
