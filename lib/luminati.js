...
E.prototype.init_https_server = function(){
    ...
    this.http_server.on('connect', (req, res, head)=>{
        ...
        socket.on('error', e=>{
            ...
            this.log.error('socket: %s', zerr.e2s(e));
        });
        socket.on('timeout', ()=>{
...
E.prototype.init_proxy_req = function(req, res){
    ...
    ctx.proxy_port = ctx.session && ctx.session.proxy_port ||
        this.opt.proxy_port;
    ctx.cred = this.session_mgr.get_req_cred(req);
...
function*lpm_request(_this, req, res, head){
    const headers = assign({}, req.headers), ua = req.headers['user-agent'];
    const is_user_req = ()=>
        !(headers['user-agent']||'').includes('luminati-proxy-manager') &&
        !headers['x-lpm-keep-alive'];
    ...
    try {
        ...
        if (is_user_req())
            _this.mgr.first_lpm_action('successful_proxy_req', ua);
        _this.emit('response', resp);
        ctx.req_sp.return(resp);
    } catch(e){
        if (is_user_req())
            _this.mgr.first_lpm_action('non_successful_proxy_req', ua);
