...
const {decode_body, write_http_reply} = require('./util.js');
...
E.prototype.init_https_server = function(){
    ...
    this.http_server.on('connect', (req, res, head)=>{
        ...
        socket.on('timeout', ()=>{
            const _local_address = socket.localAddress;
            const _local_port = socket.localPort;
            const _remote_address = socket.remoteAddress;
            const _remote_port = socket.remotePort;
            zerr.debug(`No activity on socket ${_local_address}:`
                +`${_local_port}-${_remote_address}:`
                +`${_remote_port} - close socket`);
    ...
    return (_res, socket, _head)=>{
        ...
        try {
            ...
            socket.on('error', err=>{
                ...
                {
                    const applied = ctx.rules.post(req, res, head, _res, true);
...
E.prototype.banip = function(ip, ms){
    // XXX krzysztof: probably not needed condition check
    if (!this.banlist)
        return false;
