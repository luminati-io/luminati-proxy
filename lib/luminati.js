    ...
    if (opt.ssl)
    {
        ...
        this.https_server = https.createServer(
            ...
            (req, res, head)=>{
                ...
                {
                    this._log.info(
                        `${req.req_id}: Request ip fixed ${remote_ip} ${req.url}`);
...
function*_handler(_this, req, res, head){
    ...
    this.finally(()=>{
        if (this.error)
        {
            ...
            {
                _this._log.error(
                    `${req.req_id}: ${req.method} ${req.url} - ${this.error}`);
            }
            if (!res.ended)
            {
    ...
    if (wips.length)
    {
        ...
        if (i==wips.length)
        {
            _this._log.debug(`${req.req_id}: Request ip not in whitelist ${req.url} ${ip}`);
    ...
    session_header)
{
    ...
    if (authorization)
    {
        ...
        this._log.debug(`${req.req_id}: Using request authorization`, authorization);
        if (session_header)
    ...
    if (this.opt.pool_size)
    {
        ...
        this._log.debug(`${req.req_id}: Selecting pool session`, session&&session.session);
        return session;
    }
    if (this.opt.sticky_ip)
    {
        ...
        this._log.debug(`${req.req_id}: Selecting sticky session`, session&&session.session);
        return session;
    }
    if (this.opt.session===true)
    {
        ...
        if (this.is_session_expired(session))
            this.session = null;
        this._log.debug(`${req.req_id}: Selecting seed session`, session&&session.session);
        return session;
    }
    if (this.opt.session)
    {
        ...
        let session = this.session;
        this.set_keep_alive(session);
        this._log.debug(`${req.req_id}: Selecting explicit session`, session.session);
        return session;
    }
...
class Rules {
    ...
    retry(req, res, head){
        if (!req.retry)
            req.retry = 0;
        req.retry++;
        this.luminati._log.info(`${req.req_id}: req retry${req.retry} ${req.url_full}`);
...
E.prototype._send_null_response = function(_url, req, res, timeline, response){
    this._log.debug(`${req.req_id}: Returning null response: ${req.method} ${_url}`);
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    if (req.method=='CONNECT' && ip_re.test(_url))
        _this._log.warn(`${req.req_id}: HTTPS connection to IP: `, _url);
    ...
    if (session && (!session.session || req.retry || session_header))
    {
        ...
        else if (!_this.session && _this.rules && !session_header)
        {
    ...
    if (!stats)
    {
        ...
        _this._log.debug(`${req.req_id}: adding stats for`, host);
    }
