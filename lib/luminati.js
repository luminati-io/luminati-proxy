...
let write_http_reply = (stream, res, headers)=>{
    ...
    if (stream.x_hola_context)
        headers['x-hola-context'] = stream.x_hola_context;
    ...
    if (opt.history && opt.history_aggregator)
    {
        this.on('response', response=>{
            let proxy_info = qw`x-hola-timeline-debug
                ...
                .find(i=>i)||['', '', ''];
            let node_latency = +((response.headers['x-hola-timeline-debug']
            ...
            setTimeout(()=>{
                let data = {
                    ...
                    proxy_peer: proxy_info[1]||response.headers['x-hola-ip'],
                    country: proxy_info[2],
                    timeline: stringify(response.timeline),
                    content_size: response.body_size,
                    download_size: response.stats.inbound,
                    upload_size: response.stats.outbound,
...
E.prototype._request = etask._fn(function*_request(_this, req, res, head){
    ...
    request_stats(req, res, rstats=>{
        ...
        response.stats.inbound = downloaded;
        response.stats.outbound = uploaded;
        ...
        proxy.on('response', _res=>{
            ...
            try {
                ...
                socket.on('error', err=>{
                    ...
                    _this.removeListener('refresh_sessions', end_socket);
                }).on('end', ()=>{
                ...
                _this.on('refresh_sessions', end_socket);
            }
            catch(e){ this.throw(e); }
        }).on('error', err=>{
    ...
    if (_this._bypass_proxy(_url, ssl))
    {
        let proxy;
        if (ssl)
        {
