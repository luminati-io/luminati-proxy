...
const executable_path_exists_e = lib=>{
	try { return file.exists(lib.executablePath()); }
...
try {
    ...
    puppeteer.ready = executable_path_exists_e(puppeteer);
    puppeteer.launch = (opt={})=>{
        if (executable_path_exists_e(puppeteer))
...
const fetch_chromium = ()=>etask(function*(){
    ...
    if (lpm_config.is_win)
        child = fork(install_path, {stdio: [0, 1, 2, 'ipc'], env});
    else
        child = spawn('node', [install_path], {stdio: 'inherit', env});
    child.on('exit', (code, signal)=>{
        if (signal)
            logger.notice('Chromium fetch process was interrupted '+signal);
        if (code==0)
        {
