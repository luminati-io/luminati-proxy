...
class Config {
    ...
    upload(config){
        if (lpm_config.is_win || !this.mgr._defaults.sync_config)
            return;
        let _this = this;
    ...
    get_proxy_configs(){
        ...
        // force sync_config if already in use
        if (this.opt.filename&&file.exists(this.opt.filename+'.local_backup'))
    ...
    _load_config(){
        ...
        {
            const {lpm_token, customer} = this.mgr._defaults||{};
            conf._defaults = Object.assign({lpm_token, customer,
