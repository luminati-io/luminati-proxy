...
class Config {
    ...
    serialize(proxies, _defaults){
        ...
        proxies = proxies.map(p=>_.omit(p, qw`stats proxy_type zones
            www_whitelist_ips logs conflict version`));
    ...
    save(){
        ...
        try {
            ...
            this.mgr.log.error('Could not save config %s: %s',
    ...
    get_proxy_configs(){
        const {explicit_opt, overlay_opt, token, port} = this.mgr.argv;
        ...
        if (qw`port`.some(k=>explicit_opt[k]))
        {
            qw`port`.filter(k=>explicit_opt[k]).forEach(k=>{
                proxies.filter(p=>p[k]==explicit_opt[k])
                .forEach((p, i)=>{
                    this.mgr.log.warn('Conflict between config Proxy #%s and '
        ...
        {
            Luminati.dropin.listen_port = dropin_port;
            proxies.push(Object.assign({}, Luminati.dropin, _.pick(_defaults,
                qw`zone test_url`)));
        }
        Object.assign(_defaults, _.pick(overlay_opt,
            lpm_config.default_fields));
        if (token)
            _defaults.token = token;
        const max_port = port || Luminati.default.port;
        const next = (max, key)=>{
    ...
    load_json(){
        ...
        try {
            ...
            if (!s)
                return {};
            console.log(`\nLoaded config ${this.opt.filename}`);
