...
class Config {
        ...
        return etask(function*(){
            ...
            parsed_config._defaults = zutil.omit(parsed_config._defaults||{},
                qw`customer account_id lpm_token version sync_config
                    ask_sync_config cluster`);
            try { yield _this.mgr.lpm_f.update_conf(parsed_config); }
            catch(e){
    ...
    _prepare_proxy(proxy){
        ...
        if (proxy.ext_proxies && (!Array.isArray(proxy.ext_proxies) ||
            proxy.ext_proxies.length>consts.MAX_EXT_PROXIES))
        {
    ...
    _serialize(proxies, _defaults, ts){
        ...
        const omitted = qw`config no_usage_stats force`;
    ...
    _load_config(){
        ...
        {
            const {lpm_token, customer, account_id, sync_config} =
                this.mgr._defaults||{};
