...
class Config {
    ...
    save(){
        ...
        try {
            ...
            this.log.error('Could not save config %s: %s',
    ...
    get_proxy_configs(){
        ...
        if (explicit_opt.port)
        {
            proxies.filter(p=>p.port==explicit_opt.port).forEach((p, i)=>{
                this.log.warn('Conflict between config Proxy #%s and '
    ...
    load_json(){
        ...
        try {
            ...
            if (!s)
                return {};
            this.log.notice(`Loaded config ${this.opt.filename}`);
            this.log.notice(`Running proxy configurations...`);
        } catch(e){ return {}; }
        try {
