...
class Config {
    ...
    save(){
        ...
        try {
            ...
            zerr.error('Could not save config %s: %s', this.opt.filename,
                e.message);
        }
    }
    get_string(){
    ...
    get_proxy_configs(){
        ...
        if (explicit_opt.port)
        {
            proxies.filter(p=>p.port==explicit_opt.port).forEach((p, i)=>{
                zerr.warn('Conflict between config Proxy #%s and '
    ...
    load_json(){
        ...
        try {
            ...
            if (!s)
                return {};
            zerr.notice(`Loaded config ${this.opt.filename}`);
            zerr.notice(`Running proxy configurations...`);
        } catch(e){ return {}; }
        try {
