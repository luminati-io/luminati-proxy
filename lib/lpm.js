...
class Lserver {
    constructor(opt = {}){
        this.opt = opt;
        this.port = opt.port||defaults.port;
        this.sp = etask('Lserver sp', function*(){ yield this.wait(); });
...
class Lsession_mgr {
    constructor(opt){
        this.sp = etask('Session_mgr sp', function*(){ yield this.wait(); });
    ...
    establish_session(opt = {}){
        const _this = this;
        this.log.debug('establish session');
...
class Lrequest {
    ...
    get_connect_headers(){
        ...
        console.log('connect_headers', headers);
        if (!this.sess)
            return headers;
        const cred = this.sess.get_creds();
...
class Lround_robin_pool {
    constructor(itm_constr, opt){
        if (!opt.no_sp)
        {
            this.sp = etask('Round_robin_pool sp', function*(){
                yield this.wait(); });
        }
...
class Lseq_pool extends Lround_robin_pool {
    constructor(itm_constr, opt){
        super(itm_constr, Object.assign({}, opt, {size: 1, no_sp: true}));
        this.sp = etask('Sequintial_pool sp', function*(){
            yield this.wait(); });
    }
}
...
class Lhosts {
    constructor(opt = {}){
        ...
        this.log.debug('create');
        this.sp = etask('Lhosts sp', function*(){ yield this.wait(); });
        this.hosts = [];
        this.proxy_count = opt.proxy_count || defaults.proxy_count;
        this.proxy_cache = opt._proxy_cache;
        this.conf_proxies = [].concat(opt.proxy||defaults.proxy);
...
class Lreq_router {
    constructor(opt){
        ...
        this.sp = etask('Lreq_router', function*(){ yield this.wait(); });
        this.log.debug('create');
        if (opt.bypass_proxy)
...
class Lproxy {
    constructor(opt){
        ...
        this.sp = etask('Lproxy sp', function*(){ yield this.wait(); });
