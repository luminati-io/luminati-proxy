...
class Lsession_mgr {
    ...
    establish_session(opt = {}){
        ...
        return etask(function*lsession_mgr_establish_session(){
            ...
            opt = Object.assign({}, _this.opt, {session, ip, vip, host}, opt);
...
class Lrequest {
    constructor(req, res, head, opt){
        ...
        this.proxy_port = opt.proxy_port||defaults.proxy_port;
        this.init_headers();
...
class Lresponse {
    ...
    write_head(headers = {}, opt = {}){
        ...
        const creds = lreq.sess.get_creds();
        if (creds)
...
class Lhttps_server {
    ...
    init(){
        const conf = this.ssl ? Object.assign({requestCert: false}, ssl())
            : {};
        this.srv = https.createServer(conf, this.handle_conn());
        ...
        this.srv.on('clientError', (err)=>console.log(err));
    }
    handle_conn(){
        return sock=>{
            this.log.debug('connection');
    ...
    accept(sock){
        if (!this.srv)
            this.init();
        // XXX maximk: FIX THIS
        console.log(this._connev.toString());
