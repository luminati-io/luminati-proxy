...
class Lserver {
    constructor(opt){
        ...
        this.null_resp = new Lnull_resp(opt);
        this.fw_chk_resp = new Lfw_chk_resp(opt);
        this.bypass_proxy = new Lbypass_proxy(opt);
    ...
    handle_lreq(lreq){
        ...
        let sp = etask(function*(){
            ...
            lres = yield _this.send_lreq(lreq);
            if (lres)
    ...
    send_lreq(lreq){
        const _this = this;
        if (this.fw_chk_resp.matches(lreq))
            return this.fw_chk_resp.send(lreq);
        if (this.null_resp.matches(lreq))
            return this.null_resp.send(lreq);
        if (this.only_bypass&&!this.bypass_proxy.match(lreq))
            return this.null_resp.send(lreq);
        return etask(function*(){
            _this.sp.spawn(this);
            if (_this.bypass_proxy.matches(lreq))
                return yield _this.bypass_proxy.send(lreq);
            let lproxy = _this.http_proxy;
            if (_this.https_proxy.matches(lreq))
...
class Lnull_resp {
    constructor(opt){
        if (opt.null_response)
            this.re = new RegExp(opt.null_response, 'i');
    }
    matches(lreq){
        return !!this.re&&this.re.test(lreq.url);
    }
    send(lreq){
...
class Lbypass_proxy {
    constructor(opt){
        this.opt = opt;
        this.sp = etask('Lbypass_proxy', [()=>this.wait()]);
        if (this.opt.bypass_proxy)
            this.re = new RegExp(opt.bypass_proxy, 'i');
        if (this.opt.proxy_internal_bypass)
            this.internal = this.opt.proxy_internal_bypass;

    }
    matches(lreq){
        if (this.re&&this.re.test(lreq.url))
            return true;
