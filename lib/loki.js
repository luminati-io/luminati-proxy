...
E.prototype.stats_process = function(data, gb_cost){
    ...
    this.stats_db_names.forEach(f=>{
        ...
        Object.assign(record, {
            in_bw: record.in_bw+data.in_bw,
            out_bw: record.out_bw+data.out_bw,
            bypass_bw: (record.bypass_bw||0)+data.bypass_bw,
            bypass_cost: (record.bypass_cost||0)+data.bypass_cost,
            reqs: record.reqs+1,
            success: record.success+data.success,
        });
        if (record['$loki'])
            return this.colls[f].update(record);
        this.colls[f].insert(record);
    });
};
