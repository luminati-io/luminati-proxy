...
E.prototype.stats_get = function(){
    ...
    const s = protocol.reduce(
        (acc, el)=>({
            reqs: acc.reqs+el.reqs,
            success: acc.success+el.success,
            start_ts: Math.min(acc.start_ts, el.start_ts||Infinity),
        }),
        {reqs: 0, success: 0, start_ts: Infinity});
    return {
        ...
        total: s.reqs,
        success: s.success,
        start_ts: s.start_ts==Infinity ? null : s.start_ts,
    };
};
