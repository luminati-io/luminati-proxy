...
const {HTTP2_HEADER_CONNECTION, HTTP2_HEADER_UPGRADE, HTTP2_HEADER_HOST,
    HTTP2_HEADER_HTTP2_SETTINGS, HTTP2_HEADER_KEEP_ALIVE, HTTP2_HEADER_TE,
    HTTP2_HEADER_PROXY_CONNECTION, HTTP2_HEADER_TRANSFER_ENCODING,
} = http2.constants;
...
// copied from Node's http2/util.js isIllegalConnectionSpecificHeader
const is_illegal_connection_specific_header = (name, value)=>{
    switch (name)
    {
...
const remove_invalid_http2_headers = headers=>{
    Object.keys(headers).forEach(h=>{
        if (is_illegal_connection_specific_header(h.toLowerCase(), headers[h]))
            delete headers[h];
    });
};
...
E.write_http_reply = (client_res, proxy_res, headers={}, opt={})=>{
    ...
    if (client_res instanceof http2.Http2ServerResponse)
    {
        // needed when target host does not agree to HTTP2
        remove_invalid_http2_headers(headers);
        delete proxy_res.statusMessage;
    }
    if (client_res instanceof http.ServerResponse ||
        client_res instanceof http2.Http2ServerResponse)
    {
        try {
