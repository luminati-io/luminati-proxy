...
const {Expiry_map} = require('../util/map.js');
const lpm_config = require('../util/lpm_config.js');
const pkg = require('../package.json');
const swagger = require('./swagger.json');
const {SSL_OP_NO_TLSv1_1, SOURCES} = require('./consts.js');
...
E.write_http_reply = (client_res, proxy_res, headers={}, opt={}, end=false)=>{
    ...
    if (opt.debug=='none' || hide_sensitive_info)
    {
        ...
        {
            const is_to_remove = ['x-luminati', 'x-hola', 'x-lpm']
                .some(h=>k.includes(h) && !k.includes('-error'));
            if (is_to_remove)
                delete headers[k];
        }
...
E.memoize = (func, {resolver, ttl}={})=>{
    resolver = typeof resolver == 'function' ? resolver : val=>val;
    const _memo = function(...args){
        const key = resolver(...args);
        if (_memo.cache.has(key))
            return _memo.cache.get(key);
        const cached = func(...args);
        _memo.cache.set(key, cached);
        return cached;
    };
    _memo.cache = ttl ? new Expiry_map(ttl) : new Map();
    return _memo;
};
