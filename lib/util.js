...
class Renewable_expiry_map {
    constructor(timeout){
        this.timeout = timeout || 10*date.ms.MIN;
        this._map = {};
        this.size = 0;
    }
    clear_timeout(key){
    ...
    set_timeout(key){
        this._map[key].timeout = setTimeout(this.delete.bind(this, key),
            this.timeout);
    }
    delete(key){
        if (!this.has(key))
            return;
        this.clear_timeout(key);
    ...
    set(key, value){
        this.delete(key);
        this._map[key] = {value};
        this.set_timeout(key);
        this.size++;
    }
    get(key, no_reset=false){
        if (!this.has(key))
            return null;
        if (!no_reset)
        {
    ...
    values(){
        return Object.values(this._map).map(({value})=>value);
    }
    for_each(cb){
        Object.entries(this._map).forEach(([key, {value}])=>cb(key, value));
    }
    to_obj(){
        return Object.entries(this._map).reduce((acc, [key, {value}])=>
            Object.assign(acc, {[key]: value}), {});
    }
    clear(){
        Object.keys(this._map).forEach(this.delete);
    }
}

E.Renewable_expiry_map = Renewable_expiry_map;
