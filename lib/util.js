...
E.get_status_tasks_msg = (tasks, opt={memory: true})=>{
    let msg = '';
    const fmt_num = n=>
        (+n).toLocaleString('en-GB', {maximumFractionDigits: 2});
    const total_mem_mb = os.totalmem() / 1000000;
    const get_task_str = (prefix, t)=>{
        let str = `${prefix} = CPU: ${fmt_num(t.cpu)}%`;
        if (opt.memory)
            str += `, Memory used: ${fmt_num(t.memory/100*total_mem_mb)}MB`;
        return str;
    };
