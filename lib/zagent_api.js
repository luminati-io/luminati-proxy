...
const Zagent_api = etask._class(class Zagent_api {
    ...
    *stress_test_ws(_this, req, res){
    ...
    create_server(app){
        ...
        if (https_server)
            return https_server;
        logger.warn('Running zagent server without SSL encryption! %s',
            err.message);
        return http.createServer({
            cert: process.env.SSL_CERT && file.read_e(process.env.SSL_CERT),
            key: process.env.SSL_KEY && file.read_e(process.env.SSL_KEY),
        }, app);
    }
