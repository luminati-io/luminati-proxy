...
const Zagent_api = etask._class(class Zagent_api {
    ...
    *enable_api(_this, req, res){
    ...
    get_lpm_stats(req, res){
        const days = req.query.days||5;
        const query = {hostname: {$ne: 'lumtest.com'}};
        const reqs = this.mgr.loki.requests_count(query);
        const recent_reqs = this.mgr.loki.requests_count(Object.assign({},
    ...
    *start(_this){
        ...
        app.get('/api/lpm_stats', _this.get_lpm_stats.bind(_this));
