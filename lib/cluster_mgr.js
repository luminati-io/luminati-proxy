...
class Cluster_mgr {
    ...
    run(){
        ...
        this.mgr.lpm_f.on('server_conf', this.on_server_conf.bind(this));
    }
    ...
    on_server_conf(server_conf){
        this.workers_running().forEach(w=>{
            w.send({
                code: 'SERVER_CONF',
                server_conf,
                zagent: this.mgr.argv.zagent,
                account_id: this.mgr._defaults.account_id,
            });
        });
    }
