...
class Cluster_mgr {
    ...
    run(){
        const num_workers = this.mgr.argv.cluster===true ?
            os.cpus().length : Number(this.mgr.argv.cluster)||1;
    ...
    init_worker(worker, level){
        ...
        worker.on('message', msg=>this.handle_worker_message(worker, msg));
    ...
    send_worker_setup(worker, level){
        this.worker_send(worker, {
            ...
            keys,
            new_proxy_cert: this.mgr._defaults.new_proxy_cert,
            ...
            cluster_cache: this.mgr.server_conf?.cloud?.cluster_cache,
        });
    }
    handle_worker_message(worker, msg){
        if (msg.code!='GET_PROXY_CERT')
            return;
        const mgr = this.mgr;
        const cmgr = this;
        etask(function*(){
            try {
                const cert = yield mgr.lpm_f.get_proxy_cert(msg.hostname);
                cmgr.worker_send(worker, {
                    code: 'GET_PROXY_CERT_RESULT',
                    id: msg.id,
                    cert,
                });
            } catch(e){
